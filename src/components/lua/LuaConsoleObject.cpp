...
namespace Ember
{
  namespace Lua
  {
    ...
    LuaConsoleObject::LuaConsoleObject(const std::string& command,
        const std::string& luaMethod, const std::string& description) :
        mCommandWrapper(command, this, description)
    {
      mConnector = new TemplatedConnectorBase<StringValueAdapter,
          StringValueAdapter>(StringValueAdapter(), StringValueAdapter());
      mConnector->connect(luaMethod);
    }

    LuaConsoleObject::LuaConsoleObject(const std::string& command,
        lua_Object luaMethod, const std::string& description) :
        mCommandWrapper(command, this, description)
    {
      mConnector = new TemplatedConnectorBase<StringValueAdapter,
          StringValueAdapter>(StringValueAdapter(), StringValueAdapter());
      ...
      if (luaType == LUA_TFUNCTION)
        ...
        {
          S_LOG_WARNING(
              "No valid lua function sent as argument to LuaConsoleObject::LuaConsoleObject");
        }
    }
    ...
    void
    LuaConsoleObject::runCommand(const std::string& command,
