...
namespace Ember
{
  ...
  namespace Lua
  {
    ...
    public:
       ...
       */
      Connector*
      connect(const std::string& luaMethod, lua_Object selfIndex = LUA_NOREF);
       ...
       */
      Connector*
      connect(lua_Object luaMethod, lua_Object selfIndex = LUA_NOREF);
       ...
       */
      static Connector
      createConnector(sigc::signal<void>* signal);
      ...
      template<typename TAdapter0, typename TSignal>
        static Connector
        createConnector(TSignal* signal);
    ...
    private:
       ...
       */
      bool
      checkSignalExistence(void* signal);
    ...
    template<typename TSignal, typename TAdapter0,
        typename TAdapter1 = EmptyValueAdapter>
    ...
    template<typename TAdapter0, typename TSignal>
      Connector::Connector(TSignal& signal) :
          mConnector(
              new ConnectorOne<typename TSignal::result_type, TAdapter0,
                  typename TAdapter0::value_type>(signal, TAdapter0()))
      {
      }
    ...
    template<typename TAdapter0, typename TSignal>
      Connector::Connector(TSignal& signal, TAdapter0& adapter0) :
          mConnector(
              new ConnectorOne<typename TSignal::result_type, TAdapter0,
                  typename TAdapter0::value_type>(signal, adapter0))
      {
      }
    ...
    template<typename TAdapter0, typename TAdapter1, typename TSignal>
      Connector::Connector(TSignal& signal) :
          mConnector(
              new ConnectorTwo<typename TSignal::result_type, TAdapter0,
                  TAdapter1, typename TAdapter0::value_type,
                  typename TAdapter1::value_type>(signal, TAdapter0(),
                  TAdapter1()))
      {
      }
    ...
    template<typename TAdapter0, typename TAdapter1, typename TSignal>
      Connector::Connector(TSignal& signal, TAdapter0& adapter0,
          ...
          mConnector(
              new ConnectorTwo<typename TSignal::result_type, TAdapter0,
                  TAdapter1, typename TAdapter0::value_type,
                  typename TAdapter1::value_type>(signal, adapter0, adapter1))
      {
      }

    template<typename TAdapter0, typename TSignal>
      Connector
      Connector::createConnector(TSignal* signal)
      {
        return Connector(
            new ConnectorOne<typename TSignal::result_type, TAdapter0,
                typename TAdapter0::value_type>(*signal, TAdapter0()));
