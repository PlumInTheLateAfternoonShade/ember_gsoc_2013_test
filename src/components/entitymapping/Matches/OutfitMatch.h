...
namespace Ember
{
  ...
  namespace EntityMapping
  {
    ...
    namespace Matches
    {
       ...
       */
      class OutfitMatch : public AbstractMatch<Cases::OutfitCase>,
          public AttributeDependentMatch
      {
      ...
      public:
        ...
        virtual void
        testAttribute(const Atlas::Message::Element& attribute,
            bool triggerEvaluation = false);

        const std::string&
        getOutfitName();
        virtual void
        setEntity(Eris::Entity* entity);

        void
        setEntityCreationObserver(Observers::EntityCreationObserver* observer);

      protected:

        void
        testEntity(Eris::Entity* entity);
