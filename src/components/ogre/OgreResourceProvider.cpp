...
namespace Ember
{
  namespace OgreView
  {
    ...
    const char*
    OgreResourceWrapper::getDataPtr()
    {
      return mBuffer;
    }

    bool
    OgreResourceWrapper::hasData()
    {
      return mSize != 0;
    }

    size_t
    OgreResourceWrapper::getSize()
    {
      return mSize;
    }
    ...
    ResourceWrapper
    OgreResourceProvider::getResource(const std::string& name)
    {
      Ogre::DataStreamPtr input =
          Ogre::ResourceGroupManager::getSingleton().openResource(name,
              mGroupName);

      if (input.isNull())
        {
          throw Exception(
              "Unable to open resource file '" + name + "' in resource group '"
                  + name + "'.");
        }
