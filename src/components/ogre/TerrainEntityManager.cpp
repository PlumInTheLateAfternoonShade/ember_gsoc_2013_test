...
namespace Ember
{
  namespace OgreView
  {
    ...
    TerrainEntityManager::TerrainEntityManager(Eris::View& view,
        Terrain::TerrainHandler& terrainHandler,
        Ogre::SceneManager& sceneManager) :
        mView(view), mTerrainHandler(terrainHandler), mSceneManager(
            sceneManager)
    {
      view.EntitySeen.connect(
          sigc::mem_fun(*this, &TerrainEntityManager::entitySeen));
      view.EntityCreated.connect(
          sigc::mem_fun(*this, &TerrainEntityManager::entitySeen));
      view.TopLevelEntityChanged.connect(
          sigc::mem_fun(*this, &TerrainEntityManager::topLevelEntityChanged));
    }
    ...
    void
    TerrainEntityManager::topLevelEntityChanged()
    {
      EmberEntity* entity = static_cast<EmberEntity*>(mView.getTopLevel());
      entity->setAttachment(
          new WorldAttachment(*entity,
              mSceneManager.getRootSceneNode()->createChildSceneNode(
                  "entity_" + entity->getId()), mTerrainHandler));

      initializeTerrain(entity);
    }

    void
    TerrainEntityManager::initializeTerrain(EmberEntity* entity)
    {
      ...
      if (entity->hasAttr("terrain"))
        {
          ...
          const Atlas::Message::Element& terrainElement = entity->valueOfAttr(
              "terrain");
          if (terrainElement.isMap())
            {
              ...
              if (terrainMap.count("surfaces"))
                {
                  const Atlas::Message::Element& surfaceElement(
                      terrainMap.find("surfaces")->second);
                  terrainShaderParser.createShaders(surfaceElement);
                  hasValidShaders = true;
                }
            }
          mTerrainHandler.updateTerrain(
              terrainParser.parseTerrain(terrainElement,
                  entity->getPosition().isValid() ?
                      entity->getPosition() : WFMath::Point<3>::ZERO()));
...
//	mEnvironment->setWorldPosition(mWorldPosition.LongitudeDegrees, mWorldPosition.LatitudeDegrees);
      ...
      mTopLevelTerrainConnection = entity->observe("terrain",
          sigc::bind(
              sigc::mem_fun(*this, &TerrainEntityManager::terrainChanged),
              entity));
    }

    void
    TerrainEntityManager::terrainChanged(const Atlas::Message::Element& value,
        EmberEntity* entity)
    {
      Terrain::TerrainParser terrainParser;
      updateTerrain(
          terrainParser.parseTerrain(value, entity->getPredictedPos()));
    }

    void
    TerrainEntityManager::updateTerrain(
        const std::vector<Terrain::TerrainDefPoint>& terrainDefinitionPoints)
    {
      mTerrainHandler.updateTerrain(terrainDefinitionPoints);
    }

    void
    TerrainEntityManager::entitySeen(Eris::Entity* entity)
    {
      registerEntity(static_cast<EmberEntity*>(entity));
    }

    void
    TerrainEntityManager::registerEntity(EmberEntity* entity)
    {
      if (entity->hasAttr("area"))
      ...
      else
        {
          entity->observe("area",
              sigc::bind(
                  sigc::mem_fun(*this,
                      &TerrainEntityManager::entityAreaAttrChanged), entity));
        }

      if (entity->hasAttr("terrainmod"))
        {
          entityTerrainModAttrChanged(entity->valueOfAttr("terrainmod"),
              entity);
        }
      else
        {
          entity->observe("terrainmod",
              sigc::bind(
                  sigc::mem_fun(*this,
                      &TerrainEntityManager::entityTerrainModAttrChanged),
                  entity));
        }
    }

    void
    TerrainEntityManager::entityTerrainAttrChanged(
        const Atlas::Message::Element& value, EmberEntity* entity)
    {
      ...
      if (mTerrainMods.find(entity) == mTerrainMods.end())
        {
          ...
          entity->BeingDeleted.connect(
              sigc::bind(
                  sigc::mem_fun(*this,
                      &TerrainEntityManager::entityBeingDeletedTerrainMod),
                  entity));
    ...
    void
    TerrainEntityManager::entityAreaAttrChanged(
        const Atlas::Message::Element& value, EmberEntity* entity)
    {
          ...
          if (area->init())
            {
              ...
              entity->BeingDeleted.connect(
                  sigc::bind(
                      sigc::mem_fun(*this,
                          &TerrainEntityManager::entityBeingDeletedArea),
                      entity));
            }
        }
    }
    void
    TerrainEntityManager::entityBeingDeletedTerrainMod(EmberEntity* entity)
    {
    ...
    void
    TerrainEntityManager::entityBeingDeletedArea(EmberEntity* entity)
    {
