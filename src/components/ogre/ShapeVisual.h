...
namespace Ember
{
  namespace OgreView
  {
    ...
    template<typename T>
      inline void
      ShapeVisual::update(const T& shape)
      {
        mManualObject->clear();

        if (shape.isValid() && shape.numCorners() > 1)
          {
            mManualObject->begin("/global/authoring/polygon/line",
            ...
            if (mClosed)
              {
                ...
                if (shape.numCorners() > 2)
                  {
                    mManualObject->position(
                        Convert::toOgre(shape.getCorner(0)));
                  }
              }
            mManualObject->end();
          }
