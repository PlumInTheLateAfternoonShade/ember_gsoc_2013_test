...
namespace Ember
{
  namespace OgreView
  {
    ...
    void
    MeshSerializerListener::processMaterialName(Ogre::Mesh* mesh,
        Ogre::String* name)
    {
    ...
    void
    MeshSerializerListener::processSkeletonName(Ogre::Mesh *mesh,
        Ogre::String *name)
    {
      ...
      if (Ogre::StringUtil::startsWith(*name, "../")
          || Ogre::StringUtil::startsWith(*name, "./"))
        {
          ...
          Ogre::vector<std::string>::type skeletonPathSegments =
              Ogre::StringUtil::split(*name, "/");
          Ogre::vector<std::string>::type pathSegments =
              Ogre::StringUtil::split(path, "/");

          //For every ".." part in the skeleton path, walk upwards in the directory hierarcy
          while (*skeletonPathSegments.begin() == ".."
              || *skeletonPathSegments.begin() == ".")
            {
          ...
          std::stringstream ss;
          for (Ogre::vector<std::string>::type::const_iterator I =
              pathSegments.begin(); I != pathSegments.end(); ++I)
            {
          ...
          for (Ogre::vector<std::string>::type::const_iterator I =
              skeletonPathSegments.begin(); I != skeletonPathSegments.end();
              ++I)
            {
              ss << "/" << *I;
            }
