...
namespace Ember
{
  namespace OgreView
  {
    ...
    AttachPointNode::AttachPointNode(ModelBlock* modelBlock,
        Ogre::SceneNode* modelNode, const Carpenter::AttachPoint* attachPoint,
        Ogre::ColourValue colour, Ogre::BillboardSet* billboardSet) :
        mModelBlock(modelBlock), mModelNode(modelNode), mAttachPoint(
            attachPoint), mColour(colour), mController(0)
    {
      Ogre::Vector3 position = Convert::toOgre(attachPoint->getPosition());
      std::string name = modelNode->getName() + "_"
          + attachPoint->getAttachPair()->getName() + "_"
          + attachPoint->getName();
...
//	mFlare = billboardSet->createBillboard(0,0,0 , colour);
    ...
    void
    Jesus::addAttachPointType(const std::string & type)
    {

      AttachPointColourValueMap::iterator J = mColourMap.find(type);
      if (J == mColourMap.end())
        {
          Ogre::ColourValue colour = Ogre::ColourValue(0.5, 0.5,
              0.1 * mColourMap.size());
          mColourMap.insert(
              AttachPointColourValueMap::value_type(type, colour));
        }

    }

    bool
    Jesus::addBluePrint(Carpenter::BluePrint* blueprint)
    {
      ...
      mBlueprints.insert(
          std::map<std::string, Carpenter::BluePrint*>::value_type(
              blueprint->getName(), blueprint));
      return true;
    }

    Carpenter::BluePrint*
    Jesus::getBluePrint(const std::string& name) const
    {
      std::map<std::string, Carpenter::BluePrint*>::const_iterator I =
          mBlueprints.find(name);
    ...
    Construction::Construction(Carpenter::BluePrint* bluePrint, Jesus* jesus,
        Ogre::SceneNode* node) :
    ...
    {
      for (std::vector<ModelBlock*>::iterator I = mModelBlocks.begin();
          I != mModelBlocks.end(); ++I)
        {
          delete *I;
        }
//	mBaseNode->removeAndDestroyAllChildren();
    ...
    void
    Construction::buildFromBluePrint(bool createAttachPointNodes)
    {
      std::vector<Carpenter::BuildingBlock*> buildingBlocks =
          mBlueprint->getAttachedBlocks();

      std::vector<Carpenter::BuildingBlock*>::const_iterator J =
          buildingBlocks.begin();
      std::vector<Carpenter::BuildingBlock*>::const_iterator J_end =
          buildingBlocks.end();
    ...
    ModelBlock*
    Construction::createModelBlock(
        const Carpenter::BuildingBlock* buildingBlock,
        bool createAttachPointNodes)
    {
      std::string blockSpecName =
          buildingBlock->getBuildingBlockSpec()->getName();

      Model::Model* model = mJesus->createModelForBlockType(blockSpecName,
          mBaseNode->getName() + buildingBlock->getName());
      ModelBlock* modelBlock = new ModelBlock(mBaseNode, buildingBlock, model,
          this);
    ...
    bool
    Construction::remove(ModelBlock* modelBlock)
    {
    ...
    std::vector<ModelBlock*>
    Construction::getModelBlocks() const
    {
      return mModelBlocks;
    }

    std::vector<AttachPointNode*>
    ModelBlock::getAttachPointNodes() const
    {
      return mAttachPointNodes;
    }

    Model::Model*
    Jesus::createModelForBlockType(const std::string& blockType,
        const std::string& modelName)
    {
      std::map<std::string, std::string>::iterator I = mEntityMappings.find(
          blockType);
      if (I == mEntityMappings.end())
        {
          return 0;
        }
      Model::Model* aModel = Model::Model::createModel(
          *EmberOgre::getSingleton().getSceneManager(), I->second, modelName);
...
//	return Model::Create(I->second + ".modeldef.xml", modelName);
    ...
    ModelBlock::ModelBlock(Ogre::SceneNode* baseNode,
        const Carpenter::BuildingBlock* buildingBlock, Model::Model* model,
        Construction* construction) :
        mBuildingBlock(buildingBlock), mModel(model), mModelNode(0), mConstruction(
            construction)
    {

      mNode = baseNode->createChildSceneNode(
          Convert::toOgre(buildingBlock->getPosition()),
          Convert::toOgre(buildingBlock->getOrientation()));

      mPointBillBoardSet = mNode->getCreator()->createBillboardSet(
          std::string("__construction_")
              + construction->getBluePrint()->getName() + "_"
              + buildingBlock->getName() + "_billboardset__"
              + mNode->getName());
...
//	Ogre::SceneNode* aNode = EmberOgre::getSingleton().getSceneManager()->getRootSceneNode()->createChildSceneNode();
      ...
      if (model)
        {
          ...
          if (model->getDefinition()->getScale() == 0)
            {
              const Ogre::AxisAlignedBox ogreBoundingBox =
                  model->getBoundingBox();
              const Ogre::Vector3 ogreMax = ogreBoundingBox.getMaximum();
              const Ogre::Vector3 ogreMin = ogreBoundingBox.getMinimum();

              const WFMath::AxisBox<3> wfBoundingBox =
                  buildingBlock->getBuildingBlockSpec()->getBlockSpec()->getBoundingBox();
    ...
    ModelBlock::createAttachPointNodes()
    {
      std::vector<const Carpenter::AttachPoint*> allPoints =
          mBuildingBlock->getAllPoints();

      std::vector<const Carpenter::AttachPoint*>::const_iterator I =
          allPoints.begin();
      std::vector<const Carpenter::AttachPoint*>::const_iterator I_end =
          allPoints.end();

      for (; I != I_end; ++I)
        {
          Ogre::ColourValue colour =
              mConstruction->getJesus()->getColourForAttachPoint(*I);
          AttachPointNode* pointNode = new AttachPointNode(this, mNode, *I,
              colour, mPointBillBoardSet);
    ...
    Ogre::ColourValue
    Jesus::getColourForAttachPoint(const Carpenter::AttachPoint* point) const
    {
      AttachPointColourValueMap::const_iterator I = mColourMap.find(
          point->getAttachPair()->getType());
      if (I == mColourMap.end())
        {
    ...
    AttachPointNode::select()
    {
      ...
      Ogre::ControllerManager& contMgr =
      ...
      mController = contMgr.createController(contMgr.getFrameTimeSource(), val,
          func);
    }

    void
    AttachPointNode::deselect()
    {
    ...
    LightWibbler::LightWibbler(const Ogre::ColourValue& originalColour,
    ...
    Ogre::Real
    LightWibbler::getValue(void) const
    {
      return intensity;
    }

    void
    LightWibbler::setValue(Ogre::Real value)
    {
      intensity = value;
