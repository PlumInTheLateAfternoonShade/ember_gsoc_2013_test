...
namespace Ember
{
  namespace OgreView
  {
    ...
    OgreSetup::OgreSetup() :
        DiagnoseOgre("diagnoseOgre", this,
            "Diagnoses the current Ogre state and writes the output to the log."), mRoot(
            0), mRenderWindow(0), mSceneManagerFactory(0), mMeshSerializerListener(
            0)
#ifdef BUILD_WEBEMBER
    ...
    void
    OgreSetup::runCommand(const std::string& command, const std::string& args)
    {
      if (DiagnoseOgre == command)
        {
          ...
          S_LOG_INFO(ss.str());
          ConsoleBackend::getSingleton().pushMessage(
              "Ogre diagnosis information has been written to the log.",
              "info");
        }
    }
    ...
    Ogre::Root*
    OgreSetup::createOgreSystem()
    {
      ConfigService& configSrv(
          EmberServices::getSingleton().getConfigService());
      ...
      if (configSrv.getPrefix() != "")
        {
          ...
          if (chdir(configSrv.getPrefix().c_str()))
            {
              S_LOG_WARNING(
                  "Failed to change to the prefix directory '" << configSrv.getPrefix() << "'. Ogre loading might fail.");
            }
        }
          ...
          while (token != "")
            {
              ...
              for (std::vector<std::string>::iterator I =
                  pluginLocations.begin(); I != pluginLocations.end(); ++I)
                {
                  pluginPath = (*I) + "/" + token + pluginExtension;
                  S_LOG_INFO(
                      "Trying to load the plugin '" << pluginPath << "'.");
                  try
                    {
              ...
              if (pluginLoaded)
                {
                  S_LOG_INFO(
                      "Successfully loaded the plugin '" << token << "' from '" << pluginPath << "'.");
                }
              else
                {
      ...
      if (chdir(configSrv.getEmberDataDirectory().c_str()))
        {
          S_LOG_WARNING(
              "Failed to change to the data directory '" << configSrv.getEmberDataDirectory() << "'.");
        }
    ...
    bool
    OgreSetup::showConfigurationDialog()
    {
      ...
      if (result == OgreConfigurator::OC_ADVANCED_OPTIONS)
      ...
      else
        {
          mRoot->setRenderSystem(
              mRoot->getRenderSystemByName(
                  configurator.getChosenRenderSystemName()));
          const Ogre::ConfigOptionMap& configOptions =
              configurator.getConfigOptions();
          for (Ogre::ConfigOptionMap::const_iterator I = configOptions.begin();
              I != configOptions.end(); ++I)
            {
              mRoot->getRenderSystem()->setConfigOption(I->first,
    ...
    OgreSetup::configure(void)
    {
      ...
      if (configService.itemExists("ogre", "suppressconfigdialog"))
        {
          suppressConfig = static_cast<bool>(configService.getValue("ogre",
              "suppressconfigdialog"));
        }
      try
        {
        ...
        {
          S_LOG_WARNING(
              "Error when showing config dialog. Will try to remove ogre.cfg file and retry." << ex);
          unlink(
              (EmberServices::getSingleton().getConfigService().getHomeDirectory()
                  + "/ogre.cfg").c_str());
          try
            {
      ...
      parseWindowGeometry(mRoot->getRenderSystem()->getConfigOptions(), width,
          height, fullscreen);
      ...
      std::string windowId = Input::getSingleton().createWindow(width, height,
          fullscreen, true, true, handleOpenGL);
      ...
      mRenderWindow = mRoot->createRenderWindow("MainWindow", width, height,
          false, &misc);

      Input::getSingleton().EventSizeChanged.connect(
          sigc::mem_fun(*this, &OgreSetup::input_SizeChanged));
    ...
    void
    OgreSetup::input_SizeChanged(unsigned int width, unsigned int height)
    {
...
//On Windows we can't tell the window to resize, since that will lead to an infinite loop of resize events (probably stemming from how Windows lacks a proper window manager).
    ...
    Ogre::SceneManager*
    OgreSetup::chooseSceneManager()
    {
      return mRoot->createSceneManager(Ogre::ST_GENERIC, "DefaultSceneManager");
    }

    void
    OgreSetup::parseWindowGeometry(Ogre::ConfigOptionMap& config,
          ...
          if (pos != Ogre::String::npos)
            {
              ...
              width = Ogre::StringConverter::parseUnsignedInt(
                  val.substr(0, pos));
              height = Ogre::StringConverter::parseUnsignedInt(
                  val.substr(pos + 1));
            }
        }
