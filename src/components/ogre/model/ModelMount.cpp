...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Model
    {
      ...
      void
      ModelMount::rescale(const WFMath::AxisBox<3>* wfBbox)
      {
        scaleNode(wfBbox);
      }
        ...
        if (mPose != "")
          {
            const PoseDefinitionStore& poses =
                mModel.getDefinition()->getPoseDefinitions();
      ...
      Ogre::Node&
      ModelMount::getNode() const
      {
        return mNodeProvider->getNode();
      }

      void
      ModelMount::scaleNode(const WFMath::AxisBox<3>* wfBbox)
      {
        if (!mModel.isLoaded())
          {
            return;
        ...
        if (!defaultOgreBoundingBox.isNull()
            && (defaultSize.x != 0.0f && defaultSize.y != 0.0f
                && defaultSize.z != 0.0f))
          {
            ...
            localTransform.makeTransform(
                getModel().getDefinition()->getTranslate(),
                Ogre::Vector3::UNIT_SCALE,
                getModel().getDefinition()->getRotation());
            ...
            if (wfBbox && wfBbox->isValid())
              {
                ...
                case ModelDefinition::MODEL_HEIGHT:
                  scaleX = scaleY = scaleZ = fabs(
                      (ogreMax.y - ogreMin.y) / defaultSize.y);
                  break;
                case ModelDefinition::MODEL_WIDTH:
                  scaleX = scaleY = scaleZ = fabs(
                      (ogreMax.x - ogreMin.x) / defaultSize.x);
                  break;
                case ModelDefinition::MODEL_DEPTH:
                  scaleX = scaleY = scaleZ = fabs(
                      (ogreMax.z - ogreMin.z) / defaultSize.z);
                  break;
            ...
            else if (!getModel().getScale())
              {
                ...
                S_LOG_WARNING(
                    "Could not find any scale set in the model '" << getModel().getName() << "' of type '" << getModel().getDefinition()->getName() << "'. We'll thus default to scaling the mesh so it's 0.25 meters in each dimension.");
                ...
                if (getModel().getScale() != 1)
                  {
                    ...
                    getNode().scale(getModel().getScale(),
                        getModel().getScale(), getModel().getScale());
                  }
              }
                ...
                if (I != poses.end())
                  {
                    ...
                    //rotate node to fit with WF space
                    //perhaps this is something to put in the model spec instead?
                    orientation.FromAngleAxis(Ogre::Degree(90),
                        Ogre::Vector3::UNIT_Y);
                    Ogre::Vector3 translation = (orientation
                        * (I->second.Translate * getNode().getScale()));
                    orientation = orientation * I->second.Rotate;

                    getNodeProvider()->setPositionAndOrientation(translation,
                        orientation);
                  }
              }
      ...
      INodeProvider*
      ModelMount::getNodeProvider() const
      {
