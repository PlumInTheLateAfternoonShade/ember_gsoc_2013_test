  ...
  namespace OgreView
  {
    namespace Model
    {
      ...
      ModelAttachment::ModelAttachment(EmberEntity& parentEntity,
          ModelRepresentation& modelRepresentation, INodeProvider* nodeProvider,
          const std::string& pose) :
          NodeAttachment(parentEntity, modelRepresentation.getEntity(),
              nodeProvider), mModelRepresentation(modelRepresentation), mModelMount(
              0), mIgnoreEntityData(false), mPose(pose)
      {
        if (pose != "")
          {
            const PoseDefinitionStore& poses =
                mModelRepresentation.getModel().getDefinition()->getPoseDefinitions();
      ...
      ModelAttachment::~ModelAttachment()
      {
        ...
        for (ModelFittingStore::iterator I = mFittings.begin();
            I != mFittings.end(); ++I)
          {
            delete I->second;
          }

        for (AttributeObserverStore::iterator I = mFittingsObservers.begin();
            I != mFittingsObservers.end(); ++I)
          {
            delete *I;
          }
      ...
      void
      ModelAttachment::init()
      {
        NodeAttachment::init();

        mModelMount = new ModelMount(mModelRepresentation.getModel(),
            mNodeProvider->createChildProvider(
                &mModelRepresentation.getModel()), mPose);
        mModelMount->reset();
        setupFittings();
        mModelRepresentation.getModel().Reloaded.connect(
            sigc::mem_fun(*this, &ModelAttachment::model_Reloaded));
      }

      IGraphicalRepresentation*
      ModelAttachment::getGraphicalRepresentation() const
      {
        return &mModelRepresentation;
      }

      IEntityAttachment*
      ModelAttachment::attachEntity(EmberEntity& entity)
      {
            ...
            if (I->second->getChildEntityId() == entity.getId())
              {
                ...
                entity.BeingDeleted.connect(
                    sigc::bind(
                        sigc::mem_fun(*this,
                            &ModelAttachment::fittedEntity_BeingDeleted),
                        &entity));
                break;
              }
          }
        ...
        if (attachPoint == ""
            && !mModelRepresentation.getModel().getDefinition()->getShowContained())
          {
        ...
        else
          {
            ModelRepresentation* modelRepresentation =
                ModelRepresentationManager::getSingleton().getRepresentationForEntity(
                    entity);
            ...
            if (attachPoint != "")
                ...
                if (mModelRepresentation.getModel().isLoaded())
                  {
                    try
                      {
                        const AttachPointDefinitionStore& attachpoints =
                            mModelRepresentation.getModel().getDefinition()->getAttachPointsDefinitions();
                        for (AttachPointDefinitionStore::const_iterator I =
                            attachpoints.begin(); I != attachpoints.end(); ++I)
                          {
                        ...
                        if (modelRepresentation)
                          ...
                          {
                            movable = OGRE_NEW Ogre::ManualObject(
                                OgreInfo::createUniqueResourceName(
                                    "ModelAttachedObject_"
                                        + entity.getType()->getName()));
                            deleteMovableWhenDone = true;
                          }

                        nodeProvider = new ModelBoneProvider(
                            mModelRepresentation.getModel(), attachPoint,
                            movable, deleteMovableWhenDone);
                      }
                    catch (const std::exception& ex)
                      {
                        S_LOG_WARNING(
                            "Failed to attach to attach point '"<< attachPoint << "' on model '" << mModelRepresentation.getModel().getDefinition()->getName() << "'.");
            ...
            else
              {
                nodeProvider = mNodeProvider->createChildProvider(
                    modelRepresentation ? &modelRepresentation->getModel() : 0);
              }
            NodeAttachment* nodeAttachment(0);
            if (modelRepresentation)
              {
                nodeAttachment = new ModelAttachment(getAttachedEntity(),
                    *modelRepresentation, nodeProvider, pose);
              }
            else
              {
                nodeAttachment = new NodeAttachment(getAttachedEntity(), entity,
                    nodeProvider);
              }
            nodeAttachment->init();
      ...
      void
      ModelAttachment::getOffsetForContainedNode(
          const IEntityAttachment& attachment,
          const WFMath::Point<3>& localPosition, WFMath::Vector<3>& offset)
      {
        //if the model has an offset specified, use that, else just send to the base class
        const Ogre::Vector3& modelOffset(
            mModelRepresentation.getModel().getDefinition()->getContentOffset());
        if (modelOffset != Ogre::Vector3::ZERO)
        ...
        else
          {
            ...
            for (ModelFittingStore::iterator I = mFittings.begin();
                I != mFittings.end(); ++I)
              {
                if (I->second->getChildEntityId()
                    == attachment.getAttachedEntity().getId())
                  {
                    return;
                  }
              }
            NodeAttachment::getOffsetForContainedNode(attachment, localPosition,
                offset);
          }
      }

      void
      ModelAttachment::setVisible(bool visible)
      {
      ...
      void
      ModelAttachment::entity_AttrChanged(
          const Atlas::Message::Element& attributeValue,
          const std::string& fittingName)
      {
      ...
      void
      ModelAttachment::fittedEntity_BeingDeleted(EmberEntity* entity)
      {
        for (ModelFittingStore::iterator I = mFittings.begin();
            I != mFittings.end(); ++I)
          {
      ...
      ModelAttachment::setupFittings()
      {
        ...
        for (AttachPointDefinitionStore::const_iterator I =
            attachpoints.begin(); I != attachpoints.end(); ++I)
          {
            AttributeObserver* observer = new AttributeObserver(mChildEntity,
                I->Name, ".");
            observer->EventChanged.connect(
                sigc::bind(
                    sigc::mem_fun(*this, &ModelAttachment::entity_AttrChanged),
                    I->Name));
      ...
      void
      ModelAttachment::detachFitting(EmberEntity& entity)
      {
      ...
      void
      ModelAttachment::createFitting(const std::string& fittingName,
          const std::string& entityId)
      {
        ModelFitting* fitting = new ModelFitting(mChildEntity, fittingName,
            entityId);
      ...
      void
      ModelAttachment::model_Reloaded()
      {
        if (mModelMount)
          {
            mModelMount->reset();
      ...
      void
      ModelAttachment::reattachEntities()
      {
        for (ModelFittingStore::iterator I = mFittings.begin();
            I != mFittings.end(); ++I)
          {
            if (I->second->getChild()
                && mChildEntity.hasChild(I->second->getChildEntityId()))
              {
                EmberEntity* entity = I->second->getChild();
                IEntityAttachment* attachment = attachEntity(*entity);
                entity->setAttachment(attachment);
      ...
      void
      ModelAttachment::setVisualize(const std::string& visualization,
          bool visualize)
      {
        if (visualization == "OgreBBox")
          {
            if (mModelMount && mModelMount->getNodeProvider())
              {
                mModelMount->getNodeProvider()->setVisualize(visualization,
                    visualize);
              }
          }
        else
      ...
      bool
      ModelAttachment::getVisualize(const std::string& visualization) const
      {
        if (visualization == "OgreBBox")
          {
            if (mModelMount && mModelMount->getNodeProvider())
              {
                return mModelMount->getNodeProvider()->getVisualize(
                    visualization);
              }
            else
              {
      ...
      void
      ModelAttachment::setPosition(const WFMath::Point<3>& position,
        ...
        if (!mIgnoreEntityData)
          {
            ...
            mModelRepresentation.setLocalVelocity(
                WFMath::Vector<3>(velocity).rotate(orientation.inverse()));
          }
        else
          {
            NodeAttachment::setPosition(WFMath::Point<3>::ZERO(),
                WFMath::Quaternion::Identity(), WFMath::Vector<3>::ZERO());
          }
      }
