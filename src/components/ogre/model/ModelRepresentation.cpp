...
namespace Ember
{
  namespace OgreView
  {
    namespace Model
    {
      ...
      ModelRepresentation::ModelRepresentation(EmberEntity& entity,
          Model& model, Scene& scene, EntityMapping::EntityMapping& mapping) :
          mEntity(entity), mModel(model), mScene(scene), mMapping(mapping), mCurrentMovementAction(
              0), mActiveAction(0), mTaskAction(0), mSoundEntity(0), mMovementMode(
              MM_DEFAULT)
      {
        mEntity.Acted.connect(
            sigc::mem_fun(*this, &ModelRepresentation::entity_Acted));
        mEntity.Changed.connect(
            sigc::mem_fun(*this, &ModelRepresentation::entity_Changed));
        mEntity.TaskAdded.connect(
            sigc::mem_fun(*this, &ModelRepresentation::entity_TaskAdded));
        mEntity.TaskRemoved.connect(
            sigc::mem_fun(*this, &ModelRepresentation::entity_TaskRemoved));
...
//listen for reload or reset events from the model. This allows us to alter model definitions at run time and have the in game entities update.
        mModel.Reloaded.connect(
            sigc::mem_fun(*this, &ModelRepresentation::model_Reloaded));
        mModel.Resetting.connect(
            sigc::mem_fun(*this, &ModelRepresentation::model_Resetting));
        ...
        Action* idleaction = mModel.getAction(ActivationDefinition::MOVEMENT,
            ACTION_STAND);
        if (idleaction)
          {
      ...
      ModelRepresentation::~ModelRepresentation()
      {
        ...
        const RenderingDefinition* renderingDef =
            mModel.getDefinition()->getRenderingDefinition();
        if (renderingDef && renderingDef->getScheme() != "")
          {
            mScene.deregisterEntityWithTechnique(mEntity,
                renderingDef->getScheme());
          }
      ...
      const std::string&
      ModelRepresentation::getType() const
      {
        return sTypeName;
      }

      const std::string&
      ModelRepresentation::getTypeNameForClass()
      {
        return sTypeName;
      }
      EmberEntity &
      ModelRepresentation::getEntity() const
      {
        return mEntity;
      }

      Model &
      ModelRepresentation::getModel() const
      {
        return mModel;
      }

      void
      ModelRepresentation::setModelPartShown(const std::string& partName,
          bool visible)
      {
        if (mModel.isLoaded())
          {
            ...
            if (!mModel.getUserAny().isEmpty())
              {
                EmberEntityUserObject* userObject =
                    Ogre::any_cast<EmberEntityUserObject::SharedPtr>(
                        mModel.getUserAny()).get();
       ...
       */
      bool
      ModelRepresentation::needSoundEntity()
      {
        const ActionDefinitionsStore& store =
            mModel.getDefinition()->getActionDefinitions();
        ...
        for (; I_b != I_e; ++I_b)
          {
            ...
            SoundDefinitionsStore::const_iterator I_sounds =
                (*I_b)->getSoundDefinitions().begin();
            SoundDefinitionsStore::const_iterator I_sounds_end =
                (*I_b)->getSoundDefinitions().end();
      ...
      void
      ModelRepresentation::setClientVisible(bool visible)
      {
        ...
        for (LightSet::iterator I = mModel.getLights().begin();
            I != mModel.getLights().end(); ++I)
          {
      ...
      void
      ModelRepresentation::initFromModel()
      {
        connectEntities();
        ...
        if (renderingDef && renderingDef->getScheme() != ""
            && mModel.isLoaded())
          {
            mScene.registerEntityWithTechnique(mEntity,
                renderingDef->getScheme());
...
//		forest->addEmberEntity(this);
      ...
      Ogre::Vector3
      ModelRepresentation::getScale() const
      {
      ...
      void
      ModelRepresentation::connectEntities()
      {
        ...
        ICollisionDetector* collisionDetector = new MeshCollisionDetector(
            &getModel());
        EmberEntityUserObject* userObject = new EmberEntityUserObject(
            getEntity(), collisionDetector);
        getModel().setUserAny(
            Ogre::Any(EmberEntityUserObject::SharedPtr(userObject)));

      }

      void
      ModelRepresentation::model_Reloaded()
      {
        initFromModel();
      ...
      void
      ModelRepresentation::model_Resetting()
      {
        mModel.setUserAny(Ogre::Any());
      }

      void
      ModelRepresentation::processOutfit(const Atlas::Message::MapType&)
      {
      }

      void
      ModelRepresentation::entity_Changed(const Eris::StringSet& attributeIds)
      {
        for (Eris::StringSet::const_iterator I = attributeIds.begin();
            I != attributeIds.end(); ++I)
          {
      ...
      void
      ModelRepresentation::attrChanged(const std::string& str,
          const Atlas::Message::Element& v)
      {
        ...
        if (mModel.hasParticles())
          {
            const ParticleSystemBindingsPtrSet& bindings =
                mModel.getAllParticleSystemBindings();
            for (ParticleSystemBindingsPtrSet::const_iterator I =
                bindings.begin(); I != bindings.end(); ++I)
              {
      ...
      void
      ModelRepresentation::onMovementModeChanged(MovementMode newMode)
      {
        /*	if (newMode != mMovementMode)
         {*/
        const char * actionName;
        if (newMode == ModelRepresentation::MM_WALKING
            || newMode == ModelRepresentation::MM_WALKING_BACKWARDS)
          {
        ...
        if (!mCurrentMovementAction
            || mCurrentMovementAction->getName() != actionName)
          {

            //first disable any current action
            resetAnimations();

            Action* newAction = mModel.getAction(ActivationDefinition::MOVEMENT,
                actionName);
            mCurrentMovementAction = newAction;
            if (newAction)
              {
                MotionManager::getSingleton().addAnimated(mEntity.getId(),
                    this);
              }
            else
              {
      ...
      ModelRepresentation::MovementMode
      ModelRepresentation::getMovementMode() const
      {
        return mMovementMode;
      }

      void
      ModelRepresentation::parseMovementMode(const WFMath::Vector<3>& velocity)
      {
            ...
            if (velocity.isValid())
              {
                ...
                if (velocity.x() + WFMath::numeric_constants < WFMath::CoordType
                    > ::epsilon() < 0.0f)
                  {
      ...
      void
      ModelRepresentation::setLocalVelocity(const WFMath::Vector<3>& velocity)
      {
        parseMovementMode(velocity);
      }

      void
      ModelRepresentation::updateAnimation(float timeSlice)
      {
      ...
      void
      ModelRepresentation::resetAnimations()
      {
        if (mCurrentMovementAction)
          {
      ...
      ModelRepresentation::getWorldBoundingBox(bool derive) const
      {
        return getModel().getWorldBoundingBox(derive);
      }

      const Ogre::Sphere &
      ModelRepresentation::getWorldBoundingSphere(bool derive) const
      {
        return getModel().getWorldBoundingSphere(derive);
      }

      void
      ModelRepresentation::entity_Acted(
        ...
        if (I != p.end())
          {
            ...
            Action* newAction = mModel.getAction(ActivationDefinition::ACTION,
                name);

            //If there's no action found, try to see if we have a "default action" defined to play instead.
            if (!newAction)
              {
                newAction = mModel.getAction(ActivationDefinition::ACTION,
                    "default_action");
              }
      ...
      void
      ModelRepresentation::entity_TaskAdded(Eris::Task* task)
      {
        createActionForTask(*task);
      }

      void
      ModelRepresentation::createActionForTask(const Eris::Task& task)
      {
        Action* newAction = mModel.getAction(ActivationDefinition::TASK,
            task.name());
        if (newAction)
          {
      ...
      void
      ModelRepresentation::entity_TaskRemoved(Eris::Task*)
      {
        if (mTaskAction)
          {
            mTaskAction->getAnimations().reset();
      ...
      void
      ModelRepresentation::setVisualize(const std::string& visualization,
          bool visualize)
      {
      ...
      bool
      ModelRepresentation::getVisualize(const std::string& visualization) const
      {
        if (visualization == "CollisionObject")
          {
            if (!getModel().getUserAny().isEmpty())
      ...
      void
      ModelRepresentation::reactivatePartActions()
      {
