...
namespace Ember
{
  namespace OgreView
  {
    namespace Model
    {
      ...
      void
      AnimationSet::addTime(Ogre::Real timeSlice)
      {
      ...
      void
      AnimationSet::addTime(Ogre::Real timeSlice, bool& continueAnimation)
      {
        if (mAnimations.size() > 0)
          {
            ...
            if (animationLength == 0)
            ...
            else
              {
                ...
                if (fabs(mAccumulatedTime)
                    >= animation->getLengthOfOneIteration()
                        * animation->getIterations())
                  {
      ...
      AnimationSet::reset()
      {
        for (AnimationStore::iterator I = mAnimations.begin();
            I != mAnimations.end(); ++I)
          {
            I->setEnabled(false);
            I->setTime(0.0f);
      ...
      void
      AnimationSet::addAnimation(const Animation& animation)
      {
      ...
      void
      Animation::addAnimationPart(const AnimationPart& part)
      {
        mAnimationParts.push_back(part);
        mIterationLength = std::max<Ogre::Real>(part.state->getLength(),
            mIterationLength);
      }

      void
      Animation::addTime(Ogre::Real timeSlice)
      {
      ...
      void
      Animation::setTime(Ogre::Real time)
      {

        for (AnimationPartSet::iterator I = mAnimationParts.begin();
            I != mAnimationParts.end(); ++I)
          {
            //we'll get an assert error if we try to add time to an animation with zero length
            if (I->state->getLength() != 0)
              {
      ...
      void
      Animation::setEnabled(bool enabled)
      {
        for (AnimationPartSet::const_iterator I = mAnimationParts.begin();
            I != mAnimationParts.end(); ++I)
          {
                ...
                if (state->getEnabled() != enabled)
                  {
                    ...
                    if (enabled)
                      {
                        const std::vector<BoneGroupRef>& boneGroupRefs =
                            I->boneGroupRefs;
                        for (std::vector<BoneGroupRef>::const_iterator J =
                            boneGroupRefs.begin(); J != boneGroupRefs.end();
                            ++J)
                          {
                            const BoneGroupRef& boneGroupRef = *J;
                            const BoneGroupDefinition& boneGroupDef =
                                *boneGroupRef.boneGroupDefinition;
                            if (!state->hasBlendMask())
                              {
                                state->createBlendMask(mBoneNumber, 0.0f);
                              }
                            const std::vector<size_t>& boneIndices =
                                boneGroupDef.Bones;
                            for (std::vector<size_t>::const_iterator bones_I =
                                boneIndices.begin();
                                bones_I != boneIndices.end(); ++bones_I)
                              {
                                state->setBlendMaskEntry(*bones_I,
                                    boneGroupRef.weight);
                              }
                          }
                      }
