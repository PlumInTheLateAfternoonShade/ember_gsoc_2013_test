...
namespace Ember
{
  namespace OgreView
  {
    namespace Model
    {
      ...
      ParticleSystemBinding::ParticleSystemBinding(ParticleSystem* parentSystem,
          const std::string& emitterVal, const std::string& variableName) :
          mEmitterVal(emitterVal), mVariableName(variableName), mParticleSystem(
              parentSystem), mOriginalValue(0)
      {
        //TODO: add more emitter values to bind
        if (mEmitterVal == "emission_rate")
          {
            Ogre::ParticleEmitter* emitter =
                mParticleSystem->getOgreParticleSystem()->getEmitter(0);
            if (emitter)
              {
      ...
      void
      ParticleSystemBinding::scaleValue(Ogre::Real scaler)
      {
