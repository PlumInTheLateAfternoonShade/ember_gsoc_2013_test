...
namespace Ember
{
  namespace OgreView
  {
    namespace Model
    {
      ...
      SubModel::SubModelPartMap&
      SubModel::getSubModelPartMap()
      {
        return mSubModelParts;
      }

      SubModelPart&
      SubModel::createSubModelPart(const std::string& name)
      {
        SubModelPart part(name);
        std::pair<SubModelPartMap::iterator, bool> result(
