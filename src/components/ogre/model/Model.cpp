  ...
  namespace OgreView
  {
    namespace Model
    {
      ...
      Model::Model(const std::string& name) :
          Ogre::MovableObject(name), mSkeletonOwnerEntity(0), mSkeletonInstance(
              0), mScale(0.0f), mRotation(Ogre::Quaternion::IDENTITY), mAnimationStateSet(
              0), mAttachPoints(nullptr), mBackgroundLoader(0)
      {
        mVisible = true;
      }
      Model::~Model()
      {
        ...
        if (mBackgroundLoader)
          {
            ModelDefinitionManager::getSingleton().removeBackgroundLoader(
                mBackgroundLoader);
          }
      ...
      Model::create(const std::string& modelType)
      {
        if (!mDefinition.isNull() && mDefinition->isValid())
          {
            S_LOG_WARNING(
                "Trying to call create('" + modelType
                    + "') on a Model instance that already have been created as a '"
                    + mDefinition->getName() + "'.");
            return false;
          }
        ...
        try
          {
            mDefinition = ModelDefinitionManager::getSingleton().load(modelType,
                groupName);
          }
        catch (const std::exception& ex)
          {
            S_LOG_FAILURE(
                "Could not load model of type " << modelType << " from group " << groupName << "." << ex);
            return false;
          }
        if (false && !mDefinition->isValid())
          {
            S_LOG_FAILURE(
                "Model of type " << modelType << " from group " << groupName << " is not valid.");
            return false;
          }
        else
          {
      ...
      void
      Model::_notifyManager(Ogre::SceneManager* man)
      {
      ...
      Model::createFromDefn()
      {
        ...
        if (mBackgroundLoader->poll(TimeFrame(boost::posix_time::seconds(0))))
          ...
          {
            ModelDefinitionManager::getSingleton().addBackgroundLoader(
                mBackgroundLoader);
            ...
            try
              {
                ...
                Ogre::Entity* entity = sceneManager->createEntity(entityName,
                    submodelDef->getMeshName());
                timedLog.report("Created entity.");
                if (entity->getMesh().isNull())
                  {
                    S_LOG_FAILURE(
                        "Could not load mesh " << submodelDef->getMeshName() << " which belongs to model " << mDefinition->getName() << ".");
                  }

                if (mDefinition->getRenderingDistance())
                  {
                    entity->setRenderingDistance(
                        mDefinition->getRenderingDistance());
                  }
                ...
                for (auto& partDef : submodelDef->getPartDefinitions())
                  {
                    SubModelPart& part = submodel->createSubModelPart(
                        partDef->getName());
                        ...
                        for (auto& subEntityDef : partDef->getSubEntityDefinitions())
                          {
                            try
                                ...
                                if (subEntityDef->getSubEntityName() != "")
                                ...
                                else
                                  {
                                    ...
                                    if (entity->getNumSubEntities()
                                      ...
                                      {
                                        S_LOG_WARNING(
                                            "Model definition " << mDefinition->getName() << " has a reference to entity with index " << subEntityDef->getSubEntityIndex() << " which is out of bounds.");
                                      }
                                  }
                                if (subEntity)
                                  {
                              ...
                              {
                                S_LOG_WARNING(
                                    "Error when getting sub entities for model '" << mDefinition->getName() << "'." << ex);
                              }
                          }
                      }
                    else
                    ...
                    if (partDef->getGroup() != "")
                      {
                        mGroupsToPartMap[partDef->getGroup()].push_back(
                            partDef->getName());
              ...
              {
                S_LOG_FAILURE(
                    "Submodel load error for " << entityName << "." << e);
                return false;
              }
          }
                ...
                if (mSubmodels.size())
                  {
                    for (auto& animationDef : actionDef->getAnimationDefinitions())
                      {
                        Animation animation(animationDef->getIterations(),
                            getSkeleton()->getNumBones());
                        for (auto& animationPartDef : animationDef->getAnimationPartDefinitions())
                          {
                            if (getAllAnimationStates()->hasAnimationState(
                                animationPartDef->Name))
                              {
                                ...
                                try
                                  {
                                    ...
                                    for (auto& boneGroupDef : animationPartDef->BoneGroupRefs)
                                      {
                                        auto I_boneGroup =
                                            mDefinition->getBoneGroupDefinitions().find(
                                                boneGroupDef.Name);
                                        if (I_boneGroup
                                            != mDefinition->getBoneGroupDefinitions().end())
                                          {
                                            BoneGroupRef boneGroupRef;
                                            boneGroupRef.boneGroupDefinition =
                                                I_boneGroup->second;
                                            boneGroupRef.weight =
                                                boneGroupDef.Weight;
                                            animPart.boneGroupRefs.push_back(
                                                boneGroupRef);
                                          }
                                      }
                                  ...
                                  {
                                    S_LOG_FAILURE(
                                        "Error when loading animation: " << animationPartDef->Name << "." << ex);
                                  }
                              }
                          }
        ...
        for (auto& particleSystemDef : mDefinition->mParticleSystems)
          {
            ...
            try
              {
                ogreParticleSystem = _getManager()->createParticleSystem(name,
                    particleSystemDef.Script);
              }
            catch (const std::exception& ex)
              {
                S_LOG_FAILURE(
                    "Could not create particle system: " << name << "." << ex);
                continue;
              }
            if (ogreParticleSystem)
              {
                ...
                ParticleSystem* particleSystem = new ParticleSystem(
                    ogreParticleSystem, particleSystemDef.Direction);
                for (auto& bindingDef : particleSystemDef.Bindings)
                  {
                    ParticleSystemBinding* binding = particleSystem->addBinding(
                        bindingDef.EmitterVar, bindingDef.AtlasAttribute);
        ...
        for (auto& lightDef : mDefinition->mLights)
          {
            ...
            try
              ...
              {
                S_LOG_FAILURE(
                    "Could not create light: " << name.str() << "." << ex);
                continue;
              }
            if (ogreLight)
              {
                ...
                ogreLight->setAttenuation(lightDef.range, lightDef.constant,
                    lightDef.linear, lightDef.quadratic);
      ...
      bool
      Model::hasParticles() const
      {
        return mParticleSystems.size() > 0;
      }

      const ParticleSystemBindingsPtrSet&
      Model::getAllParticleSystemBindings() const
      {
        return mAllParticleSystemBindings;
      }

      ParticleSystemSet&
      Model::getParticleSystems()
      {
      ...
      bool
      Model::addSubmodel(SubModel* submodel)
      {
        ...
        if (submodel->getEntity()->getSkeleton())
          {
            if (mSkeletonOwnerEntity != 0)
              {
                submodel->getEntity()->shareSkeletonInstanceWith(
                    mSkeletonOwnerEntity);
              }
            else
              {
      ...
      bool
      Model::removeSubmodel(SubModel* submodel)
      {
      ...
      Model::getSubModel(size_t index)
      {
        ...
        S_LOG_FAILURE(
            "Could not find submodel with index "<< index << " in model " << getName());
        return 0;

      }

      void
      Model::showPart(const std::string& partName, bool hideOtherParts)
      {
            ...
            if (hideOtherParts)
              {
                ...
                PartGroupStore::iterator partBucketI = mGroupsToPartMap.find(
                    groupName);
      ...
      void
      Model::hidePart(const std::string& partName, bool dontChangeVisibility)
      {
                    ...
                    for (auto& part : partBucketI->second)
                      {
                        if (part != partName)
                          {
                            ModelPartStore::iterator I_modelPart =
                                mModelParts.find(partName);
      ...
      void
      Model::setDisplaySkeleton(bool display)
      {
        for (auto& submodel : mSubmodels)
          {
      ...
      bool
      Model::getDisplaySkeleton(void) const
      {
      ...
      Model::isLoaded() const
      {
        ...
        return mBackgroundLoader == 0
            || mBackgroundLoader->getState() == ModelBackgroundLoader::LS_DONE;
      }
      ...
      ModelDefinition::UseScaleOf
      Model::getUseScaleOf() const
      {
        return mDefinition->getUseScaleOf();
      }

      Action*
      Model::getAction(const std::string& name)
      {
      ...
      Action*
      Model::getAction(const ActivationDefinition::Type type,
        ...
        for (auto& action : mDefinition->mActions)
          {
            for (auto& activationDefinition : action->getActivationDefinitions())
              {
                if (type == activationDefinition->type
                    && trigger == activationDefinition->trigger)
                  {
      ...
      Model::AttachPointWrapper
      Model::attachObjectToAttachPoint(const Ogre::String &attachPointName,
          Ogre::MovableObject *pMovable, const Ogre::Vector3 &scale,
            ...
            if (attachPointDef.Name == attachPointName)
              {
                ...
                Ogre::TagPoint* tagPoint = attachObjectToBone(boneName,
                    pMovable, offsetOrientation * attachPointDef.Rotation,
                    offsetPosition, scale);
                if (!mAttachPoints.get())
                  {
                    mAttachPoints = std::unique_ptr < AttachPointWrapperStore
                        > (new AttachPointWrapperStore());
                  }
      ...
      bool
      Model::hasAttachPoint(const std::string& attachPoint) const
      {
        return std::find_if(mDefinition->mAttachPoints.begin(),
            mDefinition->mAttachPoints.end(),
            [&attachPoint](const AttachPointDefinition& def)->bool
              { return def.Name == attachPoint;})
            != mDefinition->mAttachPoints.end();
      }

      Ogre::AnimationState*
      Model::getAnimationState(const Ogre::String& name)
      {
      ...
      Ogre::TagPoint*
      Model::attachObjectToBone(const Ogre::String &boneName,
          Ogre::MovableObject *pMovable,
          const Ogre::Quaternion &offsetOrientation,
          const Ogre::Vector3 &offsetPosition)
      {
        return attachObjectToBone(boneName, pMovable, offsetOrientation,
            offsetPosition, Ogre::Vector3::UNIT_SCALE);
      }
      ...
      Model::attachObjectToBone(const Ogre::String &boneName,
          Ogre::MovableObject *pMovable,
          const Ogre::Quaternion &offsetOrientation,
          const Ogre::Vector3 &offsetPosition, const Ogre::Vector3 &scale)
      {
        if (mSubmodels.size())
          {
            Ogre::Entity* entity = mSkeletonOwnerEntity;

            Ogre::TagPoint* tagPoint = entity->attachObjectToBone(boneName,
                pMovable, offsetOrientation, offsetPosition);
        ...
        else
          {
            OGRE_EXCEPT(Ogre::Exception::ERR_ITEM_NOT_FOUND,
                "There are no entities loaded!", "Model::attachObjectToBone");
          }
      }

      Ogre::MovableObject*
      Model::detachObjectFromBone(const Ogre::String &movableName)
      {
        ...
        if (mSubmodels.size() && mSkeletonOwnerEntity)
          {
            if (mAttachPoints.get())
              {
                for (auto I = mAttachPoints->begin(); I != mAttachPoints->end();
                    ++I)
                  {
                    if (I->Movable->getName() == movableName)
                      {
                        ...
                        if (I->TagPoint == I->Movable->getParentNode())
                          {
                            result = mSkeletonOwnerEntity->detachObjectFromBone(
                                movableName);
                          }
          ...
          {
            OGRE_EXCEPT(Ogre::Exception::ERR_ITEM_NOT_FOUND,
                "There are no entities loaded!", "Model::detachObjectFromBone");
          }
      }

//-----------------------------------------------------------------------
      void
      Model::detachAllObjectsFromBone(void)
      {
        if (mSubmodels.size() && mSkeletonOwnerEntity)
          {
          ...
          {
            OGRE_EXCEPT(Ogre::Exception::ERR_ITEM_NOT_FOUND,
                "There are no entities loaded!",
                "Model::detachAllObjectsFromBone");
          }
      }

      void
      Model::iterateOverMovableObjects(std::function<void
      (Ogre::MovableObject&)> movableObjectFunction)
      {
        for (auto& submodel : mSubmodels)
          {
       ...
       */
      void
      Model::_notifyCurrentCamera(Ogre::Camera* cam)
      {
            ...
            if (mVisible)
              {
                ...
                iterateOverMovableObjects([&cam](Ogre::MovableObject& obj)
                  { obj._notifyCurrentCamera(cam);});
      ...
      Model::_notifyMoved()
      {
        ...
        iterateOverMovableObjects([](Ogre::MovableObject& obj)
          { obj._notifyMoved();});
      }

      void
      Model::setUserAny(const Ogre::Any &anything)
      {
        Ogre::MovableObject::setUserAny(anything);
        iterateOverMovableObjects([&anything](Ogre::MovableObject& obj)
          { obj.setUserAny(anything);});
      }

// Overridden - see MovableObject.
      void
      Model::setRenderQueueGroup(Ogre::RenderQueueGroupID queueID)
      {
        iterateOverMovableObjects([&queueID](Ogre::MovableObject& obj)
      ...
      Model::getBoundingBox(void) const
      {
        ...
        for (auto& particleSystem : mParticleSystems)
          {
            mFull_aa_box.merge(
                particleSystem->getOgreParticleSystem()->getBoundingBox());
          }
       ...
       */
      const Ogre::AxisAlignedBox&
      Model::getWorldBoundingBox(bool derive) const
      {
        mWorldFull_aa_box.setNull();

        for (auto& submodel : mSubmodels)
          {
            mWorldFull_aa_box.merge(
                submodel->getEntity()->getWorldBoundingBox(derive));
          }
        for (auto& particleSystem : mParticleSystems)
          {
            mWorldFull_aa_box.merge(
                particleSystem->getOgreParticleSystem()->getWorldBoundingBox(
                    derive));
          }
      ...
      Ogre::Real
      Model::getBoundingRadius() const
      {
        Ogre::Real rad(0);
        for (auto& submodel : mSubmodels)
          {
            rad = std::max<Ogre::Real>(rad,
                submodel->getEntity()->getBoundingRadius());
          }
        for (auto& particleSystem : mParticleSystems)
          {
            rad = std::max<Ogre::Real>(rad,
                particleSystem->getOgreParticleSystem()->getBoundingRadius());
          }
        return rad;
       ...
       */
      void
      Model::_updateRenderQueue(Ogre::RenderQueue* queue)
      {
            ...
            for (auto& particleSystemInfo : mParticleSystems)
              {
                Ogre::ParticleSystem* particleSystem =
                    particleSystemInfo->getOgreParticleSystem();
                ...
                if (!particleSystemInfo->getDirection().isNaN())
                  {
                    Ogre::Vector3 direction =
                        particleSystem->getParentNode()->convertWorldToLocalPosition(
                            particleSystemInfo->getDirection());
                    direction.normalise();
                    //const Ogre::Quaternion& rotation = particleSystem->getParentNode()->_getDerivedOrientation();
                    Ogre::Quaternion rotation =
                        particleSystem->getParentNode()->convertWorldToLocalOrientation(
                            Ogre::Quaternion(Ogre::Degree(0),
                                particleSystemInfo->getDirection()));
                    for (int i = 0; i < particleSystem->getNumEmitters(); ++i)
                      {
                        particleSystem->getEmitter(i)->setDirection(
                            rotation * particleSystemInfo->getDirection());
                      }
                  }
...
// }
      /** Overridden from MovableObject */
      const Ogre::String&
      Model::getMovableType(void) const
      {
        return sMovableType;
      }

      void
      Model::setRenderingDistance(Ogre::Real dist)
      {
        MovableObject::setRenderingDistance(dist);
        iterateOverMovableObjects([dist](Ogre::MovableObject& obj)
          { obj.setRenderingDistance(dist);});
      }

      void
      Model::setQueryFlags(unsigned long flags)
      {
        MovableObject::setQueryFlags(flags);
        iterateOverMovableObjects([flags](Ogre::MovableObject& obj)
          { obj.setQueryFlags(flags);});
      }

      void
      Model::addQueryFlags(unsigned long flags)
      {
      ...
      void
      Model::removeQueryFlags(unsigned long flags)
      {
      ...
      void
      Model::visitRenderables(Ogre::Renderable::Visitor* visitor,
          bool debugRenderables)
      {
        if (isVisible())
          {
            iterateOverMovableObjects([&](Ogre::MovableObject& obj)
              { obj.visitRenderables(visitor, debugRenderables);});
          }
      }

      /** Overridden from MovableObject */
      void
      Model::_notifyAttached(Ogre::Node* parent, bool isTagPoint)
      {
        ...
        for (auto I = mParticleSystems.begin(); I != mParticleSystems.end();
            ++I)
          {
            ...
            try
              {
                ...
                std::pair<Ogre::ResourcePtr, bool> result =
                    Ogre::MaterialManager::getSingleton().createOrRetrieve(
                        (*I)->getOgreParticleSystem()->getMaterialName(),
                        Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
                Ogre::MaterialPtr materialPtr =
                    static_cast<Ogre::MaterialPtr>(result.first);
                if (!materialPtr.isNull())
                  {
                    if (materialPtr->isTransparent())
                      {
                        (*I)->getOgreParticleSystem()->setRenderQueueGroup(
                            Ogre::RENDER_QUEUE_9);
                      }
                  }
            ...
            catch (const Ogre::Exception& ex)
              {
                ...
                S_LOG_FAILURE(
                    "Error when loading particle system " << (*I)->getOgreParticleSystem()->getName() << ". Removing it.");
                delete *I;
      ...
      {
        //check with both the model visibility setting and with the general model setting to see whether the model should be shown
        return Ogre::MovableObject::isVisible()
            && ModelDefinitionManager::getSingleton().getShowModels();
      }

      Model*
      Model::createModel(Ogre::SceneManager& sceneManager,
          const std::string& modelType, const std::string& name)
      {
        ...
        return static_cast<Model*>(sceneManager.createMovableObject(modelName,
            ModelFactory::FACTORY_TYPE_NAME, &params));

      }

      const Model::AttachPointWrapperStore*
      Model::getAttachedPoints() const
      {
...
//-----------------------------------------------------------------------
      const Ogre::String&
      ModelFactory::getType(void) const
      {
        return FACTORY_TYPE_NAME;
      }
//-----------------------------------------------------------------------
      Ogre::MovableObject*
      ModelFactory::createInstanceImpl(const Ogre::String& name,
...
//-----------------------------------------------------------------------
      void
      ModelFactory::destroyInstance(Ogre::MovableObject* obj)
      {
        delete obj;
      }
