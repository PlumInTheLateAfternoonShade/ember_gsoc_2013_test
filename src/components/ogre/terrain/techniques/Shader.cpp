    ...
    namespace Terrain
    {
      ...
      namespace Techniques
      {
        ...
        Shader::Shader(bool includeShadows,
            ...
            Base(mGeometry, mTerrainPageSurfaces, terrainPageShadow), mIncludeShadows(
                includeShadows), mSceneManager(sceneManager)
        {
        }
          ...
          if (shaderPass)
            {
              ...
              for (SurfaceLayerStore::const_iterator I =
                  mTerrainPageSurfaces.begin(); I != mTerrainPageSurfaces.end();
                  ++I)
                {
        ...
        bool
        Shader::compileMaterial(Ogre::MaterialPtr material)
        {
          ...
          if (material->getNumSupportedTechniques() == 0)
            {
              S_LOG_WARNING(
                  "The material '" << material->getName() << "' has no supported techniques. The reason for this is: \n" << material->getUnsupportedTechniquesExplanation());
              return false;
            }
          return true;
        ...
        Shader::addPass()
        {
          ShaderPass* shaderPass(
              new ShaderPass(mSceneManager, mPage.getAlphaTextureSize(),
                  mPage.getWFPosition()));
