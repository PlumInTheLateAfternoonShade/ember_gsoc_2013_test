...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Terrain
    {
      ...
      float
      HeightMapSegment::getHeight(int x, int y) const
      {
        return mBuffer->getBuffer()->getData()[y * (mBuffer->getResolution())
            + x];
      }
...
/// the second triangle has vertex coordinates (0,0) (0,1) (1,1).
      void
      HeightMapSegment::getHeightAndNormal(float x, float y, float& h,
