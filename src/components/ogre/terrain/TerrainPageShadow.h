...
namespace Ember
{
  namespace OgreView
  {
    namespace Terrain
    {
      ...
      public:
        virtual void
        createShadowData(const TerrainPage& page,
            const TerrainPageGeometry& geometry, unsigned char* data,
            const WFMath::Vector<3>& lightDirection,
            const Ogre::ColourValue& lightColour) const = 0;
      ...
      public:
        ...
        void
        setShadowTechnique(const ITerrainPageShadowTechnique* shadowTechnique);

        void
        setLightDirection(const WFMath::Vector<3>& lightDirection);

        void
        updateShadow(const TerrainPageGeometry& geometry);

        void
        getShadowColourAt(const Ogre::Vector2& position,
        ...
        void
        loadIntoImage(Ogre::Image& ogreImage) const;
