  ...
  namespace OgreView
  {
    ...
    namespace Terrain
    {
      ...
      TerrainAreaAddTask::TerrainAreaAddTask(Mercator::Terrain& terrain,
          Mercator::Area* area, ShaderUpdateSlotType markForUpdateSlot,
          TerrainHandler& terrainHandler,
          TerrainLayerDefinitionManager& terrainLayerDefinitionManager,
          AreaShaderstore& areaShaders, AreaMap& areas,
          const std::string& entityId) :
          TerrainAreaTaskBase(terrain, area, markForUpdateSlot), mTerrainHandler(
              terrainHandler), mTerrainLayerDefinitionManager(
              terrainLayerDefinitionManager), mAreaShaders(areaShaders), mAreas(
              areas), mEntityId(entityId)
      {
      }
      ...
      void
      TerrainAreaAddTask::executeTaskInBackgroundThread(
      ...
      void
      TerrainAreaAddTask::executeTaskInMainThread()
      {
            ...
            if (!mAreaShaders.count(mArea->getLayer()))
              {
                ...
                const TerrainLayerDefinition* layerDef =
                    mTerrainLayerDefinitionManager.getDefinitionForArea(
                        mArea->getLayer());
                if (layerDef)
                  {
                    TerrainShader* shader = mTerrainHandler.createShader(
                        layerDef, new Mercator::AreaShader(mArea->getLayer()));
            ...
            if (mAreaShaders.count(mArea->getLayer()))
              {
                ...
                mShaderUpdateSlot(mAreaShaders[mArea->getLayer()],
                    mArea->bbox());
              }
          }
      }
