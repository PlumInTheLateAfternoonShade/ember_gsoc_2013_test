...
namespace Ember
{
  namespace OgreView
  {
    namespace Authoring
    {
      EntityRecipe::EntityRecipe(Ogre::ResourceManager* creator,
          const Ogre::String& name, Ogre::ResourceHandle handle,
          const Ogre::String& group, bool isManual,
          Ogre::ManualResourceLoader* loader) :
          Resource(creator, name, handle, group, isManual, loader), mEntitySpec(
              0)
      {
      ...
      EntityRecipe::EntityRecipe(const std::string& name,
      ...
      {
        for (GUIAdaptersStore::iterator I = mGUIAdapters.begin();
            I != mGUIAdapters.end(); ++I)
          {
            delete I->second;
          }

        for (BindingsStore::iterator I = mBindings.begin();
            I != mBindings.end(); ++I)
          {
            delete I->second;
          }
      ...
      void
      EntityRecipe::loadImpl(void)
      {
      }

      void
      EntityRecipe::unloadImpl(void)
      {
      }

      size_t
      EntityRecipe::calculateSize(void) const
      {
        //TODO:implement this
        return 0;
      }
      ...
      const std::string&
      EntityRecipe::getEntityType() const
      {
        return mEntityType;
      }

      GUIAdapter*
      EntityRecipe::createGUIAdapter(const std::string& name,
          const std::string& type, const std::string& tooltip)
      {
        GUIAdapter* adapter;
        adapter = new GUIAdapter(type);
        adapter->setTooltip(tooltip);
        mGUIAdapters[name] = adapter;
        adapter->EventValueChanged.connect(
            sigc::mem_fun(*this, &EntityRecipe::valueChanged));
        return adapter;
      }

      GUIAdapter*
      EntityRecipe::getGUIAdapter(const std::string& name)
      {
      ...
      const GUIAdaptersStore&
      EntityRecipe::getGUIAdapters()
      {
        return mGUIAdapters;
      }

      GUIAdapterBindings*
      EntityRecipe::createGUIAdapterBindings(const std::string& name)
      {
      ...
      void
      EntityRecipe::associateBindings()
      {
      ...
      bool
      EntityRecipe::SpecIterator::Visit(const TiXmlText& textNode)
      {
        ...
        if (!text.empty() && text.at(0) == '$')
          {
            BindingsStore::iterator bindings = mRecipe->mBindings.find(
                text.substr(1));
            if (bindings != mRecipe->mBindings.end())
              {
                bindings->second->associateXmlElement(
                    const_cast<TiXmlNode&>(*textNode.Parent()));
                S_LOG_VERBOSE(
                    "Associated " << bindings->first << " with " << text);
              }
            else
              {
      ...
      EntityRecipe::createEntity(Eris::TypeService& typeService)
      {
        S_LOG_VERBOSE("Creating entity.");

        ScriptingService& scriptingService =
            EmberServices::getSingleton().getScriptingService();
        // Loading script code
                ...
                if (adapters.size() == 1)
                  {
                    ...
                    Atlas::Message::Element val =
                        mGUIAdapters[adapterName]->getValue();
                    I->second->setValue(val);
                  }
                else
                  {
                    S_LOG_WARNING(
                        "Should be only one adapter without calling function.");
                  }
              }
            else
              {
                ...
                lua_State* L =
                    static_cast<Lua::LuaScriptingProvider*>(scriptingService.getProviderFor(
                        "LuaScriptingProvider"))->getLuaState();

                // Pushing function params
                std::vector<std::string>& adapters = I->second->getAdapters();
                for (std::vector<std::string>::iterator J = adapters.begin();
                    J != adapters.end(); J++)
                  {
                    std::string adapterName = *J;
                    Atlas::Message::Element* val = new Atlas::Message::Element(
                        mGUIAdapters[adapterName]->getValue());
                    tolua_pushusertype_and_takeownership(L, val,
                        "Atlas::Message::Element");
                  }

                // Calling test function
                scriptingService.callFunction(func, adapters.size(),
                    "LuaScriptingProvider", &callContext);

                LuaRef returnValue(callContext.getReturnValue());

                Atlas::Message::Element returnObj;
                returnObj = returnValue.asObject<Atlas::Message::Element>(
                    "Atlas::Message::Element");
            ...
            while (decoder.queueSize() > 0)
              {
                ...
                if (erisType)
                  {
                    const Atlas::Message::MapType& defaultAttributes =
                        erisType->getAttributes();
                    for (Atlas::Message::MapType::const_iterator I =
                        defaultAttributes.begin(); I != defaultAttributes.end();
                        ++I)
                      {
                        if (m.find(I->first) == m.end())
                          {
                            m.insert(
                                Atlas::Message::MapType::value_type(I->first,
                                    I->second));
                          }
                      }
                  }
      ...
      void
      EntityRecipe::setAuthor(const std::string& author)
      {
        mAuthor = author;
      }

      const std::string&
      EntityRecipe::getAuthor() const
      {
        return mAuthor;
      }

      void
      EntityRecipe::setDescription(const std::string& description)
      {
        mDescription = description;
      }

      const std::string&
      EntityRecipe::getDescription() const
      {
        return mDescription;
      }

      void
      EntityRecipe::valueChanged()
      {
