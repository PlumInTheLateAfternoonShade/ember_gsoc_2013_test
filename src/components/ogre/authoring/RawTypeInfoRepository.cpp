...
namespace Ember
{
  namespace OgreView
  {
    namespace Authoring
    {
      ...
      Atlas::Objects::Root
      RawTypeInfoRepository::getRawTypeData(const std::string& typeName) const
      {
      ...
      void
      RawTypeInfoRepository::setupConnections(ServerService& serverService)
      {
        serverService.EventReceivedObject.connect(
            sigc::mem_fun(*this,
                &RawTypeInfoRepository::server_ReceivedObject));
      }

      void
      RawTypeInfoRepository::server_ReceivedObject(
          const Atlas::Objects::Root& op)
      {
        if (op->instanceOf(Atlas::Objects::Operation::INFO_NO))
          {
            const Atlas::Objects::Operation::RootOperation& rootOp =
                Atlas::Objects::smart_static_cast<
                    Atlas::Objects::Operation::RootOperation>(op);
            const std::vector<Atlas::Objects::Root>& args(rootOp->getArgs());
            std::string objType = args.front()->getObjtype();

            if ((objType == "meta") || (objType == "class")
      ...
      void
      RawTypeInfoRepository::parseAndSaveTypeData(
          const Atlas::Objects::Root& rawType)
      {
