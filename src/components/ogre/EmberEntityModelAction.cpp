...
namespace Ember
{
  namespace OgreView
  {
    ...
    public:
      ...
      void
      visit(EntityMapping::Actions::Action& action)
      {
        EmberEntityModelAction* modelAction =
            dynamic_cast<EmberEntityModelAction*>(&action);
        if (modelAction)
          {
      ...
      void
      visit(EntityMapping::Matches::MatchBase& match)
      {

      }

      void
      visit(EntityMapping::Cases::CaseBase& caseBase)
      {
    ...
    EmberEntityModelAction::EmberEntityModelAction(EmberEntity& entity,
        const std::string& modelName, Scene& scene,
        EntityMapping::EntityMapping& modelMapping) :
        mEntity(entity), mModelName(modelName), mScene(scene), mMapping(
            modelMapping)
    {
    }
    ...
    void
    EmberEntityModelAction::activate(EntityMapping::ChangeContext& context)
    {
      Model::Model* model =
          Model::ModelRepresentationManager::getSingleton().getModelForEntity(
              mEntity);
      if (!model || model->getDefinition()->getName() != mModelName)
        {
          mEntity.setGraphicalRepresentation(0);
          model = Model::Model::createModel(mScene.getSceneManager(),
              mModelName, mEntity.getId());

          //if the model definition isn't valid, use a placeholder
          if (!model->getDefinition()->isValid())
            {
              S_LOG_FAILURE(
                  "Could not find " << mModelName << ", using placeholder.");
          ...
          Model::ModelRepresentation* representation =
              new Model::ModelRepresentation(mEntity, *model, mScene, mMapping);
    ...
    void
    EmberEntityModelAction::deactivate(EntityMapping::ChangeContext& context)
    {
      mEntity.setGraphicalRepresentation(0);
      ...
      context.EventContextComplete.connect(
          sigc::mem_fun(*this,
              &EmberEntityModelAction::ChangeContext_ContextComplete));
    }
