...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Environment
    {
      ...
      {
      public:
        FoliageLayer(Forests::PagedGeometry *geom,
            Forests::GrassLoader<FoliageLayer> *ldr);

        virtual
        ~FoliageLayer();

        virtual unsigned int
        prepareGrass(const Forests::PageInfo& page, float densityFactor,
            float volume, bool& isAvailable);

        Ogre::uint32
        getColorAt(float x, float z);

        void
        configure(Terrain::TerrainManager* terrainManager,
            const Terrain::TerrainLayerDefinition* terrainLayerDefinition,
            const Terrain::TerrainFoliageDefinition* foliageDefinition);

        virtual bool
        isColoursEnabled() const;

        virtual bool
        isNormalsEnabled() const;

        virtual bool
        isTangentsEnabled() const;

        virtual bool
        isCastShadowsEnabled() const;
      ...
      protected:
        ...
        virtual unsigned int
        _populateGrassList(Forests::PageInfo page, float *posBuff,
        ...
        void
        plantQueryExecuted(const Terrain::PlantAreaQueryResult& queryResult);
