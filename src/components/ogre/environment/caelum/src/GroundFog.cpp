...
namespace Caelum
{
  ...
  GroundFog::GroundFog(Ogre::SceneManager *sceneMgr,
      Ogre::SceneNode *caelumRootNode, const Ogre::String &domeMaterialName,
      ...
      mScene(sceneMgr)
  {
    ...
    mDomeMaterial.reset(
        InternalUtilities::checkLoadMaterialClone(domeMaterialName,
            domeMaterialName + uniqueSuffix));
    mDomeParams.setup(
        mDomeMaterial->getTechnique(0)->getPass(0)->getFragmentProgramParameters());
    ...
    mDomeEntity.reset(
        mScene->createEntity(domeEntityName, Ogre::SceneManager::PT_SPHERE));
    ...
    sceneMgr->getRenderQueue()->getQueueGroup(CAELUM_RENDER_QUEUE_GROUND_FOG)->setShadowsEnabled(
        false);
  ...
  const GroundFog::PassSet&
  GroundFog::getPasses() const
  {
    return mPasses;
  }

  void
  GroundFog::findFogPassesByName(const Ogre::String& passName)
  {
    Ogre::MaterialManager *matManager =
        Ogre::MaterialManager::getSingletonPtr();
    Ogre::MaterialManager::ResourceMapIterator matIt =
        matManager->getResourceIterator();
  ...
  void
  GroundFog::setDensity(Ogre::Real density)
  {
    if (Ogre::Math::Abs(mDensity - density) > 0.000001)
      {
        for (PassFogParamsVector::const_iterator it = mPassFogParams.begin(),
            end = mPassFogParams.end(); it != end; ++it)
          {
  ...
  Ogre::Real
  GroundFog::getDensity() const
  {
    return mDensity;
  }

  void
  GroundFog::setColour(const Ogre::ColourValue &colour)
  {
    bool different = Ogre::Math::Abs(mFogColour.r - colour.r) > 0.001
  ...
  GroundFog::getColour() const
  {
    return mFogColour;
  }

  void
  GroundFog::setVerticalDecay(Ogre::Real verticalDecay)
  {
  ...
  Ogre::Real
  GroundFog::getVerticalDecay() const
  {
    return mVerticalDecay;
  }

  void
  GroundFog::setGroundLevel(Ogre::Real groundLevel)
  {
  ...
  Ogre::Real
  GroundFog::getGroundLevel() const
  {
    return mGroundLevel;
  }

  void
  GroundFog::updateSkyFogging()
  {
  ...
  void
  GroundFog::forceUpdate()
  {
    updatePassFogParams();
    for (PassFogParamsVector::const_iterator it = mPassFogParams.begin(), end =
        mPassFogParams.end(); it != end; ++it)
      {
  ...
  void
  GroundFog::updatePassFogParams()
  {
    mPassFogParams.clear();
    for (PassSet::const_iterator it = mPasses.begin(), end = mPasses.end();
        it != end; ++it)
      {
        mPassFogParams.push_back(
            PassFogParams((*it)->getFragmentProgramParameters()));
      }
    std::sort(mPassFogParams.begin(), mPassFogParams.end(),
        PassFogParams::lessThanByParams);
    std::unique(mPassFogParams.begin(), mPassFogParams.end(),
        PassFogParams::equalByParams);
  }

  void
  GroundFog::FogParamsBase::setup(Ogre::GpuProgramParametersSharedPtr fpParams)
  {
  ...
  void
  GroundFog::DomeFogParams::setup(Ogre::GpuProgramParametersSharedPtr fpParams)
  {
  ...
  void
  GroundFog::notifyCameraChanged(Ogre::Camera *cam)
  {
    CameraBoundElement::notifyCameraChanged(cam);

    // Send camera height to shader.
    float cameraHeight = cam->getDerivedPosition().dotProduct(
        mDomeNode->_getDerivedOrientation() * Ogre::Vector3::UNIT_Y);
    mDomeParams.cameraHeight.set(mDomeParams.fpParams, cameraHeight);
  }

  void
  GroundFog::setFarRadius(Ogre::Real radius)
  {
