...
namespace Caelum
{
  ...
  public:
    ...
    checkLoadMaterialClone(const Ogre::String& originalName,
     ...
     */
    static Ogre::CompositorPtr
    checkCompositorSupported(const Ogre::String& name);
  ...
  public:
    ...
    enum DomeType
    {
      DT_SKY_DOME, DT_IMAGE_STARFIELD,
    };
     ...
     */
    static void
    generateSphericDome(const Ogre::String &name, int segments,
        DomeType domeType);
  ...
  private:
     ...
     */
    static void
    fillGradientsDomeBuffers(float *pVertex, unsigned short *pIndices,
        int segments);
     ...
     */
    static void
    fillStarfieldDomeBuffers(float *pVertex, unsigned short *pIndices,
        int segments);
  };
}
