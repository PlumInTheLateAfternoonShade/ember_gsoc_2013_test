...
namespace Caelum
{
  ...
  public:
    ...
    /// @return pointer to the new layer.
    FlatCloudLayer*
    createLayerAtHeight(Ogre::Real height);

    /// Add new layer. Takes ownership of the layer.
    void
    addLayer(FlatCloudLayer* layer);

    /// Get a pointer to a certain layer.
    inline FlatCloudLayer*
    getLayer(int index)
    {
     ...
     */
    void
    update(Ogre::Real timePassed, const Ogre::Vector3 &sunDirection,
    ...
    virtual void
    forceLayerQueryFlags(uint flags);

    /// Similar to @see CaelumSystem::forceSubcomponentVisibilityFlags.
    virtual void
    forceLayerVisibilityFlags(uint flags);
  };
}
