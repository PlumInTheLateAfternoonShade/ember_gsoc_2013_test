...
namespace Caelum
{
  ...
  public:
     ...
     */
    void
    setSunDirection(const Ogre::Vector3& dir);

    /// Explicit haze colour.
    void
    setHazeColour(const Ogre::ColourValue& hazeColour);

    /// Set the sky color gradients image.
    void
    setSkyGradientsImage(const Ogre::String& gradients);

    /// Set the atmosphere depthh gradient image.
    void
    setAtmosphereDepthImage(const Ogre::String& gradients);

    /** Enable or disable skydome haze. This makes the sky darker.
     *  By default haze is disabled.
     */
    void
    setHazeEnabled(bool value);

    /// If skydome haze is enabled.
    bool
    getHazeEnabled() const;
