...
namespace Forests
{
  ...
  unsigned int
  GrassLayer::prepareGrass(const PageInfo& page, float densityFactor,
      float volume, bool& isAvailable)
  {
    isAvailable = true;
  ...
  unsigned int
  GrassLayer::_populateGrassList(PageInfo page, float *posBuff,
  ...
  void
  GrassLayerBase::setMaterialName(const String &matName)
  {
  ...
  void
  GrassLayerBase::setMinimumSize(float width, float height)
  {
    minWidth = width;
    minHeight = height;
  }
  ...
  void
  GrassLayerBase::setMaximumSize(float width, float height)
  {
    maxWidth = width;
    if (maxHeight != height)
      {
  ...
  void
  GrassLayerBase::setRenderTechnique(GrassTechnique style, bool blendBase)
  {
  ...
  void
  GrassLayerBase::setFadeTechnique(FadeTechnique style)
  {
  ...
  void
  GrassLayerBase::setAnimationEnabled(bool enabled)
  {
    if (animate != enabled)
      {
        animate = enabled;
  ...
  void
  GrassLayerBase::setLightingEnabled(bool enabled)
  {
    lighting = enabled;
  }

  bool
  GrassLayerBase::isColoursEnabled() const
  {
    return true;
  }

  bool
  GrassLayerBase::isNormalsEnabled() const
  {
    return false;
  }

  bool
  GrassLayerBase::isTangentsEnabled() const
  {
    return false;
  }

  bool
  GrassLayerBase::isCastShadowsEnabled() const
  {
    return false;
  }

  void
  GrassLayer::setDensityMap(const String &mapFile, MapChannel channel)
  {
    if (densityMap)
      {
        densityMap->unload();
  ...
  void
  GrassLayer::setDensityMap(TexturePtr map, MapChannel channel)
  {
    if (densityMap)
      {
        densityMap->unload();
  ...
  void
  GrassLayer::setDensityMapFilter(MapFilter filter)
  {
  ...
  unsigned int
  GrassLayer::_populateGrassList_Uniform(PageInfo page, float *posBuff,
      unsigned int grassCount)
  {
        ...
        for (unsigned int i = 0; i < grassCount; ++i)
          {
            ...
            float x = parent->rTable->getRangeRandom(page.bounds.left,
                page.bounds.right);
            float z = parent->rTable->getRangeRandom(page.bounds.top,
                page.bounds.bottom);
            ...
            else if (x >= mapBounds.left && x <= mapBounds.right
                && z >= mapBounds.top && z <= mapBounds.bottom)
              {
                *posPtr++ = x;
                *posPtr++ = z;
              }
  ...
  unsigned int
  GrassLayer::_populateGrassList_UnfilteredDM(PageInfo page, float *posBuff,
      unsigned int grassCount)
  {
    float *posPtr = posBuff;
        ...
        for (unsigned int i = 0; i < grassCount; ++i)
          {
            ...
            if (parent->rTable->getUnitRandom()
                < densityMap->_getDensityAt_Unfiltered(x, z, mapBounds))
              {
                //Add to list
                *posPtr++ = x;
                *posPtr++ = z;
  ...
  unsigned int
  GrassLayer::_populateGrassList_BilinearDM(PageInfo page, float *posBuff,
      unsigned int grassCount)
  {
    float *posPtr = posBuff;
        ...
        for (unsigned int i = 0; i < grassCount; ++i)
          {
            ...
            //For example, if localDensity is .32, grasses will be added 32% of the time.
            if (parent->rTable->getUnitRandom()
                < densityMap->_getDensityAt_Bilinear(x, z, mapBounds))
              {
                //Add to list
                *posPtr++ = x;
                *posPtr++ = z;
  ...
  void
  GrassLayer::setColorMap(const String &mapFile, MapChannel channel)
  {
    if (colorMap)
      {
        colorMap->unload();
  ...
  void
  GrassLayer::setColorMap(TexturePtr map, MapChannel channel)
  {
    if (colorMap)
      {
        colorMap->unload();
  ...
  void
  GrassLayer::setColorMapFilter(MapFilter filter)
  {
    ...
    if (shaderNeedsUpdate)
      {
        ...
        if (caps->hasCapability(RSC_VERTEX_PROGRAM)
            && geom->getShadersEnabled())
          {
            ...
            if (fadeTechnique == FADETECH_GROW
                || fadeTechnique == FADETECH_ALPHAGROW)
              tmpName << maxHeight << "_";
            ...
            MaterialPtr tmpMat = MaterialManager::getSingleton().getByName(
                matName);
            if (tmpMat.isNull())
              {
                ...
                if (vertexShader.isNull())
                  {
                    ...
                    if (!shaderLanguage.compare("hlsl")
                        || !shaderLanguage.compare("cg"))
                      {
                        ...
                        if (fadeTechnique == FADETECH_ALPHA
                            || fadeTechnique == FADETECH_ALPHAGROW)
                          vertexProgSource +=
                    ...
                    else
                      {
                        ...
                        if (geom->getSceneManager()->getFogMode()
                            == Ogre::FOG_EXP2)
                          {
                            vertexProgSource +=
                ...
                tmpMat->load();
                Ogre::Material::TechniqueIterator techIterator =
                    tmpMat->getSupportedTechniqueIterator();
                while (techIterator.hasMoreElements())
                  {
                    ...
                    params->setNamedAutoConstant("camPos",
                        GpuProgramParameters::ACT_CAMERA_POSITION_OBJECT_SPACE);
                    params->setNamedAutoConstant("fadeRange",
                        GpuProgramParameters::ACT_CUSTOM, 1);

                    if (animate)
                      {
                        params->setNamedConstant("time", 1.0f);
                        params->setNamedConstant("frequency", 1.0f);
                        params->setNamedConstant("direction",
                            Ogre::Vector4::ZERO);
                      }

                    if (lighting)
                      {
                        params->setNamedAutoConstant("objSpaceLight",
                        ...
                        params->setNamedAutoConstant("lightAmbient",
                            GpuProgramParameters::ACT_DERIVED_AMBIENT_LIGHT_COLOUR);
                      }

                    if (fadeTechnique == FADETECH_GROW
                        || fadeTechnique == FADETECH_ALPHAGROW)
                      {
                        params->setNamedConstant("grassHeight",
                            maxHeight * 1.05f);
                      }
  ...
  void
  GrassPage::init(PagedGeometry *geom, const Ogre::Any &data)
  {
  ...
  void
  GrassPage::addEntity(Entity *entity, const Vector3 &position,
...
//	entity->setCastShadows(false);
  ...
  void
  GrassPage::setVisible(bool visible)
  {
    std::list<SceneNode*>::iterator i;
    for (i = nodeList.begin(); i != nodeList.end(); ++i)
      {
