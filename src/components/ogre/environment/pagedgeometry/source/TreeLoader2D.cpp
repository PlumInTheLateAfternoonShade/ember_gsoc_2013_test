...
namespace Forests
{
  ...
  TreeLoader2D::TreeLoader2D(PagedGeometry *geom, const TBounds &bounds)
  {
    ...
    actualBounds = bounds;
    gridBounds = bounds;
    gridBounds.left = pageSize
        * Math::Floor((gridBounds.left - geom->getBounds().left) / pageSize)
        + geom->getBounds().left;
    gridBounds.top = pageSize
        * Math::Floor((gridBounds.top - geom->getBounds().top) / pageSize)
        + geom->getBounds().top;
    gridBounds.right = pageSize
        * Math::Ceil((gridBounds.right - geom->getBounds().left) / pageSize)
        + geom->getBounds().left;
    gridBounds.bottom = pageSize
        * Math::Ceil((gridBounds.bottom - geom->getBounds().top) / pageSize)
        + geom->getBounds().top;
  ...
  void
  TreeLoader2D::addTree(Entity *entity, const Vector3 &position, Degree yaw,
      Real scale, void* userData)
  {
  ...
  void
#endif
  TreeLoader2D::deleteTrees(const Ogre::Vector3 &position, Ogre::Real radius,
      Entity *type)
  {
    //First convert the coordinate to PagedGeometry's local system
#ifdef PAGEDGEOMETRY_ALTERNATE_COORDSYSTEM
            ...
            for (int tileX = minPageX; tileX <= maxPageX; ++tileX)
              {
                ...
                std::vector<TreeDef> &treeList = _getGridPage(pageGrid, tileX,
                    tileZ);
                uint32 i = 0;
                while (i < treeList.size())
                  {
                    ...
                    float distX = (gridBounds.left + (tileX * pageSize)
                        + ((Real) treeList[i].xPos / 65535) * pageSize) - pos.x;
                    float distZ = (gridBounds.top + (tileZ * pageSize)
                        + ((Real) treeList[i].zPos / 65535) * pageSize) - pos.z;
                ...
                if (modified)
                  {
                    Vector3 pos(gridBounds.left + ((0.5f + tileX) * pageSize),
                        0, gridBounds.top + ((0.5f + tileZ) * pageSize));
                ...
                while (i < treeList.size())
                  {
                    ...
                    float posX = (gridBounds.left + (tileX * pageSize)
                        + ((Real) treeList[i].xPos / 65535) * pageSize);
                    float posZ = (gridBounds.top + (tileZ * pageSize)
                        + ((Real) treeList[i].zPos / 65535) * pageSize);
                    if (posX >= area.left && posX <= area.right
                        && posZ >= area.top && posZ <= area.bottom)
                      {
                        //If so, delete it
#ifdef PAGEDGEOMETRY_USER_DATA
                        deletedUserData.push_back(treeList[i].userData);
  ...
  void
  TreeLoader2D::setColorMap(const Ogre::String &mapFile, MapChannel channel)
  {
    if (colorMap)
      {
        colorMap->unload();
  ...
  void
  TreeLoader2D::setColorMap(Ogre::TexturePtr map, MapChannel channel)
  {
    if (colorMap)
      {
        colorMap->unload();
  ...
  void
  TreeLoader2D::loadPage(PageInfo &page)
  {
    ...
    if (page.xIndex < 0 || page.zIndex < 0 || page.xIndex >= pageGridX
        || page.zIndex >= pageGridZ)
      return;
    ...
    for (i = pageGridList.begin(); i != pageGridList.end(); ++i)
      {
        ...
        std::vector<TreeDef> &treeList = _getGridPage(pageGrid, page.xIndex,
            page.zIndex);
  ...
  TreeIterator2D::TreeIterator2D(TreeLoader2D *trees)
  {
    ...
    currentX = 0;
    currentZ = 0;
    currentTreeList = &trees->_getGridPage(currentGrid->second, currentX,
        currentZ);
  ...
  void
  TreeIterator2D::moveNext()
  {
    //Out of bounds check
    if (!hasMore)
      OGRE_EXCEPT(1, "Cannot read past end of TreeIterator list",
  ...
  TreeIterator2D::_readTree()
  {
    ...
    currentTreeDat.position.x = boundsLeft
        + ((Real) treeDef.xPos / 65535) * trees->pageSize;
    currentTreeDat.position.z = boundsTop
        + ((Real) treeDef.zPos / 65535) * trees->pageSize;

    //Calculate terrain height at x / z to get y
    if (trees->heightFunction != NULL)
      currentTreeDat.position.y = trees->heightFunction(
          currentTreeDat.position.x, currentTreeDat.position.z,
          trees->heightFunctionUserData);
    else
    ...
    //Get scale
    currentTreeDat.scale = (Real) treeDef.scale * (trees->maximumScale / 255)
        + trees->minimumScale;
