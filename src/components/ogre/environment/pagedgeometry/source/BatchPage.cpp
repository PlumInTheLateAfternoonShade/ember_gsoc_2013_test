...
//-------------------------------------------------------------------------------------
  ...
  void
  BatchPage::init(PagedGeometry *_geom, const Any &data)
  {
    geom = _geom;
  ...
  void
  BatchPage::addEntity(Entity *ent, const Vector3 &position,
      ...
      const Ogre::ColourValue &color)
  {
    ...
    if (mLODLevel == 0 || numManLod == 0)
      ...
      {
        const size_t bestLod =
            (numManLod < mLODLevel - 1) ? numManLod : (mLODLevel - 1);
  ...
  void
  BatchPage::setVisible(bool visible)
  {
    batch->setVisible(visible);
  }

  void
  BatchPage::setFade(bool enabled, Real visibleDist, Real invisibleDist)
  {
    if (!shadersSupported)
      return;
    ...
    while (it.hasMoreElements())
      {
        ...
        if (lightingEnabled)
          tmpName << "lit_";
        if (subBatch->vertexData->vertexDeclaration->findElementBySemantic(
            VES_DIFFUSE) != NULL)
          tmpName << "clr_";

        for (unsigned short i = 0;
            i < subBatch->vertexData->vertexDeclaration->getElementCount(); ++i)
          {
            const VertexElement *el =
                subBatch->vertexData->vertexDeclaration->getElement(i);
        ...
        if (HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull())
          {
            ...
            if (!shaderLanguage.compare("hlsl")
                || !shaderLanguage.compare("cg"))
              {
                ...
                for (unsigned short i = 0;
                    i
                        < subBatch->vertexData->vertexDeclaration->getElementCount();
                    ++i)
                  {
                    const VertexElement *el =
                        subBatch->vertexData->vertexDeclaration->getElement(i);
                    if (el->getSemantic() == VES_TEXTURE_COORDINATES)
                      {
                        ...
                        vertexProgSource += "	" + uvType + " iUV"
                            + StringConverter::toString(texNum)
                            + "			: TEXCOORD"
                            + StringConverter::toString(texNum) + ",	\n"
                                "	out " + uvType + " oUV"
                            + StringConverter::toString(texNum) + "		: TEXCOORD"
                            + StringConverter::toString(texNum) + ",	\n";
                ...
                vertexProgSource += "	out float oFog : FOG,	\n"
                ...
                if (fadeEnabled)
                  vertexProgSource += "	uniform float3 camPos, \n";
                ...
                for (unsigned short i = 0;
                    ...
                    ++i)
                  {
                    ...
                    if (el->getSemantic() == VES_TEXTURE_COORDINATES)
                      {
                        vertexProgSource += "	oUV"
                            + StringConverter::toString(texNum) + " = iUV"
                            + StringConverter::toString(texNum) + ";	\n";
                        ++texNum;
                      }
                  }
                ...
                if (sceneMgr->getFogMode() == Ogre::FOG_EXP2)
                  ...
                  {
                    vertexProgSource += "	oFog = oPosition.z; \n";
                  }
                vertexProgSource += "}";
              }
            ...
            if (!shaderLanguage.compare("glsl"))
              {
                vertexProgSource = "uniform float fadeGap;        \n"
                    "uniform float invisibleDist;   \n";
            ...
            if (vertexShader->hasCompileError())
              {
                OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR,
                    "Error loading the batching vertex shader.",
                    "BatchPage::_updateShaders()");
              }
          }
        ...
        HighLevelGpuProgramPtr fragShader =
            static_cast<HighLevelGpuProgramPtr>(HighLevelGpuProgramManager::getSingleton().getByName(
                fragmentProgramName));
        if (fragShader.isNull())
          {
            ...
            if (fragShader->hasCompileError())
              {
                OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR,
                    "Error loading the batching fragment shader.",
                    "BatchPage::_updateShaders()");
              }
          }
        ...
        if (generatedMaterial.isNull())
          {
            ...
            Ogre::Material::TechniqueIterator I =
                generatedMaterial->getSupportedTechniqueIterator();
                    ...
                    try
                      {
                        ...
                        if (shaderLanguage.compare("glsl"))
                          ...
                          {
                            params->setNamedAutoConstant("iFogParams",
                                GpuProgramParameters::ACT_FOG_PARAMS);
                          }
                        ...
                        if (fadeEnabled)
                          {
                            ...
                            //Set fade ranges
                            params->setNamedAutoConstant("invisibleDist",
                                GpuProgramParameters::ACT_CUSTOM);
                            params->setNamedConstant("invisibleDist",
                                invisibleDist);

                            params->setNamedAutoConstant("fadeGap",
                                GpuProgramParameters::ACT_CUSTOM);
                            params->setNamedConstant("fadeGap",
                                invisibleDist - visibleDist);

                            if (pass->getAlphaRejectFunction()
                                == CMPF_ALWAYS_PASS)
                        ...
                        if (pass->hasFragmentProgram())
                          {
                            ...
                            params->setIgnoreMissingParams(true);
                            params->setNamedAutoConstant("iFogColour",
                                GpuProgramParameters::ACT_FOG_COLOUR);
                          }
                      }
                    catch (...)
                      {
