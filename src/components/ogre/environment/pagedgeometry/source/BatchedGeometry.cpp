...
//-------------------------------------------------------------------------------------
  ...
  BatchedGeometry::BatchedGeometry(SceneManager *mgr, SceneNode *rootSceneNode) :
      boundsUndefined(true), sceneMgr(mgr), sceneNode(NULL), parentSceneNode(
          rootSceneNode), minDistanceSquared(0), withinFarDistance(0), built(
          false)
  {
    clear();
  }
  ...
  void
  BatchedGeometry::addEntity(Entity *ent, const Vector3 &position,
    ...
    for (uint32 i = 0; i < ent->getNumSubEntities(); ++i)
      {
        ...
        if (subMesh->vertexData == NULL)
          OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR,
              "SubMesh vertex data not found!", "BatchedGeometry::addEntity()");
  ...
  uint32
  CountUsedVertices(IndexData *id, std::map<uint32, uint32> &ibmap)
  {
    uint32 i, count;
    ...
    case HardwareIndexBuffer::IT_16BIT:
      {
        uint16 *data = (uint16*) id->indexBuffer->lock(
            id->indexStart * sizeof(uint16), id->indexCount * sizeof(uint16),
    ...
    case HardwareIndexBuffer::IT_32BIT:
      {
        uint32 *data = (uint32*) id->indexBuffer->lock(
            id->indexStart * sizeof(uint32), id->indexCount * sizeof(uint32),
            HardwareBuffer::HBL_READ_ONLY);
    ...
    default:
      throw new Ogre::Exception(0, "Unknown index buffer type",
          "Converter.cpp::CountVertices");
      break;
      }
  ...
  void
  BatchedGeometry::extractVertexDataFromShared(MeshPtr mesh)
  {
    ...
    while (subMeshIterator.hasMoreElements())
      {
        ...
        newVertexData->vertexDeclaration =
            oldVertexData->vertexDeclaration->clone();

        // Create new vertex buffers
        uint32 buffersCount =
            (uint32) oldVertexData->vertexBufferBinding->getBufferCount();
        for (uint32 bufferIndex = 0; bufferIndex < buffersCount; bufferIndex++)
          {

            // Lock shared vertex buffer
            HardwareVertexBufferSharedPtr oldVertexBuffer =
                oldVertexData->vertexBufferBinding->getBuffer(bufferIndex);
            size_t vertexSize = oldVertexBuffer->getVertexSize();
            uint8 *oldLock = (uint8*) oldVertexBuffer->lock(0,
                oldVertexData->vertexCount * vertexSize,
                HardwareBuffer::HBL_READ_ONLY);

            // Create and lock nonshared vertex buffer
            HardwareVertexBufferSharedPtr newVertexBuffer =
                HardwareBufferManager::getSingleton().createVertexBuffer(
                    vertexSize, newVertexData->vertexCount,
                    oldVertexBuffer->getUsage(),
                    oldVertexBuffer->hasShadowBuffer());
            ...
            for (i = indicesMap.begin(); i != iend; i++)
              {
                memcpy(newLock + vertexSize * i->second,
                    oldLock + vertexSize * i->first, vertexSize);
              }
            ...
            newVertexData->vertexBufferBinding->setBinding(bufferIndex,
                newVertexBuffer);
          }
        ...
        case HardwareIndexBuffer::IT_16BIT:
          {
            uint16 *data = (uint16*) indexData->indexBuffer->lock(
                indexData->indexStart * sizeof(uint16),
        ...
        case HardwareIndexBuffer::IT_32BIT:
          {
            uint32 *data = (uint32*) indexData->indexBuffer->lock(
                indexData->indexStart * sizeof(uint32),
  ...
  BatchedGeometry::getSubBatchIterator() const
  {
    return BatchedGeometry::SubBatchIterator((SubBatchMap&) subBatchMap);
  }

  String
  BatchedGeometry::getFormatString(SubEntity *ent)
  {
    ...
    if (subBatchMap.size() != 0)
      {
        ...
        //Build each batch
        for (SubBatchMap::iterator i = subBatchMap.begin();
            i != subBatchMap.end(); ++i)
          {
            i->second->build();
          }
  ...
  BatchedGeometry::clear()
  {
    ...
    //Delete each batch
    for (SubBatchMap::iterator i = subBatchMap.begin(); i != subBatchMap.end();
        ++i)
      {
        delete i->second;
      }
  ...
  void
  BatchedGeometry::_updateRenderQueue(RenderQueue *queue)
  {
    //If visible...
    if (isVisible())
      {
  ...
  bool
  BatchedGeometry::isVisible()
  {
    return mVisible && withinFarDistance;
  }

  void
  BatchedGeometry::_notifyCurrentCamera(Camera *cam)
  {
    if (getRenderingDistance() == 0)
    ...
    else
      {
        ...
        minDistanceSquared =
            max(0.0f, centerDistanceSquared - (radius * radius));
        ...
        withinFarDistance = minDistanceSquared
            <= Math::Sqr(getRenderingDistance());
      }
  }

  Ogre::Vector3
  BatchedGeometry::_convertToLocal(const Vector3 &globalVec) const
  {
  ...
  BatchedGeometry::SubBatch::SubBatch(BatchedGeometry *parent, SubEntity *ent)
  {
    ...
    if (origMat)
      {
        material = MaterialManager::getSingleton().getByName(
            getMaterialClone(*origMat)->getName());
      }
    else
      {
        MaterialManager::ResourceCreateOrRetrieveResult result =
            MaterialManager::getSingleton().createOrRetrieve(
                "PagedGeometry_Batched_Material", "General");
        if (result.first.isNull())
          {
            OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS,
                "BatchedGeometry failed to create a material for entity with invalid material.",
                "BatchedGeometry::SubBatch::SubBatch(BatchedGeometry *parent, SubEntity *ent)");
          }
    ...
    const VertexElement* blendIndices =
        vertexData->vertexDeclaration->findElementBySemantic(VES_BLEND_INDICES);
    ...
    if (blendIndices && blendWeights)
      {
        ...
        assert(
            blendIndices->getSource() == blendWeights->getSource() && "Blend indices and weights should be in the same buffer");
        assert(
            blendIndices->getSize() + blendWeights->getSize() == vertexData->vertexBufferBinding->getBuffer(blendIndices->getSource())->getVertexSize() && "Blend indices and blend buffers should have buffer to themselves!");

        //Remove the blend weights
        vertexData->vertexBufferBinding->unsetBinding(
            blendIndices->getSource());
  ...
  Material *
  BatchedGeometry::SubBatch::getMaterialClone(Material &mat)
  {
    String clonedName = mat.getName() + "_Batched";
    MaterialPtr clonedMat = MaterialManager::getSingleton().getByName(
        clonedName);
    if (clonedMat.isNull())
  ...
  void
  BatchedGeometry::SubBatch::addSubEntity(SubEntity *ent,
    ...
    if (newMesh.color != ColourValue::White)
      {
        ...
        VertexElementType format =
            Root::getSingleton().getRenderSystem()->getColourVertexElementType();
        switch (format)
          {
        ...
        case VET_COLOUR_ABGR:
          break;
        default:
          OGRE_EXCEPT(0, "Unknown RenderSystem color format",
              "BatchedGeometry::SubBatch::addSubMesh()")
          ;
          break;
          }
      }
  ...
  void
  BatchedGeometry::SubBatch::build()
  {
    assert(!built);
    ...
    HardwareIndexBuffer::IndexType srcIndexType =
        meshType->indexData->indexBuffer->getType();
    HardwareIndexBuffer::IndexType destIndexType;
    if (vertexData->vertexCount > 0xFFFF
        || srcIndexType == HardwareIndexBuffer::IT_32BIT)
    ...
    indexData->indexBuffer =
        HardwareBufferManager::getSingleton().createIndexBuffer(destIndexType,
            indexData->indexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);

    //Lock the index buffer
    uint32 *indexBuffer32;
    uint16 *indexBuffer16;
    if (destIndexType == HardwareIndexBuffer::IT_32BIT)
      indexBuffer32 = static_cast<uint32*>(indexData->indexBuffer->lock(
          HardwareBuffer::HBL_DISCARD));
    else
      indexBuffer16 = static_cast<uint16*>(indexData->indexBuffer->lock(
          HardwareBuffer::HBL_DISCARD));
    ...
    for (Ogre::ushort i = 0; i < vertBinding->getBufferCount(); ++i)
      {
        HardwareVertexBufferSharedPtr buffer =
            HardwareBufferManager::getSingleton().createVertexBuffer(
                vertDecl->getVertexSize(i), vertexData->vertexCount,
                HardwareBuffer::HBU_STATIC_WRITE_ONLY);
        vertBinding->setBinding(i, buffer);

        vertexBuffers.push_back(
            static_cast<uchar*>(buffer->lock(HardwareBuffer::HBL_DISCARD)));
            ...
            if (i < sourceBinds->getBufferCount())
              {
                ...
                HardwareVertexBufferSharedPtr sourceBuffer =
                    sourceBinds->getBuffer(i);
                uchar *sourceBase = static_cast<uchar*>(sourceBuffer->lock(
                    HardwareBuffer::HBL_READ_ONLY));
                    ...
                    for (ei = elems.begin(); ei != elems.end(); ++ei)
                      {
                        ...
                        case VES_POSITION:
                          ...
                          //Transform
                          tmp = (queuedMesh.orientation
                              * (tmp * queuedMesh.scale)) + queuedMesh.position;
            ...
            else
              {
                ...
                //Get the locked output buffer
                uint32 *startPtr =
                    (uint32*) vertexBuffers[vertBinding->getBufferCount() - 1];
                ...
                uint32 tmpColor = tmpR | (tmpG << 8) | (tmpB << 16)
                    | (0xFF << 24);
                ...
                vertexBuffers[vertBinding->getBufferCount() - 1] +=
                    (sizeof(uint32) * sourceVertexData->vertexCount);
              }
          }
        ...
        if (srcIndexType == HardwareIndexBuffer::IT_32BIT)
          {
            //Lock the input buffer
            uint32 *source =
                static_cast<uint32*>(sourceIndexData->indexBuffer->lock(
                    sourceIndexData->indexStart, sourceIndexData->indexCount,
                    HardwareBuffer::HBL_READ_ONLY));
            ...
            if (destIndexType == HardwareIndexBuffer::IT_32BIT)
              {
                ...
                //Lock the input buffer
                uint16 *source =
                    static_cast<uint16*>(sourceIndexData->indexBuffer->lock(
                        sourceIndexData->indexStart,
                        sourceIndexData->indexCount,
                        HardwareBuffer::HBL_READ_ONLY));
            ...
            else
              {
                ...
                while (source != sourceEnd)
                  {
                    *indexBuffer16++ = static_cast<uint16>(*source++
                        + indexOffset);
                  }
  ...
  void
  BatchedGeometry::SubBatch::clear()
  {
  ...
  void
  BatchedGeometry::SubBatch::addSelfToRenderQueue(RenderQueue *queue,
      uint8 group)
  {
    if (built)
      {
        ...
        bestTechnique = material->getBestTechnique(
            material->getLodIndex(
                parent->minDistanceSquared * parent->minDistanceSquared));
  ...
  void
  BatchedGeometry::SubBatch::getRenderOperation(RenderOperation& op)
  {
  ...
  Real
  BatchedGeometry::SubBatch::getSquaredViewDepth(const Camera* cam) const
  {
    Vector3 camVec = parent->_convertToLocal(cam->getDerivedPosition())
        - parent->center;
...
//Eihort-compatible getLights()
  ...
  BatchedGeometry::SubBatch::getLights(void) const
  {
