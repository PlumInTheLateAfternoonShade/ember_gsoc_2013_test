...
//-------------------------------------------------------------------------------------
  ...
  void
  WindBatchPage::init(PagedGeometry *geom, const Any &data)
  {
  ...
  WindBatchPage::_updateShaders()
  {
    ...
    while (it.hasMoreElements())
      {
        WindBatchedGeometry::WindSubBatch *subBatch =
            dynamic_cast<WindBatchedGeometry::WindSubBatch*>(it.getNext());
            ...
            if (!shaderLanguage.compare("hlsl")
                || !shaderLanguage.compare("cg"))
              {
                ...
                for (unsigned short j = 0;
                  ...
                  {
                    const VertexElement *el =
                        subBatch->vertexData->vertexDeclaration->getElement(j);
                    if (el->getSemantic() == VES_TEXTURE_COORDINATES)
                      {
                        ++texCoordCount;
                      }
                    ...
                    if (el->getSemantic() == VES_TEXTURE_COORDINATES)
                      {
                        if (el->getIndex() == texCoordCount - 2)
                          {
                            vertexProgSource += "	float4 params 	: TEXCOORD"
                                + StringConverter::toString(texCoordCount - 2)
                                + ", \n";
                          }
                        else
                          {
                ...
                for (unsigned short i = 0; i < texCoordCount - 2; ++i)
                  {
                    vertexProgSource += "	oUV" + StringConverter::toString(i)
                        + " = iUV" + StringConverter::toString(i) + "; \n";
                  }
                        ...
                        if (el->getIndex() == texCoordCount - 2)
                        ...
                        else
                          {
                            if (el->getIndex() == texCoordCount - 1)
                              {
                                vertexProgSource +=
                                    "	vec4 originPos = gl_MultiTexCoord"
                                        + StringConverter::toString(
                                            texCoordCount - 1) + "; \n";
                              }
                            else
        ...
        if (generatedMaterial.isNull())
          {
            ...
            for (unsigned short t = 0;
                t < generatedMaterial->getNumTechniques(); ++t)
              {
