...
namespace Forests
{
  ...
  DensityMap *
  DensityMap::load(const String &fileName, MapChannel channel)
  {
    //Load image
    TexturePtr map = TextureManager::getSingleton().load(fileName,
  ...
  DensityMap *
  DensityMap::load(TexturePtr texture, MapChannel channel)
  {
    const String key = texture->getName()
        + StringConverter::toString((int) channel);
  ...
  DensityMap::DensityMap(TexturePtr map, MapChannel channel)
  {
    ...
    pixels = new PixelBox(Box(0, 0, buff->getWidth(), buff->getHeight()),
        PF_BYTE_L);
    ...
    if (channel == CHANNEL_COLOR)
    ...
    else
      {
        ...
        PixelBox tmpPixels(Box(0, 0, buff->getWidth(), buff->getHeight()),
            PF_R8G8B8A8);
...
//Make sure a density map exists before calling this.
  float
  DensityMap::_getDensityAt_Unfiltered(float x, float z,
      const TRect<Real> &mapBounds)
  {
    assert(pixels);

    // Early out if the coordinates are outside map bounds.
    if (x < mapBounds.left || x >= mapBounds.right || z < mapBounds.top
        || z >= mapBounds.bottom)
      {
        return 0.0f;
      }
    ...
    if (Root::getSingleton().getRenderSystem()->getName()
...
//Make sure a density map exists before calling this.
  float
  DensityMap::_getDensityAt_Bilinear(float x, float z,
      const TRect<Real> &mapBounds)
  {
    assert(pixels);
    ...
    if (xIndex < 0 || zIndex < 0 || xIndex >= mapWidth - 1
        || zIndex >= mapHeight - 1)
      return 0.0f;
...
//----------------------------------------------------------------------------------------------
  ...
  ColorMap *
  ColorMap::load(const String &fileName, MapChannel channel)
  {
    //Load image
    TexturePtr map = TextureManager::getSingleton().load(fileName,
        ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
  ...
  ColorMap *
  ColorMap::load(TexturePtr texture, MapChannel channel)
  {
    const String key = texture->getName()
        + StringConverter::toString((int) channel);
...
//Returns the color map value at the given location
  uint32
  ColorMap::_getColorAt(float x, float z, const TRect<Real> &mapBounds)
  {
    assert(pixels);
  ...
  uint32
  ColorMap::_interpolateColor(uint32 color1, uint32 color2, float ratio,
      float ratioInv)
  {
    uint8 a1, b1, c1, d1;
    a1 = (color1 & 0xFF);
  ...
  uint32
  ColorMap::_getColorAt_Bilinear(float x, float z, const TRect<Real> &mapBounds)
  {
    assert(pixels);
