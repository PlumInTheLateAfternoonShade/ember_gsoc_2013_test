...
//-------------------------------------------------------------------------------------
  ...
  public:
     ...
     */
    void
    setBounds(const TBounds bounds);
     ...
     */
    TBounds
    convertAABToTBounds(const Ogre::AxisAlignedBox & aab) const;
    ...
    template<class PageType>
      inline GeometryPageManager*
      addDetailLevel(Ogre::Real maxRange, Ogre::Real transitionLength = 0,
     ...
     */
    inline const std::list<GeometryPageManager*> &
    getDetailLevels()
    {
      return managerList;
    }
     ...
     */
    void
    setPageLoader(PageLoader *loader);
     ...
     */
    void
    reloadGeometryPage(const Ogre::Vector3 &point, bool forceLoadImmediately =
        false);
     ...
     */
    void
    reloadGeometryPages(const Ogre::Vector3 &center, Ogre::Real radius);
     ...
     */
    void
    reloadGeometryPages(const TBounds & area);
     ...
     */
    void
    preloadGeometry(const TBounds & area);
    ...
    Ogre::Vector3
    _convertToLocal(const Ogre::Vector3 &globalVec) const;
     ...
     */
    void
    setCustomParam(std::string entity, std::string paramName, float paramValue);
     ...
     */
    void
    setCustomParam(std::string paramName, float paramValue);
     ...
     */
    float
    getCustomParam(std::string entity, std::string paramName,
     ...
     */
    float
    getCustomParam(std::string paramName, float defaultParamValue) const;
    ...
    getRenderQueue() const;

  protected:
    //Internal function - do not use
    void
    _addDetailLevel(GeometryPageManager *mgr, Ogre::Real maxRange,
...
//-------------------------------------------------------------------------------------
  ...
  public:
     ...
     */
    virtual void
    init(PagedGeometry *geom, const Ogre::Any &data) = 0;

    void
    setQueryFlag(Ogre::uint32 flag)
    {
      mHasQueryFlag = true;
      mQueryFlag = flag;
    }
     ...
     */
    virtual void
    setRegion(Ogre::Real left, Ogre::Real top, Ogre::Real right,
        Ogre::Real bottom)
    {
    }
    ;
     ...
     */
    virtual void
    addEntity(Ogre::Entity *ent, const Ogre::Vector3 &position,
     ...
     */
    virtual void
    setFade(bool enabled, Ogre::Real visibleDist = 0, Ogre::Real invisibleDist =
        0) = 0;
     ...
     */
    virtual const Ogre::AxisAlignedBox &
    getBoundingBox();
     ...
     */
    virtual void
    addEntityToBoundingBox(Ogre::Entity *ent, const Ogre::Vector3 &position,
        const Ogre::Quaternion &rotation, const Ogre::Vector3 &scale);
...
//-------------------------------------------------------------------------------------
  ...
  protected:
    ...
    addEntity(Ogre::Entity *ent, const Ogre::Vector3 &position,
        const Ogre::Quaternion &rotation, const Ogre::Vector3 &scale =
            Ogre::Vector3::UNIT_SCALE, const Ogre::ColourValue &color =
...
//-------------------------------------------------------------------------------------
  ...
  public:
     ...
     */
    inline void
    setNearRange(Ogre::Real nearRange)
    {
      nearDist = nearRange;
     ...
     */
    inline void
    setFarRange(Ogre::Real farRange)
    {
      farDist = farRange;
     ...
     */
    inline void
    setCacheSpeed(unsigned long maxCacheInterval = 200,
    ...
    inline void
    setTransition(Ogre::Real transitionLength)
    {
    ...
    /** \brief Internal function - DO NOT USE */
    inline TPGeometryPages
    getLoadedPages() const
    {
      return loadedList;
    }
    ...
    template<class PageType>
      void
      initPages(const TBounds& bounds, const Ogre::Any &data = Ogre::Any(),
          Ogre::uint32 queryFlag = 0);

    /** \brief Internal function - DO NOT USE */
    void
    update(unsigned long deltaTime, Ogre::Vector3 &camPos,
        Ogre::Vector3 &camSpeed, bool &enableCache,
  ...
  private:
    ...
    inline GeometryPage *
    _getGridPage(const int x, const int z)
    {
#ifdef _DEBUG
    ...
    inline void
    _setGridPage(const int x, const int z, GeometryPage *page)
    {
#ifdef _DEBUG
      if(x >= geomGridX || z >= geomGridZ )
      OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS,
    ...
    void
    _loadPage(GeometryPage *page);
    void
    _unloadPage(GeometryPage *page);
    void
    _unloadPageDelayed(GeometryPage *page);

    //Utility function for scrolling pages in the grid by the given amount
    void
    _scrollGridPages(int shiftX, int shiftZ);
...
//-------------------------------------------------------------------------------------
  ...
  template<class PageType>
    inline GeometryPageManager*
    PagedGeometry::addDetailLevel(Ogre::Real maxRange,
        Ogre::Real transitionLength, const Ogre::Any &data,
  ...
  template<class PageType>
    inline void
    GeometryPageManager::initPages(const TBounds& bounds, const Ogre::Any &data,
        Ogre::uint32 queryFlag)
    {
      ...
      int xioffset = Ogre::Math::Floor(
          gridBounds.left / mainGeom->getPageSize());
      int zioffset = Ogre::Math::Floor(
          gridBounds.top / mainGeom->getPageSize());
          ...
          for (int z = 0; z < geomGridZ; ++z)
            {
              ...
              page->_centerPoint.x = ((x + 0.5f) * mainGeom->getPageSize())
                  + gridBounds.left;
              page->_centerPoint.z = ((z + 0.5f) * mainGeom->getPageSize())
                  + gridBounds.top;
