...
public:
  ...
  getUnitRandom();
  float
  getRangeRandom(float start, float end);
...
inline
RandomTable::RandomTable(unsigned long size) :
    tableSize(size), table(0), customRandomIndex(0)
{
...
inline void
RandomTable::resetRandomIndex()
{
  customRandomIndex = 0;
}

inline float
RandomTable::getUnitRandom()
{
...
inline float
RandomTable::getRangeRandom(float start, float end)
{
