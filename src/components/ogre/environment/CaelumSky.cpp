  ...
  namespace OgreView
  {
    ...
    namespace Environment
    {
          ...
          CaelumEnvironmentComponent(environment)
      {
        ...
        if (mCaelumSystem->getCloudSystem())
          {
            ...
            Caelum::FlatCloudLayer* cloudLayer =
                cloudSystem->createLayerAtHeight(1500);
        ...
        registerConfigListener("caelum", "cloudspeed",
            sigc::mem_fun(*this, &CaelumSky::Config_CloudSpeed));
        registerConfigListener("caelum", "cloudblendtime",
            sigc::mem_fun(*this, &CaelumSky::Config_CloudBlendTime));
        registerConfigListener("caelum", "cloudcover",
            sigc::mem_fun(*this, &CaelumSky::Config_CloudCover));
      ...
      void
      CaelumSky::Config_CloudSpeed(const std::string& section,
      ...
      void
      CaelumSky::Config_CloudBlendTime(const std::string& section,
          const std::string& key, varconf::Variable& variable)
      {
        if (variable.is_double() && mCaelumSystem->getCloudSystem())
          {
            mCaelumSystem->getCloudSystem()->getLayer(0)->setCloudBlendTime(
                static_cast<double>(variable));
          }
      }
      void
      CaelumSky::Config_CloudCover(const std::string& section,
          const std::string& key, varconf::Variable& variable)
      {
        if (variable.is_double() && mCaelumSystem->getCloudSystem())
          {
            mCaelumSystem->getCloudSystem()->getLayer(0)->setCloudCover(
                static_cast<double>(variable));
          }
      }

      void
      CaelumSky::setDensity(float density)
      {
        mCaelumSystem->setGlobalFogDensityMultiplier(density);
      }
      float
      CaelumSky::getDensity() const
      {
        return mCaelumSystem->getGlobalFogDensityMultiplier();
      }

      bool
      CaelumSky::frameEnded(const Ogre::FrameEvent & event)
      {
        return true;
      }
