...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Environment
    {
      ...
      CaelumSun::CaelumSun(CaelumEnvironment& environment,
          Caelum::BaseSkyLight* sun) :
          CaelumEnvironmentComponent(environment), mSun(sun)
      {
        ...
        registerConfigListener("caelum", "sunambientmultiplier",
            sigc::mem_fun(*this, &CaelumSun::Config_SunAmbientMultiplier));
        registerConfigListener("caelum", "sundiffusemultiplier",
            sigc::mem_fun(*this, &CaelumSun::Config_SunDiffuseMultiplier));
        registerConfigListener("caelum", "sunspecularmultiplier",
            sigc::mem_fun(*this, &CaelumSun::Config_SunSpecularMultiplier));
      ...
      void
      CaelumSun::setAmbientLight(const Ogre::ColourValue& colour)
      {
        mCaelumSystem->getSceneMgr()->setAmbientLight(colour);
      }

      Ogre::Vector3
      CaelumSun::getSunDirection() const
      {
        return mSun->getLightDirection();
      }

      WFMath::Vector<3>
      CaelumSun::getMainLightDirection() const
      {
        return Convert::toWF<WFMath::Vector<3>>(getSunDirection());
      }

      Ogre::ColourValue
      CaelumSun::getAmbientLightColour() const
      {
        return mSun->getMainLight()->getDiffuseColour();
      }

      void
      CaelumSun::Config_SunAmbientMultiplier(const std::string& section,
      ...
      void
      CaelumSun::Config_SunDiffuseMultiplier(const std::string& section,
          const std::string& key, varconf::Variable& variable)
      {
        Ogre::ColourValue colour;
        if (parse(variable, colour) && mSun)
      ...
      void
      CaelumSun::Config_SunSpecularMultiplier(const std::string& section,
          const std::string& key, varconf::Variable& variable)
      {
        Ogre::ColourValue colour;
        if (parse(variable, colour) && mSun)
      ...
      bool
      CaelumSun::parse(varconf::Variable& variable, Ogre::ColourValue& colour)
      {
