...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Environment
    {
      ...
      CaelumEnvironment::CaelumEnvironment(Ogre::SceneManager *sceneMgr,
          Ogre::RenderWindow* window, Ogre::Camera& camera,
          Eris::Calendar& calendar) :
          SetCaelumTime("set_caelumtime", this,
              "Sets the caelum time. parameters: <hour> <minute>"), mCaelumSystem(
              0), mSceneMgr(sceneMgr), mWindow(window), mCamera(camera), mCalendar(
              calendar), mSky(0), mSun(0), mWater(0)
      {
        mCalendar.Updated.connect(
            sigc::mem_fun(*this, &CaelumEnvironment::Calendar_Updated));
      }
      ...
      void
      CaelumEnvironment::createFirmament()
      {
        try
          {
      ...
      void
      CaelumEnvironment::destroyFirmament()
      {
        delete mSky;
        mSky = 0;
        delete mSun;
      ...
      void
      CaelumEnvironment::Calendar_Updated()
      {
        ...
        if (now.valid())
          {
            if (mCaelumSystem)
              {
                mCaelumSystem->getUniversalClock()->setGregorianDateTime(
                    now.year(), now.month(), now.dayOfMonth(), now.hours(),
      ...
      void
      CaelumEnvironment::setWaterEnabled(bool enabled)
      {
        if (enabled)
          {
            if (!mWater)
      ...
      void
      CaelumEnvironment::setupCaelum(::Ogre::SceneManager *sceneMgr,
          ::Ogre::RenderWindow* window, ::Ogre::Camera& camera)
      {
        ...
        try
          {
            mCaelumSystem->setSkyDome(
                new Caelum::SkyDome(mSceneMgr,
                    mCaelumSystem->getCaelumCameraNode()));
          }
        ...
        try
          {
            mCaelumSystem->setSun(
                new Caelum::SphereSun(mSceneMgr,
                    mCaelumSystem->getCaelumCameraNode()));
        ...
        try
          {
            mCaelumSystem->setMoon(
                new Caelum::Moon(mSceneMgr,
                    mCaelumSystem->getCaelumCameraNode()));
          }
        catch (const Caelum::UnsupportedException& ex)
          {
        ...
        try
          {
            mCaelumSystem->setCloudSystem(
                new Caelum::CloudSystem(mSceneMgr,
                    mCaelumSystem->getCaelumGroundNode()));
          }
        catch (const Caelum::UnsupportedException& ex)
          {
        ...
        try
          {
            mCaelumSystem->setPointStarfield(
                new Caelum::PointStarfield(mSceneMgr,
                    mCaelumSystem->getCaelumCameraNode()));
          }
        catch (const Caelum::UnsupportedException& ex)
          {
            ...
            try
              {
                mCaelumSystem->setImageStarfield(
                    new Caelum::ImageStarfield(mSceneMgr,
                        mCaelumSystem->getCaelumCameraNode()));
              }
            catch (const Caelum::UnsupportedException& ex)
              {
                S_LOG_WARNING(
                    "Error when creating Caelum image star field." << ex);
              }
          }
        ...
        mCaelumSystem->setMinimumAmbientLight(
        ...
        if (currentServerTime.valid())
          {
            mCaelumSystem->getUniversalClock()->setGregorianDateTime(
                currentServerTime.year(), currentServerTime.month(),
                currentServerTime.dayOfMonth(), currentServerTime.hours(),
                currentServerTime.minutes(), currentServerTime.seconds());
          }
        else
          {
            S_LOG_WARNING(
                "Could not get server time, using local time for environment.");
            int year, month, day, hour, minute, second;
            Ember::Time::getLocalTime(year, month, day, hour, minute, second);
            mCaelumSystem->getUniversalClock()->setGregorianDateTime(year,
      ...
      ISun*
      CaelumEnvironment::getSun()
      {
        return mSun;
      }

      ISky*
      CaelumEnvironment::getSky()
      {
        return mSky;
      }

      IFog*
      CaelumEnvironment::getFog()
      {
        return mSky;
        //return mFog;
      }
      ...
      IWater*
      CaelumEnvironment::getWater()
      {
        return mWater;
      }

      void
      CaelumEnvironment::setTime(int hour, int minute, int second)
      {
            ...
            if (currentServerTime.valid())
              {
                mCaelumSystem->getUniversalClock()->setGregorianDateTime(
                    currentServerTime.year(), currentServerTime.month(),
                    currentServerTime.dayOfMonth(), hour, minute, second);
              }
            else
              {
                int year, month, day, _hour, _minute, _second;
                Ember::Time::getLocalTime(year, month, day, _hour, _minute,
                    _second);
                mCaelumSystem->getUniversalClock()->setGregorianDateTime(year,
                    month, day, hour, minute, second);
              }
      ...
      void
      CaelumEnvironment::setTime(int seconds)
      {
        if (mCaelumSystem && mCaelumSystem->getUniversalClock())
          {
            Eris::DateTime currentServerTime = mCalendar.now();
            if (currentServerTime.valid())
              {
                mCaelumSystem->getUniversalClock()->setGregorianDateTime(
                    currentServerTime.year(), currentServerTime.month(),
                    currentServerTime.dayOfMonth(), 0, 0, seconds);
              }
            else
              {
                int year, month, day, hour, minute, _second;
                Ember::Time::getLocalTime(year, month, day, hour, minute,
                    _second);
                mCaelumSystem->getUniversalClock()->setGregorianDateTime(year,
                    month, day, 0, 0, seconds);
              }
          }
      }

      void
      CaelumEnvironment::setTimeMultiplier(float multiplier)
      {
        if (mCaelumSystem && mCaelumSystem->getUniversalClock())
          {
      ...
      float
      CaelumEnvironment::getTimeMultiplier() const
      {
        if (mCaelumSystem && mCaelumSystem->getUniversalClock())
          {
      ...
      void
      CaelumEnvironment::setWorldPosition(float longitudeDegrees,
      ...
      void
      CaelumEnvironment::runCommand(const std::string &command,
