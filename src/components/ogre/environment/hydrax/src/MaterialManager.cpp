...
namespace Hydrax
{
  MaterialManager::MaterialManager(Hydrax *h) :
      mComponents(HYDRAX_COMPONENTS_NONE), mHydrax(h), mCreated(false)
  {
    for (int k = 0; k < 6; k++)
      {
  ...
  void
  MaterialManager::removeMaterials()
  {
  ...
  bool
  MaterialManager::createMaterials(const HydraxComponent &Components,
      const Options &Options)
  {
    ...
    if (_isComponent(Components, HYDRAX_COMPONENT_UNDERWATER))
      {
        ...
        if (!_createSimpleColorMaterial(Ogre::ColourValue::Red, MAT_SIMPLE_RED,
            _def_Simple_Red_Material_Name, false))
          {
            return false;
          }
    ...
    for (TechIt = mDepthTechniques.begin(); TechIt != mDepthTechniques.end();
        TechIt++)
      {
        if (!(*TechIt))
          {
  ...
  bool
  MaterialManager::fillGpuProgramsToPass(Ogre::Pass* Pass,
      const Ogre::String GpuProgramNames[2], const ShaderMode& SM,
      const Ogre::String EntryPoints[2], const Ogre::String Data[2])
  {
    GpuProgram GpuPrograms[2] =
      { GPUP_VERTEX, GPUP_FRAGMENT };

    for (int k = 0; k < 2; k++)
      {
        if (!createGpuProgram(GpuProgramNames[k], SM, GpuPrograms[k],
            EntryPoints[k], Data[k]))
          {
            return false;
          }
      }
  ...
  bool
  MaterialManager::createGpuProgram(const Ogre::String &Name,
      const ShaderMode& SM, const GpuProgram& GPUP,
      const Ogre::String& EntryPoint, const Ogre::String& Data)
  {
    ...
    Ogre::HighLevelGpuProgramPtr HLGpuProgram =
        Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(Name,
            HYDRAX_RESOURCE_GROUP, ShaderModesStr[static_cast<int>(SM)],
            GpuPType);
  ...
  bool
  MaterialManager::_createWaterMaterial(const HydraxComponent &Components,
      const Options &Options)
  {
        ...
        case SM_CG:
          {
            ...
            if (Options.NM == NM_TEXTURE)
              {
                FragmentProgramData += "float2 iUvNoise      : TEXCOORD"
                    + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
                TEXCOORDNUM++;

              }
            FragmentProgramData += "float4 iUvProjection : TEXCOORD"
                + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
            TEXCOORDNUM++;
            if (Options.NM == NM_VERTEX)
              {
                FragmentProgramData += "float4 iNormal       : TEXCOORD"
                    + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
                TEXCOORDNUM++;
              }
            if (cFoam)
              {
                FragmentProgramData += "float4 iWorldPosition  : TEXCOORD"
                    + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
              }
            ...
            if (cDepth)
              {
                FragmentProgramData += "uniform float3       uWaterColor,\n";
              }
            if (cSmooth)
              {
                FragmentProgramData += "uniform float        uSmoothPower,\n";
              }
            if (cSun)
              {
                FragmentProgramData += Ogre::String(
            ...
            if (cCaustics)
              {
                FragmentProgramData += "uniform float        uCausticsPower,\n";
              }
            ...
            if (Options.NM == NM_TEXTURE || Options.NM == NM_RTT)
              {
                FragmentProgramData +=
                    "uniform sampler2D    uNormalMap       : register(s"
                        + Ogre::StringConverter::toString(TexNum) + "),\n";
                TexNum++;
              }

            FragmentProgramData += Ogre::String(
                "uniform sampler2D    uReflectionMap   : register(s"
                    + Ogre::StringConverter::toString(TexNum) + "),\n")
            ...
            if (cDepth)
              {
                FragmentProgramData +=
                    "uniform sampler2D    uDepthMap        : register(s"
                        + Ogre::StringConverter::toString(TexNum) + "),\n";
                TexNum++;
              }
            ...
            //"uniform sampler1D    uFresnelMap      : register(s" + Ogre::StringConverter::toString(TexNum) + ")";
                "uniform sampler2D    uFresnelMap      : register(s"
                    + Ogre::StringConverter::toString(TexNum) + ")";
            TexNum++;
    ...
    WaterMaterial = Ogre::MaterialManager::getSingleton().create(
        _def_Water_Material_Name, HYDRAX_RESOURCE_GROUP);

    Ogre::Pass *WM_Technique0_Pass0 = WaterMaterial->getTechnique(0)->getPass(
        0);
    ...
    Ogre::String GpuProgramsData[2] =
      { VertexProgramData, FragmentProgramData };
    Ogre::String GpuProgramNames[2] =
      { _def_Water_Shader_VP_Name, _def_Water_Shader_FP_Name };
    Ogre::String EntryPoints[2] =
      { "main_vp", "main_fp" };

    fillGpuProgramsToPass(WM_Technique0_Pass0, GpuProgramNames, Options.SM,
        EntryPoints, GpuProgramsData);

    Ogre::GpuProgramParametersSharedPtr VP_Parameters =
        WM_Technique0_Pass0->getVertexProgramParameters();
    Ogre::GpuProgramParametersSharedPtr FP_Parameters =
        WM_Technique0_Pass0->getFragmentProgramParameters();
    ...
    FP_Parameters->setNamedAutoConstant("uEyePosition",
        Ogre::GpuProgramParameters::ACT_CAMERA_POSITION_OBJECT_SPACE);

    FP_Parameters->setNamedConstant("uFullReflectionDistance",
        mHydrax->getFullReflectionDistance());
    FP_Parameters->setNamedConstant("uGlobalTransparency",
        mHydrax->getGlobalTransparency());
    FP_Parameters->setNamedConstant("uNormalDistortion",
        mHydrax->getNormalDistortion());

    if (cDepth)
      {
        FP_Parameters->setNamedConstant("uWaterColor",
            mHydrax->getWaterColor());
      }
    if (cSmooth)
      {
        FP_Parameters->setNamedConstant("uSmoothPower",
            mHydrax->getSmoothPower());
      }
    if (cSun)
      {
        FP_Parameters->setNamedConstant("uSunPosition",
            mHydrax->getMesh()->getObjectSpacePosition(
                mHydrax->getSunPosition()));
        FP_Parameters->setNamedConstant("uSunStrength",
            mHydrax->getSunStrength());
        FP_Parameters->setNamedConstant("uSunArea", mHydrax->getSunArea());
        FP_Parameters->setNamedConstant("uSunColor", mHydrax->getSunColor());
      }
    if (cFoam)
      {
        FP_Parameters->setNamedConstant("uFoamRange",
            mHydrax->getMesh()->getOptions().MeshStrength);
        FP_Parameters->setNamedConstant("uFoamMaxDistance",
            mHydrax->getFoamMaxDistance());
        ...
        FP_Parameters->setNamedConstant("uFoamTransparency",
            mHydrax->getFoamTransparency());
      }
    if (cCaustics)
      {
        FP_Parameters->setNamedConstant("uCausticsPower",
            mHydrax->getCausticsPower());
      }

    if (Options.NM == NM_TEXTURE || Options.NM == NM_RTT)
      {
        WM_Technique0_Pass0->createTextureUnitState("HydraxNormalMap")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_WRAP);
      }

    WM_Technique0_Pass0->createTextureUnitState("HydraxReflectionMap")->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_CLAMP);
    WM_Technique0_Pass0->createTextureUnitState("HydraxRefractionMap")->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_CLAMP);

    if (cDepth)
      {
        WM_Technique0_Pass0->createTextureUnitState("HydraxDepthMap")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_CLAMP);
      }

    WM_Technique0_Pass0->createTextureUnitState("Fresnel.bmp")->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_CLAMP);

    if (cFoam)
      {
        WM_Technique0_Pass0->createTextureUnitState("Foam.png")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_WRAP);
      }
  ...
  bool
  MaterialManager::_createDepthMaterial(const HydraxComponent &Components,
      const Options &Options)
  {
    const bool cCaustics = _isComponent(Components, HYDRAX_COMPONENT_CAUSTICS);
    ...
    DepthMaterial = Ogre::MaterialManager::getSingleton().create(
        _def_Depth_Material_Name, HYDRAX_RESOURCE_GROUP);

    DepthMaterial->getTechnique(0)->setSchemeName("HydraxDepth");

    Ogre::Pass *DM_Technique0_Pass0 = DepthMaterial->getTechnique(0)->getPass(
        0);

    Ogre::String GpuProgramsData[2] =
      { VertexProgramData, FragmentProgramData };
    Ogre::String GpuProgramNames[2] =
      { _def_Depth_Shader_VP_Name, _def_Depth_Shader_FP_Name };
    Ogre::String EntryPoints[2] =
      { "main_vp", "main_fp" };

    fillGpuProgramsToPass(DM_Technique0_Pass0, GpuProgramNames, Options.SM,
        EntryPoints, GpuProgramsData);

    Ogre::GpuProgramParametersSharedPtr VP_Parameters =
        DM_Technique0_Pass0->getVertexProgramParameters();
    Ogre::GpuProgramParametersSharedPtr FP_Parameters =
    ...
    FP_Parameters->setNamedConstant("uDepthLimit",
        1 / mHydrax->getDepthLimit());

    if (cCaustics)
      {
        FP_Parameters->setNamedConstant("uCausticsScale",
            mHydrax->getCausticsScale());
        FP_Parameters->setNamedConstant("uCausticsEnd",
            mHydrax->getCausticsEnd());

        Ogre::TextureUnitState *TUS_Caustics =
            DM_Technique0_Pass0->createTextureUnitState("Caustics.bmp");
        TUS_Caustics->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_WRAP);
  ...
  bool
  MaterialManager::_createUnderwaterMaterial(const HydraxComponent &Components,
      const Options &Options)
  {
    const bool cDepth = _isComponent(Components, HYDRAX_COMPONENT_DEPTH);
    const bool cSmooth = _isComponent(Components, HYDRAX_COMPONENT_SMOOTH);
    ...
    const bool cFoam = _isComponent(Components, HYDRAX_COMPONENT_FOAM);
    const bool cCaustics = _isComponent(Components, HYDRAX_COMPONENT_CAUSTICS);
    const bool cUReflections = _isComponent(Components,
        ...
        case SM_CG:
          {
            ...
            if (cDepth && cUReflections)
              {
                FragmentProgramData +=
                    "uniform sampler2D    uDepthReflectionMap : register(s"
                        + Ogre::StringConverter::toString(TexNum) + "),\n";
                TexNum++;
              }
    ...
    UnderwaterMaterial = Ogre::MaterialManager::getSingleton().create(
        _def_Underwater_Material_Name, HYDRAX_RESOURCE_GROUP);

    Ogre::Pass *UM_Technique0_Pass0 =
        UnderwaterMaterial->getTechnique(0)->getPass(0);
    ...
    Ogre::String GpuProgramsData[2] =
      { VertexProgramData, FragmentProgramData };
    Ogre::String GpuProgramNames[2] =
      { _def_Underwater_Shader_VP_Name, _def_Underwater_Shader_FP_Name };
    Ogre::String EntryPoints[2] =
      { "main_vp", "main_fp" };

    fillGpuProgramsToPass(UM_Technique0_Pass0, GpuProgramNames, Options.SM,
        EntryPoints, GpuProgramsData);

    Ogre::GpuProgramParametersSharedPtr VP_Parameters =
        UM_Technique0_Pass0->getVertexProgramParameters();
    Ogre::GpuProgramParametersSharedPtr FP_Parameters =
        UM_Technique0_Pass0->getFragmentProgramParameters();
    ...
    if (Options.NM == NM_TEXTURE || Options.NM == NM_RTT)
      {
        UM_Technique0_Pass0->createTextureUnitState("HydraxNormalMap")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_WRAP);
      }

    if (cUReflections)
      {
        UM_Technique0_Pass0->createTextureUnitState("HydraxReflectionMap")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_CLAMP);
      }
    UM_Technique0_Pass0->createTextureUnitState("HydraxRefractionMap")->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_CLAMP);

    if (cDepth && cUReflections)
      {
        UM_Technique0_Pass0->createTextureUnitState()->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_CLAMP);
      }

    UM_Technique0_Pass0->createTextureUnitState("Fresnel.bmp")->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_CLAMP);

    if (cFoam)
      {
        UM_Technique0_Pass0->createTextureUnitState("Foam.png")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_WRAP);
      }
  ...
  bool
  MaterialManager::_createUnderwaterCompositor(
      const HydraxComponent &Components, const Options &Options)
  {
    const bool cCaustics = _isComponent(Components, HYDRAX_COMPONENT_CAUSTICS);
    const bool cDepth = _isComponent(Components, HYDRAX_COMPONENT_DEPTH);
    const bool cGodRays = _isComponent(Components,
    ...
    // Build our material
    Ogre::MaterialPtr &UnderwaterCompositorMaterial = getMaterial(
        MAT_UNDERWATER_COMPOSITOR);
    UnderwaterCompositorMaterial = Ogre::MaterialManager::getSingleton().create(
        _def_Underwater_Compositor_Material_Name, HYDRAX_RESOURCE_GROUP);

    Ogre::Pass *DM_Technique0_Pass0 =
        UnderwaterCompositorMaterial->getTechnique(0)->getPass(0);
    ...
    if (cGodRays)
      {
        ...
        FP_Parameters->setNamedConstant("uLightDirection",
            (mHydrax->getMesh()->getObjectSpacePosition(
                mHydrax->getCamera()->getPosition())
                - mHydrax->getMesh()->getObjectSpacePosition(
                    mHydrax->getSunPosition())).normalisedCopy());
        FP_Parameters->setNamedConstant("uIntensity",
            mHydrax->getGodRaysIntensity());
        FP_Parameters->setNamedConstant("uHGg", mHydrax->getGodRaysExposure());
        FP_Parameters->setNamedAutoConstant("uCameraPos",
    ...
    DM_Technique0_Pass0->createTextureUnitState()->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_CLAMP);
    DM_Technique0_Pass0->createTextureUnitState("UnderwaterDistortion.jpg")->setTextureAddressingMode(
        Ogre::TextureUnitState::TAM_WRAP);
    if (cDepth)
      {
        DM_Technique0_Pass0->createTextureUnitState("HydraxDepthMap")->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_CLAMP);
      }
    ...
    UnderwaterCompositor = Ogre::CompositorManager::getSingleton().create(
        _def_Underwater_Compositor_Name, HYDRAX_RESOURCE_GROUP);

    Ogre::CompositionTechnique* UnderWaterComp_Technique =
        UnderwaterCompositor->createTechnique();

    // Create the texture definition to render the original scene
    Ogre::CompositionTechnique::TextureDefinition* TDef =
        UnderWaterComp_Technique->createTextureDefinition("OriginalScene");
    TDef->width = 0;
    TDef->height = 0;
    ...
    Ogre::CompositionTargetPass* CTPass =
        UnderWaterComp_Technique->createTargetPass();
    ...
    Ogre::CompositionTargetPass* CTOutputPass =
        UnderWaterComp_Technique->getOutputTargetPass();
    ...
    Ogre::CompositorManager::getSingleton().addCompositor(
        mHydrax->getViewport(), _def_Underwater_Compositor_Name)->addListener(
  ...
  bool
  MaterialManager::_createSimpleColorMaterial(const Ogre::ColourValue& Color,
      const MaterialType& MT, const Ogre::String& Name, const bool& DepthCheck,
      const bool& DepthWrite)
  {
    Ogre::MaterialPtr &SimpleColorMaterial = getMaterial(MT);
    SimpleColorMaterial = Ogre::MaterialManager::getSingleton().create(Name,
    ...
    SCM_T0_Pass0->createTextureUnitState()->setColourOperationEx(
        Ogre::LBX_MODULATE, Ogre::LBS_MANUAL, Ogre::LBS_CURRENT, Color);
  ...
  void
  MaterialManager::reload(const MaterialType &Material)
  {
    ...
    const bool cUReflections = _isComponent(mComponents,
        HYDRAX_COMPONENT_UNDERWATER_REFLECTIONS);
    ...
    case MAT_WATER:
      {
        ...
        case NM_RTT:
          {
            M_Technique0_Pass0->getTextureUnitState(0)->setTextureName(
                "HydraxNormalMap");
            M_Technique0_Pass0->getTextureUnitState(1)->setTextureName(
                "HydraxReflectionMap");
            M_Technique0_Pass0->getTextureUnitState(2)->setTextureName(
                "HydraxRefractionMap");
            if (cDepth)
              {
                M_Technique0_Pass0->getTextureUnitState(3)->setTextureName(
                    "HydraxDepthMap");
              }
          }
          break;
        ...
        Ogre::GpuProgramParametersSharedPtr VP_Parameters =
            M_Technique0_Pass0->getVertexProgramParameters();
        Ogre::GpuProgramParametersSharedPtr FP_Parameters =
            M_Technique0_Pass0->getFragmentProgramParameters();
        ...
        if (cSun)
          {
            ...
            FP_Parameters->setNamedConstant("uSunStrength",
                mHydrax->getSunStrength());
            FP_Parameters->setNamedConstant("uSunArea", mHydrax->getSunArea());
            FP_Parameters->setNamedConstant("uSunColor",
                mHydrax->getSunColor());
          }
        if (cFoam)
          {
            ...
            FP_Parameters->setNamedConstant("uFoamMaxDistance",
                mHydrax->getFoamMaxDistance());
            FP_Parameters->setNamedConstant("uFoamScale",
                mHydrax->getFoamScale());
            FP_Parameters->setNamedConstant("uFoamStart",
                mHydrax->getFoamStart());
            FP_Parameters->setNamedConstant("uFoamTransparency",
                mHydrax->getFoamTransparency());
          }
        if (cCaustics)
        ...
        case NM_RTT:
          {
            ...
            if (cUReflections)
              {
                M_Technique0_Pass0->getTextureUnitState(Index)->setTextureName(
                    "HydraxReflectionMap");
                Index++;
              }
            M_Technique0_Pass0->getTextureUnitState(Index)->setTextureName(
    ...
    if (Ogre::Pass::getDirtyHashList().size() != 0
        || Ogre::Pass::getPassGraveyard().size() != 0)
      {
        Ogre::SceneManagerEnumerator::SceneManagerIterator scenesIter =
  ...
  void
  MaterialManager::addDepthTechnique(Ogre::Technique *Technique,
      const bool& AutoUpdate)
  {
    ...
    Ogre::GpuProgramParametersSharedPtr VP_Parameters =
        DM_Technique_Pass0->getVertexProgramParameters();
    Ogre::GpuProgramParametersSharedPtr FP_Parameters =
        DM_Technique_Pass0->getFragmentProgramParameters();
    ...
    if (_isComponent(mComponents, HYDRAX_COMPONENT_CAUSTICS))
      {
        ...
        Ogre::TextureUnitState *TUS_Caustics =
            DM_Technique_Pass0->createTextureUnitState("Caustics.bmp");
        TUS_Caustics->setTextureAddressingMode(
            Ogre::TextureUnitState::TAM_WRAP);
        TUS_Caustics->setAnimatedTextureName("Caustics.bmp", 32, 1.5);
      }
  ...
  void
  MaterialManager::setCompositorEnable(const CompositorType &Compositor,
      const bool &Enable)
  {
    ...
    Ogre::CompositorManager::getSingleton().setCompositorEnabled(
        mHydrax->getViewport(), Comp->getName(), Enable);

    mCompositorsEnable[static_cast<int>(Compositor)] = Enable;
  }

  bool
  MaterialManager::_isComponent(const HydraxComponent &List,
      const HydraxComponent &ToCheck) const
  {
    if (List & ToCheck)
      {
        return true;
  ...
  void
  MaterialManager::setGpuProgramParameter(const GpuProgram &GpuP,
      const MaterialType &MType, const Ogre::String &Name,
    ...
    if (MType == MAT_DEPTH)
      {
        ...
        for (TechIt = mDepthTechniques.begin();
            TechIt != mDepthTechniques.end(); TechIt++)
          {
            if (!(*TechIt))
              {
                mDepthTechniques.erase(TechIt);
            ...
            case GPUP_VERTEX:
              {
                Parameters =
                    (*TechIt)->getPass(0)->getVertexProgramParameters();
              }
              break;
            ...
            case GPUP_FRAGMENT:
              {
                Parameters =
                    (*TechIt)->getPass(0)->getFragmentProgramParameters();
              }
              break;
              }
  ...
  void
  MaterialManager::UnderwaterCompositorListener::notifyMaterialSetup(
      Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
  {
  }

  void
  MaterialManager::UnderwaterCompositorListener::notifyMaterialRender(
      Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
  {
    const Ogre::Vector3& WC = mMaterialManager->mHydrax->getWaterColor();
    Ogre::CompositorPtr &UnderwaterCompositor = mMaterialManager->getCompositor(
        COMP_UNDERWATER);
    UnderwaterCompositor->getTechnique(0)->getTargetPass(0)->getPass(0)->setClearColour(
        Ogre::ColourValue(WC.x, WC.y, WC.z));
    ...
    if (mMaterialManager->mHydrax->isComponent(HYDRAX_COMPONENT_DEPTH))
      {
        FP_Parameters->setNamedConstant("uWaterColor",
            mMaterialManager->mHydrax->getWaterColor());
      }

    if (mMaterialManager->mHydrax->isComponent(HYDRAX_COMPONENT_CAUSTICS))
      {
        FP_Parameters->setNamedConstant("uCausticsPower",
            mMaterialManager->mHydrax->getCausticsPower());
      }

    if (mMaterialManager->mHydrax->isComponent(
        HYDRAX_COMPONENT_UNDERWATER_GODRAYS))
      {
        FP_Parameters->setNamedConstant("uSunColor",
            mMaterialManager->mHydrax->getSunColor());

        FP_Parameters->setNamedConstant("uLightDirection",
            (mMaterialManager->mHydrax->getMesh()->getObjectSpacePosition(
                mMaterialManager->mHydrax->getCamera()->getPosition())
                - mMaterialManager->mHydrax->getMesh()->getObjectSpacePosition(
                    mMaterialManager->mHydrax->getSunPosition())).normalisedCopy());

        FP_Parameters->setNamedConstant("uIntensity",
            mMaterialManager->mHydrax->getGodRaysIntensity());
        FP_Parameters->setNamedConstant("uHGg",
            mMaterialManager->mHydrax->getGodRaysExposure());

        Ogre::GpuProgramParametersSharedPtr VP_Parameters =
            mat->getTechnique(0)->getPass(0)->getVertexProgramParameters();

        // FAR_LEFT_TOP
        VP_Parameters->setNamedConstant("uCorner0",
            mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[5]);
        // FAR_RIGHT_TOP - FAR_LEFT_TOP
        VP_Parameters->setNamedConstant("uCorner01",
            mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[4]
                - mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[5]);
        // FAR_LEFT_BOTTOM - FAR_LEFT_TOP
        VP_Parameters->setNamedConstant("uCorner02",
    ...
    if (mMaterialManager->mCompositorsNeedToBeReloaded[COMP_UNDERWATER])
      {
        ...
        if (mMaterialManager->_isComponent(mMaterialManager->mComponents,
            HYDRAX_COMPONENT_DEPTH))
          {
            DM_Technique0_Pass0->getTextureUnitState(2)->setTextureName(
                "HydraxDepthMap");
          }
