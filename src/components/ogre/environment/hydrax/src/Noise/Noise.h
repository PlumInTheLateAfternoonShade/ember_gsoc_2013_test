...
namespace Hydrax
{
  namespace Noise
  {
    ...
    public:
       ...
       */
      virtual bool
      createGPUNormalMapResources(GPUNormalMapManager *g);

      /** Remove GPUNormalMap resources
       @param g GPUNormalMapManager pointer
       */
      virtual void
      removeGPUNormalMapResources(GPUNormalMapManager *g);

      /** Call it each frame
       @param timeSinceLastFrame Time since last frame(delta)
       */
      virtual void
      update(const Ogre::Real &timeSinceLastFrame) = 0;

      /** Save config
       @param Data String reference 
       */
      virtual void
      saveCfg(Ogre::String &Data);
       ...
       */
      virtual bool
      loadCfg(const Ogre::ConfigFile &CfgFile);
       ...
       */
      inline const bool&
      areGPUNormalMapResourcesCreated() const
      {
       ...
       */
      virtual float
      getValue(const float &x, const float &y) = 0;
