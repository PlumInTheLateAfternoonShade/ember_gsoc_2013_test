...
namespace Hydrax
{
  Decal::Decal(Hydrax *h, const Ogre::String &TextureName, const int& Id) :
      mHydrax(h), mTextureName(TextureName), mId(Id), mRegisteredPass(0), mPosition(
          Ogre::Vector2(0, 0)), mSize(Ogre::Vector2(1, 1)), mOrientation(
          Ogre::Radian(0)), mTransparency(1), mVisible(true)
  {
  ...
  void
  Decal::registerPass(Ogre::Pass* _Pass)
  {
    unregister();
    ...
    Ogre::TextureUnitState *DecalTexture = _Pass->createTextureUnitState(
        mTextureName);
    DecalTexture->setProjectiveTexturing(true, mProjector);
    DecalTexture->setTextureAddressingMode(Ogre::TextureUnitState::TAM_CLAMP);
    DecalTexture->setTextureFiltering(Ogre::FO_LINEAR, Ogre::FO_LINEAR,
        Ogre::FO_NONE);
    DecalTexture->setAlphaOperation(Ogre::LBX_MODULATE, Ogre::LBS_TEXTURE,
        Ogre::LBS_MANUAL, 1.0, mTransparency);
  ...
  void
  Decal::setPosition(const Ogre::Vector2& Position)
  {
    mPosition = Position;
  ...
  void
  Decal::setSize(const Ogre::Vector2& Size)
  {
    mSize = Size;
  ...
  void
  Decal::setOrientation(const Ogre::Radian& Orientation)
  {
  ...
  void
  Decal::setTransparency(const Ogre::Real& Transparency)
  {
    mTransparency = Transparency;

    if (mRegisteredPass)
      {
        mRegisteredPass->getTextureUnitState(0)->setAlphaOperation(
            Ogre::LBX_MODULATE, Ogre::LBS_TEXTURE, Ogre::LBS_MANUAL, 1.0,
            mTransparency);
      }
  }

  void
  Decal::setVisible(const bool& Visible)
  {
    mVisible = Visible;
  ...
  DecalsManager::DecalsManager(Hydrax *h) :
      mHydrax(h), mNextId(0), mLastUnderwater(false), mWaterStrength(5)
  {
  }
                ...
                if (!mHydrax->_isCurrentFrameUnderwater())
                  {
                    (*DecalIt)->registerPass(
                        mHydrax->getMaterialManager()->getMaterial(
                            MaterialManager::MAT_WATER)->getTechnique(0)->createPass());
                  }
                else
                  {
                    (*DecalIt)->registerPass(
                        mHydrax->getMaterialManager()->getMaterial(
                            MaterialManager::MAT_UNDERWATER)->getTechnique(0)->createPass());
                  }
              }
    ...
    for (DecalIt = mDecals.begin(); DecalIt != mDecals.end(); DecalIt++)
      {
        ...
        DecalBox = Ogre::AxisAlignedBox(DPos.x - DSize.x,
            HHeight - mWaterStrength, DPos.y - DSize.y, DPos.x + DSize.x,
  ...
  Decal*
  DecalsManager::add(const Ogre::String& TextureName)
  {
  ...
  Decal*
  DecalsManager::get(const int& Id)
  {
    for (DecalIt = mDecals.begin(); DecalIt != mDecals.end(); DecalIt++)
      {
        if ((*DecalIt)->getId() == Id)
  ...
  void
  DecalsManager::remove(const int& Id)
  {
    for (DecalIt = mDecals.begin(); DecalIt != mDecals.end(); DecalIt++)
      {
        if ((*DecalIt)->getId() == Id)
