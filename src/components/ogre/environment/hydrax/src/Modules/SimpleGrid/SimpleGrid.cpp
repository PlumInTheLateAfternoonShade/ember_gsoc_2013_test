...
namespace Hydrax
{
  namespace Module
  {
    ...
    Ogre::String
    _SG_getNormalModeString(const MaterialManager::NormalMode& NormalMode)
    {
      if (NormalMode == MaterialManager::NM_VERTEX)
        {
          return "Vertex";
    ...
    SimpleGrid::SimpleGrid(Hydrax *h, Noise::Noise *n,
        ...
        Module("SimpleGrid" + _SG_getNormalModeString(NormalMode), n,
            Mesh::Options(256, Size(mOptions.MeshSize),
                _SG_getVertexTypeFromNormalMode(NormalMode)), NormalMode), mHydrax(
            h), mVertices(0)
    {
    }
    ...
    SimpleGrid::SimpleGrid(Hydrax *h, Noise::Noise *n,
        ...
        Module("SimpleGrid" + _SG_getNormalModeString(NormalMode), n,
            Mesh::Options(Options.Complexity, Size(Options.MeshSize),
                _SG_getVertexTypeFromNormalMode(NormalMode)), NormalMode), mHydrax(
            h), mVertices(0)
    {
      setOptions(Options);
    ...
    void
    SimpleGrid::setOptions(const Options &Options)
    {
      ...
      if (isCreated())
        {
          if (Options.Complexity != mOptions.Complexity)
            {
              ...
              if (mNormalMode == MaterialManager::NM_RTT)
                {
                  if (!mNoise->createGPUNormalMapResources(
                      mHydrax->getGPUNormalMapManager()))
                    {
                      HydraxLOG(
                          mNoise->getName()
              ...
              Ogre::String MaterialNameTmp =
                  mHydrax->getMesh()->getMaterialName();
          ...
          if (getNormalMode() == MaterialManager::NM_VERTEX)
            {
              Mesh::POS_NORM_VERTEX* Vertices =
                  static_cast<Mesh::POS_NORM_VERTEX*>(mVertices);
                  ...
                  for (u = 0; u < mOptions.Complexity; u++)
                    {
                      Vertices[v * mOptions.Complexity + u].x =
                          (static_cast<float>(v) / (mOptions.Complexity - 1))
                              * mOptions.MeshSize.Width;
                      Vertices[v * mOptions.Complexity + u].z =
                          (static_cast<float>(u) / (mOptions.Complexity - 1))
                              * mOptions.MeshSize.Height;
                    }
                }
            }
          else if (getNormalMode() == MaterialManager::NM_RTT)
            {
              Mesh::POS_VERTEX* Vertices =
                  static_cast<Mesh::POS_VERTEX*>(mVertices);
    ...
    SimpleGrid::create()
    {
      ...
      if (getNormalMode() == MaterialManager::NM_VERTEX)
        {
          mVertices = new Mesh::POS_NORM_VERTEX[mOptions.Complexity
              * mOptions.Complexity];
          Mesh::POS_NORM_VERTEX* Vertices =
              static_cast<Mesh::POS_NORM_VERTEX*>(mVertices);
        ...
        {
          mVertices = new Mesh::POS_VERTEX[mOptions.Complexity
              * mOptions.Complexity];
    ...
    void
    SimpleGrid::saveCfg(Ogre::String &Data)
    {
      Module::saveCfg(Data);

      Data += CfgFileManager::_getCfgString("SG_Complexity",
          mOptions.Complexity);
      ...
      Data += CfgFileManager::_getCfgString("SG_ChoppyWaves",
          mOptions.ChoppyWaves);
      Data += CfgFileManager::_getCfgString("SG_ChoppyStrength",
          mOptions.ChoppyStrength);
      Data += "\n";
    }

    bool
    SimpleGrid::loadCfg(const Ogre::ConfigFile &CfgFile)
    {
    ...
    void
    SimpleGrid::update(const Ogre::Real &timeSinceLastFrame)
    {
      if (!isCreated())
        {
          return;
      ...
      if (getNormalMode() == MaterialManager::NM_VERTEX)
        {
          ...
          for (v = 0; v < mOptions.Complexity; v++)
            {
              v_ = (static_cast<float>(v) / (mOptions.Complexity - 1))
                  * mOptions.MeshSize.Width;

              for (u = 0; u < mOptions.Complexity; u++)
                {
                  Vertices[i].y = mNoise->getValue(v_,
                      (static_cast<float>(u) / (mOptions.Complexity - 1))
                          * mOptions.MeshSize.Height) * mOptions.Strength;
                  i++;
                }
            }
        }
      else if (getNormalMode() == MaterialManager::NM_RTT)
        {
          ...
          mHydrax->getMesh()->getEntity()->getParentSceneNode()->getWorldTransforms(
              &mWorldMatrix);
              ...
              for (u = 0; u < mOptions.Complexity; u++)
                {
                  ...
                  Vertices[i].y = mNoise->getValue(p.x, p.z)
                      * mOptions.Strength;
                  i++;
                }
            }
              ...
              for (v = 1; v < (mOptions.Complexity - 1); v++)
                {
                  for (u = 1; u < (mOptions.Complexity - 1); u++)
                    {
                      Vertices[v * mOptions.Complexity + u].y = 0.2f
      ...
      mHydrax->getMesh()->updateGeometry(
          mOptions.Complexity * mOptions.Complexity, mVertices);
    }

    void
    SimpleGrid::_calculeNormals()
    {
      if (getNormalMode() != MaterialManager::NM_VERTEX)
        {
          return;
      ...
      for (v = 1; v < (mOptions.Complexity - 1); v++)
        {
          for (u = 1; u < (mOptions.Complexity - 1); u++)
            {
              vec1 = Ogre::Vector3(
                  Vertices[v * mOptions.Complexity + u + 1].x
                      - Vertices[v * mOptions.Complexity + u - 1].x,
                  Vertices[v * mOptions.Complexity + u + 1].y
                      - Vertices[v * mOptions.Complexity + u - 1].y,
                  Vertices[v * mOptions.Complexity + u + 1].z
                      - Vertices[v * mOptions.Complexity + u - 1].z);

              vec2 = Ogre::Vector3(
                  Vertices[(v + 1) * mOptions.Complexity + u].x
                      - Vertices[(v - 1) * mOptions.Complexity + u].x,
                  Vertices[(v + 1) * mOptions.Complexity + u].y
                      - Vertices[(v - 1) * mOptions.Complexity + u].y,
                  Vertices[(v + 1) * mOptions.Complexity + u].z
                      - Vertices[(v - 1) * mOptions.Complexity + u].z);
    ...
    void
    SimpleGrid::_performChoppyWaves()
    {
      if (getNormalMode() != MaterialManager::NM_VERTEX
          || !mOptions.ChoppyWaves)
        {
      ...
      for (v = 1; v < (mOptions.Complexity - 1); v++)
        {
          for (u = 1; u < (mOptions.Complexity - 1); u++)
            {
              Vertices[v * mOptions.Complexity + u].x += Vertices[v
                  * mOptions.Complexity + u].nx * mOptions.ChoppyStrength
                  * Underwater;
              Vertices[v * mOptions.Complexity + u].z += Vertices[v
                  * mOptions.Complexity + u].nz * mOptions.ChoppyStrength
                  * Underwater;
            }
        }
    }
    ...
    float
    SimpleGrid::getHeigth(const Ogre::Vector2 &Position)
    {
      if (getNormalMode() != MaterialManager::NM_RTT)
        {
          Ogre::Vector2 RelativePos = mHydrax->getMesh()->getGridPosition(
              Position);

          RelativePos.x *= mOptions.MeshSize.Width;
          RelativePos.y *= mOptions.MeshSize.Height;

          return mHydrax->getPosition().y
              + mNoise->getValue(RelativePos.x, RelativePos.y)
                  * mOptions.Strength;
        }
      else // RTT Normals calculations works with world-space coords
        {
          return mHydrax->getPosition().y
              + mNoise->getValue(Position.x, Position.y) * mOptions.Strength;
        }
    }
  }
}