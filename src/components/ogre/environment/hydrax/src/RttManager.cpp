...
namespace Hydrax
{
      ...
      mHydrax(h), mPlanesSceneNode(0), mReflectionDisplacementError(0.5f)
  {
    Ogre::String RttNames[6] =
      { _def_Hydrax_Reflection_Rtt_Name, _def_Hydrax_Refraction_Rtt_Name,
  ...
  void
  RttManager::initialize(const RttType& Rtt)
  {
    if (!mPlanesSceneNode)
      {
        mPlanesSceneNode =
            mHydrax->getSceneManager()->getRootSceneNode()->createChildSceneNode();
        mHydrax->getSceneManager()->addRenderQueueListener(
            &mReflectionListener.mCReflectionQueueListener);
      }
    ...
    case RTT_REFLECTION:
      {
        ...
        _initializeRtt(Rtt, Ogre::Vector3::UNIT_Y,
            Ogre::ColourValue(WC.x, WC.y, WC.z), RenderSkyBox,
            &mReflectionListener);
      }
      break;
    ...
    case RTT_REFRACTION:
      {
        ...
        _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y,
            Ogre::ColourValue(WC.x, WC.y, WC.z), RenderSkyBox,
            &mRefractionListener);
      }
      break;
    ...
    case RTT_DEPTH:
      {
        _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y,
            Ogre::ColourValue::Black, false, &mDepthListener, "HydraxDepth",
            false);
      }
      break;
    ...
    case RTT_DEPTH_REFLECTION:
      {
        _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y,
            Ogre::ColourValue::Black, false, &mDepthReflectionListener,
            "HydraxDepth", false);
      }
      break;
    ...
    case RTT_GPU_NORMAL_MAP:
      {
        _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y,
            Ogre::ColourValue(0.5, 1, 0.5), false, &mGPUNormalMapListener, "",
            false);
      }
      break;
      }
  ...
  void
  RttManager::remove(const RttType& Rtt)
  {
    ...
    if (!Tex.isNull())
      {
        ...
        Ogre::MeshManager::getSingleton().remove(
            mRttOptions[Rtt].Name + "ClipPlane");
    ...
    if (mPlanesSceneNode)
      {
        ...
        mPlanesSceneNode->getParentSceneNode()->removeAndDestroyChild(
            mPlanesSceneNode->getName());
        mPlanesSceneNode = 0;
      }
  }

  void
  RttManager::_initializeRtt(const RttType& Rtt,
      ...
      const Ogre::String& MaterialScheme, const bool& ShadowsEnabled)
  {
    remove(Rtt);
    ...
    Ogre::MeshManager::getSingleton().createPlane(
        mRttOptions[Rtt].Name + "ClipPlane", HYDRAX_RESOURCE_GROUP,
        *mPlanes[Rtt], mHydrax->getMesh()->getSize().Width,
        mHydrax->getMesh()->getSize().Height, // PG grid.. <-- look into
    ...
    mTextures[Rtt] = Ogre::TextureManager::getSingleton().createManual(
        mRttOptions[Rtt].Name, HYDRAX_RESOURCE_GROUP, Ogre::TEX_TYPE_2D,
        TSize.Width, TSize.Height, 0, getPixelFormat(Rtt),
        Ogre::TU_RENDERTARGET);

    Ogre::RenderTarget* RT_Texture =
        mTextures[Rtt]->getBuffer()->getRenderTarget();

    Ogre::Viewport *RT_Texture_Viewport = RT_Texture->addViewport(
        mHydrax->getCamera());
  ...
  void
  RttManager::setTextureSize(const RttType& Rtt, const Size& S)
  {
    ...
    if (!getTexture(Rtt).isNull())
      {
        ...
        if (mHydrax->isComponent(HYDRAX_COMPONENT_UNDERWATER))
          {
            mHydrax->getMaterialManager()->reload(
                MaterialManager::MAT_UNDERWATER);

            if (mHydrax->_isCurrentFrameUnderwater())
              {
                mHydrax->getMaterialManager()->reload(
                    MaterialManager::MAT_UNDERWATER_COMPOSITOR);
              }
          }
      }
  }
  ...
  void
  RttManager::setTexturesSize(const Size& S)
  {
  ...
  const Ogre::PixelFormat
  RttManager::getPixelFormat(const RttType& Rtt) const
  {
  ...
  void
  RttManager::CReflectionListener::preRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    ...
    if (mHydrax->_isCurrentFrameUnderwater())
      {
        mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->translate(0,
            -mHydrax->getPlanesError(), 0);
      }
    else
      {
        mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->translate(0,
    ...
    // Underwater
    if (mHydrax->_isCurrentFrameUnderwater()
        && (mHydrax->getCamera()->getPosition().y
    ...
    mHydrax->getCamera()->enableReflection(
        mRttManager->mPlanes[RTT_REFLECTION]);
    ...
    if (IsInUnderwaterError)
      ...
      {
        mHydrax->getCamera()->enableCustomNearClipPlane(
            mRttManager->mPlanes[RTT_REFLECTION]);
      }
  }

  void
  RttManager::CReflectionListener::postRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
  ...
  void
  RttManager::CReflectionListener::CReflectionQueueListener::renderQueueStarted(
      Ogre::uint8 queueGroupId, const Ogre::String &invocation,
      bool &skipThisInvocation)
  {
    if ((queueGroupId == Ogre::RENDER_QUEUE_SKIES_EARLY
        || queueGroupId == Ogre::RENDER_QUEUE_SKIES_LATE) && mActive)
      {
  ...
  void
  RttManager::CReflectionListener::CReflectionQueueListener::renderQueueEnded(
      Ogre::uint8 queueGroupId, const Ogre::String &invocation,
      bool &skipThisInvocation)
  {
    if ((queueGroupId == Ogre::RENDER_QUEUE_SKIES_EARLY
        || queueGroupId == Ogre::RENDER_QUEUE_SKIES_LATE) && mActive)
      {
        mRttManager->mHydrax->getCamera()->enableCustomNearClipPlane(
            mRttManager->mPlanes[RTT_REFLECTION]);
        Ogre::Root::getSingleton().getRenderSystem()->_setProjectionMatrix(
            mRttManager->mHydrax->getCamera()->getProjectionMatrixRS());
      }
  ...
  void
  RttManager::CRefractionListener::preRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
    ...
    if (Ogre::Math::Abs(
        mHydrax->getPosition().y - mHydrax->getCamera()->getPosition().y)
        > mHydrax->getPlanesError())
      {
        if (mHydrax->_isCurrentFrameUnderwater())
          {
            mRttManager->mPlanes[RTT_REFRACTION]->normal =
                -mRttManager->mPlanes[RTT_REFRACTION]->normal;
            mRttManager->mPlanes[RTT_REFRACTION]->getParentNode()->translate(0,
                -mHydrax->getPlanesError(), 0);
          }
        else
          {
        ...
        mHydrax->getCamera()->enableCustomNearClipPlane(
            mRttManager->mPlanes[RTT_REFRACTION]);
      }
  }

  void
  RttManager::CRefractionListener::postRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
  ...
  void
  RttManager::CDepthListener::preRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
    ...
    while (EntityIterator.hasMoreElements())
      {
        CurrentEntity =
            static_cast<Ogre::Entity*>(EntityIterator.peekNextValue());
        ...
        for (k = 0; k < CurrentEntity->getNumSubEntities(); k++)
          {
            ...
            CurrentEntity->getSubEntity(k)->setMaterialName(
                mHydrax->getMaterialManager()->getMaterial(
                    MaterialManager::MAT_DEPTH)->getName());
          }
    ...
    if (Ogre::Math::Abs(
        ...
        > mHydrax->getPlanesError())
      {
        if (mHydrax->_isCurrentFrameUnderwater())
          {
            mRttManager->mPlanes[RTT_DEPTH]->normal =
                -mRttManager->mPlanes[RTT_DEPTH]->normal;
            mRttManager->mPlanes[RTT_DEPTH]->getParentNode()->translate(0,
                -mHydrax->getPlanesError(), 0);
          }
        else
          {
    ...
    if (mHydrax->_isCurrentFrameUnderwater())
      {
        ...
        mHydrax->getMesh()->getEntity()->setMaterialName(
            mHydrax->getMaterialManager()->getMaterial(
                MaterialManager::MAT_SIMPLE_RED)->getName());
        mHydrax->getMesh()->getEntity()->setRenderQueueGroup(
  ...
  void
  RttManager::CDepthListener::postRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
  ...
  void
  RttManager::CDepthReflectionListener::preRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
    ...
    mRttManager->mPlanes[RTT_DEPTH_REFLECTION]->getParentNode()->translate(0,
        -mHydrax->getPlanesError(), 0);

    bool IsInUnderwaterError = false;

    if (mHydrax->getCamera()->getPosition().y
    ...
    mHydrax->getCamera()->enableReflection(
        mRttManager->mPlanes[RTT_DEPTH_REFLECTION]);
  ...
  void
  RttManager::CDepthReflectionListener::postRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
    Hydrax *mHydrax = mRttManager->mHydrax;
    ...
    if (mCameraPlaneDiff != 0)
      {
        mRttManager->mPlanes[RTT_DEPTH_REFLECTION]->getParentNode()->translate(
            0, mCameraPlaneDiff, 0);
      }
  ...
  void
  RttManager::CGPUNormalMapListener::preRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
  ...
  void
  RttManager::CGPUNormalMapListener::postRenderTargetUpdate(
      const Ogre::RenderTargetEvent& evt)
  {
