  ...
  namespace OgreView
  {
    ...
    namespace Environment
    {
      ...
      public:
         ...
         */
        WaterAdjustRenderTargetListener(Ogre::SceneNode* waterNode,
            float xTextureSize, float zTextureSize) :
            mWaterNode(waterNode), mXTextureSize(xTextureSize), mZTextureSize(
                zTextureSize)
        {
        ...
        void
        preViewportUpdate(const Ogre::RenderTargetViewportEvent & evt)
        {
          const Ogre::Vector3& cameraPos =
              evt.source->getCamera()->getDerivedPosition();
          mWaterNode->setPosition(
              cameraPos.x - std::fmod(cameraPos.x, mXTextureSize),
              mWaterNode->getPosition().y,
              cameraPos.z - std::fmod(cameraPos.z, mZTextureSize));

        }

      };

      SimpleWater::SimpleWater(Ogre::Camera& camera,
          Ogre::SceneManager& sceneMgr, Ogre::RenderTarget& mainRenderTarget) :
          mCamera(camera), mSceneMgr(sceneMgr), mWaterNode(0), mWaterBobbingNode(
              0), mWaterEntity(0), mMainRenderTarget(mainRenderTarget), mRenderTargetListener(
              0), mWaterBobbingController(0)
      {
      }
...
//		}
        ...
        if (mWaterBobbingController)
          {
            Ogre::ControllerManager::getSingleton().destroyController(
                mWaterBobbingController);
          }
        delete mRenderTargetListener;
      }

      bool
      SimpleWater::isSupported() const
      {
        ...
        try
          {
            ...
            Ogre::MeshManager::getSingleton().createPlane("SimpleWaterPlane",
            ...
            mRenderTargetListener = new WaterAdjustRenderTargetListener(
                mWaterNode, textureSize, textureSize);
            mMainRenderTarget.addListener(mRenderTargetListener);

            Ogre::ControllerFunctionRealPtr func(
                OGRE_NEW Ogre::WaveformControllerFunction(Ogre::WFT_SINE, 0,
                    0.1));
            Ogre::ControllerValueRealPtr dest(
                OGRE_NEW NodeAnimator(*mWaterBobbingNode,
                    Ogre::Vector3(0, 1, 0)));
            Ogre::ControllerManager& cm =
                Ogre::ControllerManager::getSingleton();
            mWaterBobbingController = cm.createController(
                cm.getFrameTimeSource(), dest, func);
      ...
      ICollisionDetector*
      SimpleWater::createCollisionDetector()
      {
        return new SimpleWaterCollisionDetector(*this);
      }

      bool
      SimpleWater::setUserAny(const Ogre::Any &anything)
      {
        if (mWaterEntity)
          {
      ...
      void
      SimpleWater::setLevel(float height)
      {
        if (mWaterNode)
          {
