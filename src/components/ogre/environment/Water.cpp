  ...
  namespace OgreView
  {
    ...
    namespace Environment
    {
      ...
      public:
        ...
        void
        setPlaneEntity(Entity* plane)
        {
          pPlaneEnt = plane;
        }

        void
        preRenderTargetUpdate(const RenderTargetEvent& evt)
        {
        ...
        void
        postRenderTargetUpdate(const RenderTargetEvent& evt)
        {
      ...
      public:
        ...
        void
        setReflectionPlane(Plane &aPlane)
        {
          reflectionPlane = aPlane;
        }

        void
        setCamera(Ogre::Camera* aCamera)
        {
          theCam = aCamera;
        }
      ...
      Water::Water(Ogre::Camera &camera, Ogre::SceneManager& sceneMgr) :
          mCamera(camera), mSceneMgr(sceneMgr), mRefractionListener(0), mReflectionListener(
              0), mWaterNode(0), mWaterEntity(0)
      {
      }

      bool
      Water::isSupported() const
      {
        ...
        try
          {
            ...
            Ogre::MeshManager::getSingleton().createPlane("WaterPlane",
                "environment", waterPlane, 10000, 10000, 5, 5, true, 1, 1000,
                1000, Ogre::Vector3::UNIT_Z);
            ...
            Ogre::TexturePtr texture =
                TextureManager::getSingleton().createManual("Refraction",
                    "General", TEX_TYPE_2D, 512, 512, 0, PF_A8R8G8B8,
                    TU_RENDERTARGET);
            ...
            /*		RenderTexture* rttTex = EmberOgre::getSingleton().getOgreRoot()->getRenderSystem()->createRenderTexture( "Refraction", 512, 512 );*/

              {
                ...
                if (!mat.isNull())
                  {
                    mat->getTechnique(0)->getPass(0)->getTextureUnitState(2)->setTextureName(
                        "Refraction");
            ...
            texture = TextureManager::getSingleton().createManual("Reflection",
                "General", TEX_TYPE_2D, 512, 512, 0, PF_A8R8G8B8,
                TU_RENDERTARGET);
            rttTex = texture->getBuffer()->getRenderTarget();
              {
                ...
                if (!mat.isNull())
                  {
                    mat->getTechnique(0)->getPass(0)->getTextureUnitState(1)->setTextureName(
                        "Reflection");
                    v->setOverlaysEnabled(false);
