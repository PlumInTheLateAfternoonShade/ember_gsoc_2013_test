  ...
  namespace OgreView
  {
        ...
        mSceneManager(0), mMainCamera(0)
    {
      mSceneManager = Ogre::Root::getSingleton().createSceneManager(
          Ogre::ST_EXTERIOR_REAL_FAR, "EmberPagingSceneManagerInstance");
    ...
    Scene::~Scene()
    {
      if (mTechniques.size() > 0)
        {
          S_LOG_WARNING(
              "Scene was deleted while there still was registered techniques.");
        }
    ...
    void
    Scene::registerEntityWithTechnique(EmberEntity& entity,
    ...
    void
    Scene::deregisterEntityWithTechnique(EmberEntity& entity,
        const std::string& technique)
    {
      RenderingTechniqueStore::const_iterator I = mTechniques.find(technique);
      if (I != mTechniques.end())
    ...
    void
    Scene::addRenderingTechnique(const std::string& name,
        ISceneRenderingTechnique* technique)
    {
      if (mTechniques.count(name) == 0)
        {
          mTechniques.insert(
              RenderingTechniqueStore::value_type(name, technique));
        }
    }

    ISceneRenderingTechnique*
    Scene::removeRenderingTechnique(const std::string& name)
    {
    ...
    void
    Scene::registerPageDataProvider(IPageDataProvider* pageDataProvider)
    {
      static_cast<EmberPagingSceneManager*>(mSceneManager)->registerProvider(
          pageDataProvider);
    }

    Terrain::ISceneManagerAdapter*
    Scene::createAdapter()
    {
      return new EmberPagingSceneManagerAdapter(
          static_cast<EmberPagingSceneManager&>(*mSceneManager));
    }
