...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Mapping
    {
      ...
      void
      XMLEntityMappingDefinitionSerializer::parseScript(
        ...
        if (rootElem)
          {
            ...
            for (TiXmlElement* smElem = rootElem->FirstChildElement(
                "entitymapping"); smElem != 0;
                smElem = smElem->NextSiblingElement("entitymapping"))
              {
                    ...
                    try
                      {
                        ...
                        TiXmlElement* matchElement =
                            smElem->FirstChildElement();
                        parseMatchElement(*definition, definition->getRoot(),
                            matchElement);
                      ...
                      {
                        S_LOG_FAILURE(
                            "Error when reading model mapping with name '" << tmp << "'." << ex);
                        delete definition;
                      }
                    catch (...)
                      {
                        S_LOG_FAILURE(
                            "Error when reading model mapping with name '" << tmp << "'.");
                        delete definition;
                      }
                  }
                    ...
                    try
                      {
                        ...
                        caseDef.getCaseParameters().push_back(
                            CaseDefinition::ParameterEntry("equals", name));
            ...
            for (TiXmlElement* smElem = rootElem->FirstChildElement(
                "autoentitymapping"); smElem != 0;
                smElem = smElem->NextSiblingElement("autoentitymapping"))
              {
                const char* tmp = smElem->Attribute("name");
                if (!tmp)
                  {
                    ...
                    try
                      {
                        ...
                        const char* tmpModelName = smElem->Attribute(
                            "modelname");
                        if (tmpModelName)
                          {
      ...
      void
      XMLEntityMappingDefinitionSerializer::parseMatchElement(
          EntityMappingDefinition& definition, MatchDefinition& matchDef,
      ...
      void
      XMLEntityMappingDefinitionSerializer::parseCaseElement(
          EntityMappingDefinition& definition, CaseDefinition& caseDef,
          TiXmlElement* element)
      {
        for (TiXmlAttribute* attribute = element->FirstAttribute();
            attribute != 0; attribute = attribute->Next())
        ...
        if (!element->NoChildren())
          {
            for (TiXmlElement* childElement = element->FirstChildElement();
                ...
                childElement = childElement->NextSiblingElement())
              {
                ...
                else if (std::string(childElement->Value())
                    == std::string("caseparam"))
                  {
                    //it's a case parameter
                    if (const char* attributeValue = childElement->Attribute(
                        "type"))
                      {
      ...
      void
      XMLEntityMappingDefinitionSerializer::parseActionElement(
          EntityMappingDefinition& definition, ActionDefinition& actionDef,
          TiXmlElement* element)
      {
        for (TiXmlAttribute* attribute = element->FirstAttribute();
            attribute != 0; attribute = attribute->Next())
