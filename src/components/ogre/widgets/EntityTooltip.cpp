...
namespace Ember
{
  namespace OgreView
  {
    namespace Gui
    {
      ...
      EmberEntityTooltipWidget::EmberEntityTooltipWidget(
      ...
      CEGUI::Size
      EmberEntityTooltipWidget::getTextSize_impl() const
      {
        return CEGUI::Size(180, 80);
      }

      const CEGUI::String EmberEntityTooltipWidget::WidgetTypeName(
          "Ember/EntityTooltip");
      ...
      EntityTooltip::registerFactory()
      {
        CEGUI::WindowFactoryManager::addFactory<
            CEGUI::TplWindowFactory<EmberEntityTooltipWidget>>();
      }

      EntityTooltip::EntityTooltip(World& world,
          EmberEntityTooltipWidget& tooltip, Icons::IconManager& iconManager) :
          mWorld(world), mTooltip(tooltip), mIconManager(iconManager), isUpdatingOurselves(
              false), mImageWindow(0), mTextWindow(0)
      {
        mImageWindow = CEGUI::WindowManager::getSingleton().createWindow(
            "EmberLook/StaticImage");
        mImageWindow->setSize(
            CEGUI::UVector2(CEGUI::UDim(0, 64), CEGUI::UDim(0, 64)));
        ...
        mTextWindow = CEGUI::WindowManager::getSingleton().createWindow(
            "EmberLook/StaticText");
        mTextWindow->setSize(
            CEGUI::UVector2(CEGUI::UDim(1, -64), CEGUI::UDim(1, 0)));
        mTextWindow->setPosition(
            CEGUI::UVector2(CEGUI::UDim(0, 64), CEGUI::UDim(0, 0)));
        ...
        tooltip.subscribeEvent(CEGUI::Window::EventTextChanged,
            CEGUI::Event::Subscriber(&EntityTooltip::tooltip_TextChanged,
                this));
      }
      ...
      CEGUI::Tooltip&
      EntityTooltip::getTooltipWindow() const
      {
        return mTooltip;
      }

      bool
      EntityTooltip::tooltip_TextChanged(const CEGUI::EventArgs &e)
      {
                ...
                if (icon)
                  {
                    mImageWindow->setProperty("Image",
                        CEGUI::PropertyHelper::imageToString(icon->getImage()));
                  }
                else
                  {
                    mImageWindow->setProperty("Image", "");
      ...
      std::string
      EntityTooltip::composeEntityInfoText(EmberEntity& entity)
      {
        std::stringstream ss;
        if (entity.getName() != "")
          {
            ss << entity.getName() << " (of type "
      ...
      EmberEntity*
      EntityTooltip::getActiveEntity()
      {
        if (mTooltip.getTargetWindow())
          {
            return mWorld.getEmberEntity(
                mTooltip.getTargetWindow()->getTooltipText().c_str());
          }
        return 0;
      }
