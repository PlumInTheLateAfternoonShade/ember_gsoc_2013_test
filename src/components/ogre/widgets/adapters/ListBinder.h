      ...
      namespace Adapters
      {
        ...
        template<class T, class WidgetT>
          ...
          public:
             ...
             */
            void
            addType(const std::string& key, const std::string& displayName,
                T type);
          ...
          protected:
            ...
            public:
               ...
               */
              BinderInstance(const std::string& displayName, T type,
                  size_t listIndex = 0) :
                  DisplayName(displayName), Type(type), ListIndex(listIndex)
              {
              }
             ...
             */
            bool
            listbox_SelectionChanged(const CEGUI::EventArgs& e);
...
// 	}
        ...
        template<typename T, typename WidgetT>
          void
          ListBinder<T, WidgetT>::sync()
          {
            mListbox->resetList();
            for (typename TypeStore::iterator I = mTypes.begin();
                I != mTypes.end(); ++I)
              {
                CEGUI::ListboxItem* item = new ColouredListItem(
                    I->second.DisplayName, mListbox->getItemCount());
        ...
        template<typename T, typename WidgetT>
          void
          ListBinder<T, WidgetT>::addType(const std::string& key,
              const std::string& displayName, T type)
          {
            mTypes.insert(
                typename TypeStore::value_type(key,
                    BinderInstance(displayName, type)));
          }

        template<typename T, typename WidgetT>
          bool
          ListBinder<T, WidgetT>::listbox_SelectionChanged(
        ...
        template<typename T, typename WidgetT>
          typename ListBinder<T, WidgetT>::SelectedType
          ListBinder<T, WidgetT>::getCurrentSelected()
          {
        ...
        template<typename T, typename WidgetT>
          T*
          ListBinder<T, WidgetT>::select(const std::string& key)
          {
            typename TypeStore::iterator I = mTypes.find(key);
            if (I != mTypes.end())
              {
                CEGUI::ListboxItem* item = mListbox->getListboxItemFromIndex(
                    I->second.ListIndex);
                if (item)
                  {
