...
// 	axisBox.fromAtlas(element.asList());
            if (mXWindow)
              {
                mXWindow->setText(
                    ValueTypeHelper<float, std::string>::convert(
          ...
          bool
          OrientationAdapter::window_TextChanged(const CEGUI::EventArgs& e)
          {
            if (!mSelfUpdate)
              {
          ...
          void
          OrientationAdapter::fillElementFromGui()
          {
