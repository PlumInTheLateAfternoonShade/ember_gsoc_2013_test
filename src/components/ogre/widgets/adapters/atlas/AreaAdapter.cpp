      ...
      namespace Adapters
      {
        ...
        namespace Atlas
        {
          ...
          AreaAdapter::AreaAdapter(const ::Atlas::Message::Element& element,
              CEGUI::PushButton* showButton, CEGUI::Combobox* layerWindow,
              EmberEntity* entity) :
              AdapterBase(element), mLayer(0), mLayerWindow(layerWindow), mEntity(
                  entity), mPolygonAdapter(nullptr)
          {
            if (element.isMap())
              {
                const ::Atlas::Message::MapType& areaData(element.asMap());
                ::Atlas::Message::MapType::const_iterator shapeI =
                    areaData.find("shape");
                if (shapeI != areaData.end())
                  {
                    mPolygonAdapter = std::unique_ptr < PolygonAdapter
                        > (new PolygonAdapter(shapeI->second, showButton,
                            entity));
                  }
                else
                  {
                    mPolygonAdapter = std::unique_ptr < PolygonAdapter
                        > (new PolygonAdapter(::Atlas::Message::MapType(),
                            showButton, entity));
                  }
            ...
            if (mLayerWindow)
              {
                addGuiEventConnection(
                    mLayerWindow->subscribeEvent(
                        CEGUI::Combobox::EventListSelectionChanged,
                        CEGUI::Event::Subscriber(
                            &AreaAdapter::layerWindow_ListSelectionChanged,
                            this)));
              }
          ...
          void
          AreaAdapter::updateGui(const ::Atlas::Message::Element& element)
          {
          }

          bool
          AreaAdapter::layerWindow_TextChanged(const CEGUI::EventArgs& e)
          {
            if (!mSelfUpdate)
              {
          ...
          void
          AreaAdapter::toggleDisplayOfPolygon()
          {
            mPolygonAdapter->toggleDisplayOfPolygon();
          }

          void
          AreaAdapter::createNewPolygon()
          {
            mPolygonAdapter->createNewPolygon();
          }

          void
          AreaAdapter::fillElementFromGui()
          {
            ...
            if (mPolygonAdapter->hasShape())
              {
                Terrain::TerrainAreaParser parser;
                mEditedValue = parser.createElement(mPolygonAdapter->getShape(),
                    mLayer);
              }
            else
              {
          ...
          bool
          AreaAdapter::_hasChanges()
          {
            return mOriginalValue != getChangedElement();
          }

          void
          AreaAdapter::addAreaSuggestion(int id, const std::string& name)
          {
          ...
          void
          AreaAdapter::clearAreaSuggestions()
          {
