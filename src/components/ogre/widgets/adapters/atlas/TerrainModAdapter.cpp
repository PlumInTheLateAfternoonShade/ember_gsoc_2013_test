      ...
      namespace Adapters
      {
        ...
        namespace Atlas
        {
          ...
          TerrainModAdapter::TerrainModAdapter(
              ...
              CEGUI::PushButton* showButton, EmberEntity* entity,
              CEGUI::Combobox* posTypeCombobox,
              CEGUI::Combobox* modTypeCombobox, CEGUI::Editbox* heightTextbox) :
              AdapterBase(element), mEntity(entity), mPolygonAdapter(nullptr), mHeightTextbox(
                  heightTextbox), mTerrainModsBinder(modTypeCombobox), mPositioningsBinder(
                  posTypeCombobox)
          {
            ...
            if (element.isMap())
              {
                ...
                ::Atlas::Message::MapType::const_iterator I = areaData.find(
                    "shape");
                if (I != areaData.end())
                  {
                    mPolygonAdapter = std::unique_ptr < PolygonAdapter
                        > (new PolygonAdapter(I->second, showButton, entity));
                  }
                else
                  {
                    mPolygonAdapter = std::unique_ptr < PolygonAdapter
                        > (new PolygonAdapter(::Atlas::Message::Element(),
                            showButton, entity));
                  }
              }
            else
            ...
            if (heightTextbox)
              {
                addGuiEventConnection(
                    heightTextbox->subscribeEvent(
                        CEGUI::Window::EventTextChanged,
                        CEGUI::Event::Subscriber(
                            &TerrainModAdapter::heightTextbox_TextChanged,
                            this)));
              }
            mTerrainModsBinder.addType("levelmod", "Level", LevelTerrainMod());
            mTerrainModsBinder.addType("adjustmod", "Adjust",
                AdjustTerrainMod());

            mPositioningsBinder.addType("height", "Fixed", FixedPositioning());
            mPositioningsBinder.addType("heightoffset", "Relative",
                RelativePositioning());
          ...
          void
          TerrainModAdapter::updateGui(const ::Atlas::Message::Element& element)
          {
            ...
            if (element.isMap())
              {
                ...
                ::Atlas::Message::MapType::const_iterator I = mapElement.find(
                    "height");
                float height = 0;
                if (I != mapElement.end())
                  {
          ...
          void
          TerrainModAdapter::fillElementFromGui()
          {
            ::Atlas::Message::MapType mapElement;
            ::Atlas::Message::Element shapeElement =
                mPolygonAdapter->getChangedElement();
            ...
            std::pair<const std::string, PositioningBase*> positioning =
                mPositioningsBinder.getCurrentSelected();
            if (positioning.second)
              {
                mapElement[positioning.first] = ::Atlas::Message::Element(
                    atof(mHeightTextbox->getText().c_str()));
              }
            std::pair<const std::string, TerrainModBase*> modType =
                mTerrainModsBinder.getCurrentSelected();
            if (modType.second)
              {
