...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Gui
    {
      ...
      void
      EntityIconDragDropPreview::createPreview(EntityIcon* icon)
      {
            ...
            if (icon && icon->getEntity())
              {
                ...
                Gui::HelpMessage message("Entity Drag Preview",
                    "Release the left mouse button to place the entity at the selected location. Press Escape to cancel.",
                    "entity icon drag drop preview", "dragDropMessage");
                Gui::QuickHelp::getSingleton().updateText(message);
                mModelPreviewWorker = new ModelPreviewWorker(mWorld,
                    mIconEntity);
                mModelPreviewWorker->EventCleanupCreation.connect(
                    sigc::mem_fun(*this,
                        &EntityIconDragDropPreview::cleanupCreation));
                mModelPreviewWorker->EventFinalizeCreation.connect(
                    sigc::mem_fun(*this,
                        &EntityIconDragDropPreview::finalizeCreation));
              }
          }
      }
      ...
      EntityIconDragDropPreview::finalizeCreation()
      {
        mDropOffset = mModelPreviewWorker->getPosition()
            - mWorld.getAvatar()->getClientSideAvatarPosition();
      ...
      WFMath::Vector<3>
      EntityIconDragDropPreview::getDropOffset() const
      {
        return mDropOffset;
      }

      WFMath::Quaternion
      EntityIconDragDropPreview::getDropOrientation() const
      {
        return mDropOrientation;
      }

      ModelPreviewWorker::ModelPreviewWorker(World& world,
          Eris::ViewEntity* entity) :
          mWorld(world), mEntity(0), mEntityNode(0), mModel(0), mModelMount(0), mMovement(
              0)
      {
        ...
        // Temporary entity
        mEntity = new Authoring::DetachedEntity("-1", erisType,
            entity->getView()->getAvatar()->getConnection()->getTypeService());
        mEntity->setFromMessage(mEntityMessage);

        // Creating scene node
        mEntityNode =
            mWorld.getSceneManager().getRootSceneNode()->createChildSceneNode();
        ...
        std::unique_ptr<EntityMapping::EntityMapping> modelMapping(
            Mapping::EmberEntityMappingManager::getSingleton().getManager().createMapping(
                *mEntity, actionCreator, 0));
        if (modelMapping.get())
          {
        ...
        mMovement = new ModelPreviewWorkerMovement(*this,
            mWorld.getMainCamera(), *mEntity, mEntityNode);
      ...
      void
      ModelPreviewWorker::setModel(const std::string& modelName)
      {
        if (mModel)
          {
        ...
        mModel->Reloaded.connect(
            sigc::mem_fun(*this, &ModelPreviewWorker::model_Reloaded));

        //if the model definition isn't valid, use a placeholder
        if (!mModel->getDefinition()->isValid())
          {
            S_LOG_FAILURE(
                "Could not find " << modelName << ", using placeholder.");
            //add a placeholder model
            Model::ModelDefinitionPtr modelDef = mModel->getDefinition();
            modelDef->createSubModelDefinition(
                "3d_objects/primitives/models/box.mesh")->createPartDefinition(
                "main")->setShow(true);
            modelDef->setValid(true);
            modelDef->reloadAllInstances();
          }

        mModelMount = new Model::ModelMount(*mModel,
            new SceneNodeProvider(*mEntityNode, mModel));
      ...
      ModelPreviewWorker::getPosition() const
      {
        return Convert::toWF<WFMath::Point<3>>(mEntityNode->getPosition());
      }

      const WFMath::Quaternion
      ModelPreviewWorker::getOrientation() const
      {
        return Convert::toWF(mEntityNode->getOrientation());
      }

      void
      ModelPreviewWorker::showModelPart(const std::string& partName)
      {
        if (mModel)
          {
      ...
      void
      ModelPreviewWorker::hideModelPart(const std::string& partName)
      {
        if (mModel)
          {
      ...
      void
      ModelPreviewWorker::initFromModel()
      {
        scaleNode();
      }

      void
      ModelPreviewWorker::model_Reloaded()
      {
        initFromModel();
      }

      void
      ModelPreviewWorker::scaleNode()
      {
        if (mModelMount)
          {
          ...
          {
            S_LOG_WARNING(
                "Tried to scale node without there being a valid model mount.");
          }
      }

      bool
      ModelPreviewWorker::hasBBox()
      {
        return mEntity->hasBBox();
      }

      const WFMath::AxisBox<3> &
      ModelPreviewWorker::getBBox()
      {
        return mEntity->getBBox();
      }

      ModelPreviewWorkerPartAction::ModelPreviewWorkerPartAction(
          ModelPreviewWorker& modelPreviewWorker, std::string partName) :
      ...
      void
      ModelPreviewWorkerPartAction::activate(
          EntityMapping::ChangeContext& context)
      {
      ...
      void
      ModelPreviewWorkerPartAction::deactivate(
          EntityMapping::ChangeContext& context)
      {
      ...
      ModelPreviewWorkerModelAction::ModelPreviewWorkerModelAction(
          ModelPreviewWorker& modelPreviewWorker, std::string modelName) :
      ...
      void
      ModelPreviewWorkerModelAction::activate(
          EntityMapping::ChangeContext& context)
      {
      ...
      void
      ModelPreviewWorkerModelAction::deactivate(
          EntityMapping::ChangeContext& context)
      {
      ...
      void
      ModelPreviewWorkerHideModelAction::activate(
          EntityMapping::ChangeContext& context)
      {
        mModelPreviewWorker.setModel("");
      }
      ...
      void
      ModelPreviewWorkerHideModelAction::deactivate(
          EntityMapping::ChangeContext& context)
      {
      }
      ...
      void
      ModelPreviewWorkerActionCreator::createActions(
          EntityMapping::EntityMapping& modelMapping,
          EntityMapping::Cases::CaseBase* aCase,
          EntityMapping::Definitions::CaseDefinition& caseDefinition)
      {
        EntityMapping::Definitions::CaseDefinition::ActionStore::iterator endJ =
            caseDefinition.getActions().end();
        for (EntityMapping::Definitions::CaseDefinition::ActionStore::iterator J =
            caseDefinition.getActions().begin(); J != endJ; ++J)
          {
            if (J->getType() == "display-part")
              {
                ModelPreviewWorkerPartAction* action =
                    new ModelPreviewWorkerPartAction(mModelPreviewWorker,
                        J->getValue());
                aCase->addAction(action);
              }
            else if (J->getType() == "display-model")
              {
                ModelPreviewWorkerModelAction* action =
                    new ModelPreviewWorkerModelAction(mModelPreviewWorker,
                        J->getValue());
                aCase->addAction(action);
              }
            else if (J->getType() == "hide-model")
              {
                ModelPreviewWorkerHideModelAction* action =
                    new ModelPreviewWorkerHideModelAction(mModelPreviewWorker);
                aCase->addAction(action);
              }
          }
      }
      ...
      ModelPreviewWorkerMovementBridge::ModelPreviewWorkerMovementBridge(
          ...
          Authoring::EntityMoverBase(entity, node, *node->getCreator()), mModelPreviewWorker(
              modelPreviewWorker)
      {
      }
      ...
      void
      ModelPreviewWorkerMovementBridge::finalizeMovement()
      {
        mModelPreviewWorker.EventFinalizeCreation.emit();
      }
      void
      ModelPreviewWorkerMovementBridge::cancelMovement()
      {
      ...
      ModelPreviewWorkerMovement::ModelPreviewWorkerMovement(
          ...
          const Camera::MainCamera& camera, Authoring::DetachedEntity& entity,
          Ogre::SceneNode* node) :
          mMoveAdapter(camera)
      {
        ...
        ModelPreviewWorkerMovementBridge* bridge =
            new ModelPreviewWorkerMovementBridge(mModelPreviewWorker, entity,
                node);
