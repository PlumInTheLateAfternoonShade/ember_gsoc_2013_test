  ...
  namespace OgreView
  {
    ...
    namespace Gui
    {
      ...
      ConsoleAdapter::ConsoleAdapter(CEGUI::Editbox* inputBox) :
          mInputBox(inputBox), mBackend(ConsoleBackend::getSingletonPtr()), mTabPressed(
              false), mSelected(0), mReturnKeyDown(false)
      {
        mInputBox->subscribeEvent(CEGUI::Editbox::EventKeyUp,
            CEGUI::Event::Subscriber(&ConsoleAdapter::consoleInputBox_KeyUp,
                this));
        mInputBox->subscribeEvent(CEGUI::Editbox::EventKeyDown,
            CEGUI::Event::Subscriber(&ConsoleAdapter::consoleInputBox_KeyDown,
                this));
      }
      ...
      bool
      ConsoleAdapter::consoleInputBox_KeyUp(const CEGUI::EventArgs& args)
      {
        const CEGUI::KeyEventArgs& keyargs =
            static_cast<const CEGUI::KeyEventArgs&>(args);
        ...
        case CEGUI::Key::ArrowUp:
          {
            if (mBackend->getHistory().getHistoryPosition() == 0)
            ...
            else
              {
                ...
                mBackend->getHistory().changeHistory(
                    mBackend->getHistory().getHistoryPosition(),
                    mInputBox->getText().c_str());
              }
            ...
            if (mBackend->getHistory().getHistoryPosition() > 0)
              {
                ...
                if (mBackend->getHistory().getHistoryPosition() == 0)
                  ...
                  {
                    mInputBox->setText(
                        mBackend->getHistory().getHistoryString());
                  }
              }
            ...
            if (mTabPressed == true)
              {
                const std::set<std::string> commands(
                    mBackend->getPrefixes(sCommand));

                //std::cout << sCommand << std::endl;
                if (commands.size() > 0)
                  {
                    std::set<std::string>::const_iterator iCommand(
                        commands.begin());
                    std::string sMessage("");
                        ...
                        if (select == mSelected)
                          {
                            ...
                            mInputBox->setText(
                                sCommandLine.substr(0,
                                    mInputBox->getCaratIndex())
                                    + iCommand->substr(
                                        mInputBox->getCaratIndex() - 1));
                            mInputBox->setSelection(mInputBox->getCaratIndex(),
                                0xFFFFFFFF);
                          }
                        sMessage += *iCommand + ' ';
                        ++iCommand;
                ...
                else
                  {
                    ...
                    if (commands.size() == 1)
                    ...
                    else
                      {
                        ...
                        std::set<std::string>::const_iterator iSelected(
                            commands.begin());
                        std::set<std::string>::const_iterator iCommand(
                            commands.begin());
                        ...
                        mInputBox->setText(
                            std::string("/") + sCommonPrefix
                                + iSelected->substr(sCommonPrefix.length()));
                        mInputBox->setCaratIndex(sCommonPrefix.length() + 1);
                        mInputBox->setSelection(sCommonPrefix.length() + 1,
                            0xFFFFFFFF);
                      }
                  }
              }
            ...
            if (mReturnKeyDown)
              {
                ...
                if (mInputBox->getSelectionLength() > 0)
                  {
                    unsigned long ulSelectionEnd(
                        mInputBox->getSelectionEndIndex());
