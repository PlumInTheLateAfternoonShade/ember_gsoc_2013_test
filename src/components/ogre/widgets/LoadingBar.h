...
namespace Ember
{
  ...
  namespace OgreView
  {
    namespace Gui
    {
      ...
      public:
        LoadingBarSection(LoadingBar& loadingBar, float size,
      ...
      public:
        ...
        virtual
        ~WfutLoadingBarSection();
      private:

        void
        wfutService_DownloadComplete(const std::string& url,
            const std::string& filename);

        void
        wfutService_DownloadFailed(const std::string& url,
            const std::string& filename, const std::string& reason);

        void
        wfutService_AllDownloadsComplete();

        void
        wfutService_DownloadingServerList(const std::string& url);

        void
        wfutService_UpdatesCalculated(unsigned int numberOfFilesToUpdate);
      ...
      {
      public:
        ResourceGroupLoadingBarSection(LoadingBarSection& section,
            unsigned short numGroupsInit = 1, unsigned short numGroupsLoad = 1,
        ...
        void
        resourceGroupScriptingStarted(const Ogre::String & groupName,
            size_t scriptCount);
        void
        scriptParseStarted(const Ogre::String & scriptName,
            bool& skipThisScript);
        void
        scriptParseEnded(const Ogre::String& scriptName, bool skipped);
        void
        resourceGroupScriptingEnded(const Ogre::String & groupName);
        void
        resourceGroupLoadStarted(const Ogre::String & groupName,
            size_t resourceCount);
        void
        resourceLoadStarted(const Ogre::ResourcePtr& resource);
        void
        resourceLoadEnded(void);
        void
        worldGeometryStageStarted(const Ogre::String & description)
        {
        }
        void
        ...
        void
        resourceGroupLoadEnded(const Ogre::String & groupName);
      ...
      public:
        ...
        void
        addSection(LoadingBarSection* section);

        void
        activateSection(LoadingBarSection* section);
        void
        ...
        setCaption(const std::string& caption);
        void
        setVersionText(const std::string& versionText);
         ...
         */
        void
        deleteOverlayContainerContents(Ogre::OverlayContainer& container) const;
         ...
         */
        void
        updateRender(bool forceRender = false);
