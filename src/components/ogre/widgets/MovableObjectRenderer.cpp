  ...
  namespace OgreView
  {
    namespace Gui
    {
      ...
      public:
        ...
        virtual void
        postRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
        {
          mWindow.invalidate();
        }
      };
      ...
      MovableObjectRenderer::MovableObjectRenderer(CEGUI::Window* image) :
          mTexture(0), mAutoShowFull(true), mImage(image), mActive(true), mAxisEntity(
              0), mAxesNode(0), mWindowUpdater(0)
      {
        int width = static_cast<int>(image->getPixelSize().d_width);
        int height = static_cast<int>(image->getPixelSize().d_height);
        if (width != 0 && height != 0)
          {
            mTexture = new EntityCEGUITexture(image->getName().c_str(), width,
                height);

            mImage->setProperty("Image",
                CEGUI::PropertyHelper::imageToString(mTexture->getImage()));

            image->subscribeEvent(CEGUI::Window::EventSized,
                CEGUI::Event::Subscriber(&MovableObjectRenderer::image_Sized,
                    this));

            // Register this as a frame listener
            Ogre::Root::getSingleton().addFrameListener(this);
            mWindowUpdater = new CEGUIWindowUpdater(*mImage);
            mTexture->getRenderContext()->getRenderTexture()->addListener(
                mWindowUpdater);
      ...
      MovableObjectRenderer::~MovableObjectRenderer()
      {
        ...
        if (mTexture && mWindowUpdater)
          {
            mTexture->getRenderContext()->getRenderTexture()->removeListener(
                mWindowUpdater);
          }
      ...
      Ogre::Quaternion
      MovableObjectRenderer::getEntityRotation() const
      {
        return mTexture->getRenderContext()->getSceneNode()->getOrientation();
      }

      void
      MovableObjectRenderer::setEntityRotation(const Ogre::Quaternion& rotation)
      {
        mTexture->getRenderContext()->getSceneNode()->setOrientation(rotation);
      }

      Ogre::Vector3
      MovableObjectRenderer::getEntityTranslation() const
      {
        return mTexture->getRenderContext()->getSceneNode()->getPosition();
      }

      void
      MovableObjectRenderer::setEntityTranslation(
      ...
      void
      MovableObjectRenderer::resetCameraOrientation()
      {
        mTexture->getRenderContext()->resetCameraOrientation();
      }

      void
      MovableObjectRenderer::pitchCamera(Ogre::Degree degrees)
      {
        mTexture->getRenderContext()->pitchCamera(degrees);
      }

      void
      MovableObjectRenderer::yawCamera(Ogre::Degree degrees)
      {
        mTexture->getRenderContext()->yawCamera(degrees);
      }

      void
      MovableObjectRenderer::rollCamera(Ogre::Degree degrees)
      {
        mTexture->getRenderContext()->rollCamera(degrees);
      }

      bool
      MovableObjectRenderer::getAutoShowFull() const
      {
        return mAutoShowFull;
      }

      void
      MovableObjectRenderer::setAutoShowFull(bool showFull)
      {
        mAutoShowFull = showFull;
      }

      void
      MovableObjectRenderer::showFull()
      {
        mTexture->getRenderContext()->showFull(getMovableObject());
      }

      void
      MovableObjectRenderer::rescaleAxisMarker()
      {
        if (mAxesNode && getMovableObject())
          {
            Ogre::Real axesScale = (getMovableObject()->getBoundingRadius()
                / 5.0f);
      ...
      void
      MovableObjectRenderer::setCameraDistance(float distance)
      {

        mTexture->getRenderContext()->setCameraDistance(
            mTexture->getRenderContext()->getDefaultCameraDistance()
                * distance);
      ...
      float
      MovableObjectRenderer::getCameraDistance()
      {
        return mTexture->getRenderContext()->getCameraDistance();
      }

      float
      MovableObjectRenderer::getAbsoluteCameraDistance()
      {
        return mTexture->getRenderContext()->getAbsoluteCameraDistance();
      }

      bool
      MovableObjectRenderer::frameStarted(const Ogre::FrameEvent& event)
      {
      ...
      MovableObjectRenderer::updateRender()
      {
        try
          ...
          {
            S_LOG_FAILURE(
                "Error when updating render for MovableObjectRenderer." << ex);
          }
      }

      void
      MovableObjectRenderer::setBackgroundColour(
      ...
      void
      MovableObjectRenderer::setBackgroundColour(float red, float green,
          float blue, float alpha)
      {
        mTexture->getRenderContext()->setBackgroundColour(red, green, blue,
            alpha);
      }

      void
      MovableObjectRenderer::showAxis()
      {
        if (!mAxesNode)
          {
            mAxesNode =
                mTexture->getRenderContext()->getSceneManager()->getRootSceneNode()->createChildSceneNode();
          }
        if (!mAxisEntity)
          {
            ...
            try
              {
                mAxisEntity =
                    mTexture->getRenderContext()->getSceneManager()->createEntity(
                        name + "_axes", "axes.mesh");
                if (mAxisEntity)
                  {
                    try
                      {
      ...
      void
      MovableObjectRenderer::hideAxis()
      {
        if (mAxesNode)
          {
      ...
      SimpleRenderContext::CameraPositioningMode
      MovableObjectRenderer::getCameraPositionMode() const
      {
        return mTexture->getRenderContext()->getCameraPositionMode();
      }

      void
      MovableObjectRenderer::setCameraPositionMode(
          SimpleRenderContext::CameraPositioningMode mode)
      {
        mTexture->getRenderContext()->setCameraPositionMode(mode);
      }

      EntityCEGUITexture&
      MovableObjectRenderer::getEntityTexture()
      {
        return *mTexture;
      }

      bool
      MovableObjectRenderer::image_Sized(const CEGUI::EventArgs& e)
      {
        ...
        if (size.d_width > 0.0f && size.d_height > 0.0f)
          {
            mTexture->getRenderContext()->getCamera()->setAspectRatio(
