...
namespace Ember
{
  namespace OgreView
  {
    ...
    namespace Gui
    {
      ...
      TexturePair
      AssetsManager::showTexture(const std::string textureName)
      {
                ...
                if (!texturePtr->isLoaded())
                  {
                    try
                      ...
                      {
                        S_LOG_WARNING(
                            "Error when loading " << textureName << ". This texture will not be shown.");
                        return TexturePair();
                      }
                  }
      ...
      TexturePair
      AssetsManager::createTextureImage(Ogre::TexturePtr texturePtr,
          const std::string& imageSetName)
      {
        ...
        CEGUI::Texture* ogreCEGUITexture =
            &GUIManager::getSingleton().createTexture(texturePtr);
        ...
        textureImageset = &CEGUI::ImagesetManager::getSingleton().create(
            imageSetName, *ogreCEGUITexture);

        //we only want one element: the whole texture
        textureImageset->defineImage("full_image",
            CEGUI::Rect(0, 0, texturePtr->getWidth(), texturePtr->getHeight()),
            CEGUI::Point(0, 0));

        //assign our image element to the StaticImage widget
        const CEGUI::Image* textureImage = &textureImageset->getImage(
            "full_image");
      ...
      std::string
      AssetsManager::materialAsText(Ogre::MaterialPtr material)
      {
        if (material.isNull())
          {
            return "";
      ...
      std::string
      AssetsManager::resolveResourceNameFromFilePath(
          const std::string& filePath)
      {
        const std::multimap<std::string, std::string>& locations =
            EmberOgre::getSingleton().getResourceLocations();

        for (std::multimap<std::string, std::string>::const_iterator I =
            locations.begin(); I != locations.end(); ++I)
          {
            ...
            if (Ogre::StringUtil::startsWith(filePath, resourceLocation, true))
              {
                ...
                std::string localMeshPath = filePath.substr(
                    resourceLocation.length(), std::string::npos);
      ...
      std::string
      AssetsManager::resolveFilePathForMesh(Ogre::MeshPtr meshPtr)
      {
        ...
        for (std::multimap<std::string, std::string>::const_iterator I =
            locations.begin(); I != locations.end(); ++I)
          {
            ...
            Ogre::FileInfoListPtr files = manager.findResourceFileInfo(group,
                fileName, false);
            for (Ogre::FileInfoList::const_iterator J = files->begin();
                J != files->end(); ++J)
              {
      ...
      bool
      AssetsManager::exportMesh(Ogre::MeshPtr mesh, const std::string& filePath)
      {
        if (filePath != "")
          {
            ...
            try
              ...
              {
                S_LOG_FAILURE(
                    "Error when exporting mesh " << mesh->getName() << "to path " << filePath <<"." << ex);
                return false;
              }
            return true;
          }
