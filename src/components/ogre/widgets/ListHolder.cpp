...
namespace Ember
{
  namespace OgreView
  {
    namespace Gui
    {
      ...
      ListHolder::ListHolder(CEGUI::Listbox* listbox,
          CEGUI::Editbox* filterEditbox) :
          mListbox(listbox), mFilterEditbox(filterEditbox)
      {
        if (filterEditbox)
          {
            BIND_CEGUI_EVENT(filterEditbox, CEGUI::Window::EventTextChanged,
                ListHolder::filterEditbox_TextChanged);
          }
...
// 	}
      ...
      void
      ListHolder::addItem(CEGUI::ListboxItem* item)
      {
      ...
      void
      ListHolder::insertItem(CEGUI::ListboxItem* item,
          const CEGUI::ListboxItem* position)
      {
        //not yet supported
      ...
      void
      ListHolder::removeItem(const CEGUI::ListboxItem* item)
      {
        if (mListbox)
          {
            ListItemStore::iterator pos = std::find(mItems.begin(),
                mItems.end(), item);
            if (pos != mItems.end())
              {
                mItems.erase(pos);
      ...
      bool
      ListHolder::isItemAllowed(CEGUI::ListboxItem* item)
      {
        if (mFilterEditbox)
          {
      ...
      ListHolder::updateItems()
      {
        ...
        if (mListbox)
          {
            for (ListItemStore::iterator I = mItems.begin(); I != mItems.end();
                ++I)
              {
...
// }
      ...
      bool
      ListHolder::filterEditbox_TextChanged(const CEGUI::EventArgs& args)
      {
        updateItems();
        return true;
      }
      void
      ListHolder::resetList()
      {
        if (mListbox)
          {
            mListbox->resetList();
