  ...
  namespace OgreView
  {
    ...
    namespace Gui
    {
      ...
      namespace Icons
      {
        ...
        IconImageStoreEntry::IconImageStoreEntry(IconImageStore& iconImageStore,
            const PixelPos& pixelPosInImageset) :
            mImage(0), mIconImageStore(iconImageStore), mPixelPosInImageset(
                pixelPosInImageset)
        {
          createImage();
        }
        ...
        void
        IconImageStoreEntry::createImage()
        {
          std::stringstream ss;
          ss << mPixelPosInImageset.first << "_" << mPixelPosInImageset.second;
          mImageName = ss.str();
          mIconImageStore.mImageset->defineImage(mImageName,
              CEGUI::Rect(mPixelPosInImageset.first, mPixelPosInImageset.second,
        ...
        const CEGUI::Image*
        IconImageStoreEntry::getImage()
        {
          return mImage;
        }

        const CEGUI::Image*
        IconImageStoreEntry::getImage() const
        {
          return mImage;
        }

        const std::string&
        IconImageStoreEntry::getImageName()
        {
          return mImageName;
        }

        const std::string&
        IconImageStoreEntry::getImageName() const
        {
          return mImageName;
        }

        Ogre::TexturePtr
        IconImageStoreEntry::getTexture()
        {
          return mIconImageStore.mTexPtr;
        }

        Ogre::PixelBox
        IconImageStoreEntry::getImagePixelBox()
        {
          return mIconImageStore.getImage().getPixelBox().getSubVolume(getBox());
        }

        Ogre::Image::Box
        IconImageStoreEntry::getBox()
        {
          Ogre::Image::Box box(mPixelPosInImageset.first,
        ...
        Ogre::TRect<float>
        IconImageStoreEntry::getRelativeBox()
        {
          ...
          Ogre::TRect<float> box(
              static_cast<float>(mPixelPosInImageset.first) / imageSize,
              static_cast<float>(mPixelPosInImageset.second) / imageSize,
              static_cast<float>(mPixelPosInImageset.first
                  + mIconImageStore.mIconSize) / imageSize,
              static_cast<float>(mPixelPosInImageset.second
                  + mIconImageStore.mIconSize) / imageSize);
          return box;
        }

        IconImageStore::IconImageStore(const std::string& imagesetName) :
            mImagesetName(imagesetName), mIconSize(64), mImageSize(256), mImageDataStream(
                OGRE_NEW Ogre::MemoryDataStream(mImageSize * mImageSize * 4,
                    true)), mCeguiTexture(0), mImageset(0)
        {
          createImageset();
          createEntries();
        }
        ...
        IconImageStore::IconImageStore(const std::string& imagesetName,
            ...
            mImagesetName(imagesetName), mTexPtr(texPtr), mImageDataStream(0), mCeguiTexture(
                0), mImageset(0)
        {
          ...
          mImageset = &CEGUI::ImagesetManager::getSingleton().create(
              mImagesetName, *mCeguiTexture);
        ...
        {
          for (IconImageStoreEntryStore::iterator I(mIconImages.begin());
              I != mIconImages.end(); ++I)
            {
              delete *I;
            }
        ...
        void
        IconImageStore::createImageset()
        {

          //reset the image
          memset(mImageDataStream->getPtr(), '\0', mImageDataStream->size());
          mImage.loadDynamicImage(mImageDataStream->getPtr(), mImageSize,
              mImageSize, 1, Ogre::PF_A8R8G8B8);
          ...
          mTexPtr = Ogre::TextureManager::getSingleton().loadImage(
              mImagesetName, "Gui", mImage, Ogre::TEX_TYPE_2D, 0);
          if (mTexPtr.isNull())
            {
              ...
              for (int y = 0; y < entriesPerAxis; ++y)
                {
                  ...
                  IconImageStoreEntry* entry = new IconImageStoreEntry(*this,
                      IconImageStoreEntry::PixelPos(pixelPosStartX,
                          pixelPosStartY));
        ...
        size_t
        IconImageStore::getNumberOfUnclaimedIcons()
        {
          return mUnclaimedIconImages.size();
        }

        IconImageStoreEntry*
        IconImageStore::claimImageEntry()
        {
          if (!getNumberOfUnclaimedIcons())
            {
              S_LOG_WARNING(
                  "Trying to claim image entry from store with no unclaimed entries.");
              return 0;
            }
        ...
        void
        IconImageStore::returnImageEntry(IconImageStoreEntry* imageEntry)
        {
...
// 	}
          if (std::find(mIconImages.begin(), mIconImages.end(), imageEntry)
              == mIconImages.end())
            {
              S_LOG_WARNING(
                  "Trying to return an image entry which doesn't belong to this store.");
            }
