  ...
  namespace OgreView
  {
    ...
    namespace Gui
    {
          ...
          mLastMode(Input::IM_GUI), mGuiManager(guiManager)
      {
        ...
        Input::getSingleton().EventChangedInputMode.connect(
            sigc::mem_fun(*this, &ActiveWidgetHandler::Input_InputModeChanged));
      ...
      void
      ActiveWidgetHandler::Input_InputModeChanged(Input::InputMode mode)
      {
        if (mode != Input::IM_GUI && mLastMode == Input::IM_GUI)
          {
            //save the current active widget
            CEGUI::Window* window =
                mGuiManager.getMainSheet()->getActiveChild();
            if (window)
              {
            ...
            if (mLastActiveWidgetName != "")
              {
                ...
                try
                  ...
                  {
                    S_LOG_WARNING(
                        "Error when trying to restore previously captured window.");
                  }
                mLastActiveWidgetName = "";
              }
            mLastMode = mode;
