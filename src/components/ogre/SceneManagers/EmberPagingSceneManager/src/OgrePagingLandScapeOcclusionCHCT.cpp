...
namespace Ogre
{
  ...
  //-----------------------------------------------------------------------
  void
  CHCTraversal::onTree(PagingLandScapeOctree& node,
      VisibleObjectsBoundsInfo * const visibleBounds) const
  {
    ...
    PagingLandScapeOctree *n = node.mChildren[0][0][0];
    if (n && n->hasChildren())
      stack.push(n);
  ...
  //-----------------------------------------------------------------------
  void
  CHCTraversal::onLeaf(PagingLandScapeOctreeNode& n,
      VisibleObjectsBoundsInfo * const visibleBounds) const
  {
#ifdef _VISIBILITYDEBUG
  ...
  //-----------------------------------------------------------------------
  CHCTraversal::CHCTraversal(FrontToBackNodeSorterPriorityQueue& vStack,
      Occlusion& o) :
      stack(vStack), occlusion(o), frameId(o.getFrame()), mIsNotSolidScene(
          occlusion.isNotSolidScene())
  {
  }
}
