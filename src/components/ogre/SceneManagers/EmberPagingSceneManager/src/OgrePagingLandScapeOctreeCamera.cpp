...
namespace Ogre
{
  ...
  //-----------------------------------------------------------------------
  PagingLandScapeOctreeCamera::PagingLandScapeOctreeCamera(const String& name,
      SceneManager* sm) :
      Camera(name, sm), mVisFacesLastCHCRender(0), isOcclusionSystemRegistered(
          false), mOcclusionMode(VIEW_FRUSTUM_DIRECT), mFrameId(0), mFrameSceneId(
          0)
  {
  ...
  //-----------------------------------------------------------------------
  bool
  PagingLandScapeOctreeCamera::nextFrame(const unsigned int framesSceneId)
  {
  ...
  //----------------------------------------------------------------------- 
  bool
  PagingLandScapeOctreeCamera::isRegisteredInOcclusionSystem() const
  {
    return isOcclusionSystemRegistered;
  }
  //-----------------------------------------------------------------------
  void
  PagingLandScapeOctreeCamera::setRegisteredInOcclusionSystem(
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeOctreeCamera::changeOcclusionMode(
      culling_modes nextOcclusionMode)
  {
    if (nextOcclusionMode != mOcclusionMode)
      {
        if (nextOcclusionMode == VIEW_FRUSTUM_DIRECT)
          ...
          {
            if (!getSceneManager()->getDestinationRenderSystem()->getCapabilities()->hasCapability(
                RSC_HWOCCLUSION))
              {
                OGRE_EXCEPT(1, "Your card does not support HWOCCLUSION ",
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeOctreeCamera::updateRegistrationInOcclusionSystem()
  {
    if (needRegistrationInOcclusionSystem() && !isRegisteredInOcclusionSystem())
      {
        static_cast<PagingLandScapeOctreeSceneManager *>(getSceneManager())->registerCamera(
            this);
        setRegisteredInOcclusionSystem(true);
      }
    else if (!needRegistrationInOcclusionSystem()
        && isRegisteredInOcclusionSystem())
      {
        static_cast<PagingLandScapeOctreeSceneManager *>(getSceneManager())->unregisterCamera(
            this);
  ...
  PagingLandScapeOctreeCamera::setNextOcclusionMode()
  {
    culling_modes nextOcclusionMode = static_cast<culling_modes>((mOcclusionMode
        + 1) % NUM_CULLING_MODE);
  ...
  //-----------------------------------------------------------------------
  bool
  PagingLandScapeOctreeCamera::needRegistrationInOcclusionSystem() const
  {
#ifdef _VISIBILITYDEBUG	
    return true;
#endif //_VISIBILITYDEBUG   
    if (mOcclusionMode == CHC || mOcclusionMode == CHC_CONSERVATIVE)
      return true;
    else
      return false;
  }
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeOctreeCamera::setOcclusionMode(culling_modes occlusionMode)
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeOctreeCamera::setOcclusionModeAsString(
      const String &cullingModeAsString)
  {
  ...
  //-----------------------------------------------------------------------
  String
  PagingLandScapeOctreeCamera::getOcclusionModeAsString() const
  {
    ...
    case CHC_CONSERVATIVE:
      return String("CHC_CONSERVATIVE");
  ...
  //-----------------------------------------------------------------------
  PagingLandScapeOctreeCamera::Visibility
  PagingLandScapeOctreeCamera::getVisibility(const AxisAlignedBox &bound) const
  {
    ...
    for (unsigned int plane = 0; plane < 6; ++plane)
      {
        ...
        const Plane::Side side = getFrustumPlane(plane).getSide(centre,
            halfSize);
