...
namespace Ogre
{
  ...
  PagingLandScapePageManager::PagingLandScapePageManager(
      PagingLandScapeSceneManager* scnMgr) :
      mSceneManager(scnMgr), mOptions(scnMgr->getOptions()), mData2d(
          scnMgr->getData2DManager()), mTexture(scnMgr->getTextureManager()), mRenderablesMgr(
          scnMgr->getRenderableManager()), mWidth(0), mHeight(0), mPause(99), mCurrentcam(
          0), mTerrainReady(false), mRenderQueueGroupID(
          static_cast<Ogre::RenderQueueGroupID>(scnMgr->getWorldGeometryRenderQueue())),
      //mRenderQueueGroupID (RENDER_QUEUE_WORLD_GEOMETRY_2),
      mOnFrame(false), mEnabled(false)
  {
    mPagePool.clear();
    mActivePages.clear();
    mFreePages.clear();
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::load()
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::clear()
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::WorldDimensionChange()
  {
    reset();
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::setMapMaterial()
  {
    std::for_each(mActivePages.begin(), mActivePages.end(),
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::_updateLod()
  {
    PagingLandScapePageList::iterator lend = mLoadedPages.end();
    for (PagingLandScapePageList::iterator l = mLoadedPages.begin(); l != lend;
        ++l)
      {
        (*l)->updateLod();
      }
  ...
  //-----------------------------------------------------------------------
  bool
  PagingLandScapePageManager::frameStarted(const FrameEvent& evt)
  {
    if (mEnabled) //if not queued to be removed from framelistener or Paused
      {
        if (mOptions->VisMap)
          {
            mSceneManager->getHorizon()->prepare(
                static_cast<PagingLandScapeCamera*>(mOptions->primaryCamera));
          }
        mOnFrame = false;
      }
    return true;
  ...
  //-----------------------------------------------------------------------
  bool
  PagingLandScapePageManager::frameEnded(const FrameEvent& evt)
  {
  ...
  //-----------------------------------------------------------------------
  PagingLandScapePage*
  PagingLandScapePageManager::getNewPage(unsigned int x, unsigned int z)
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::releasePage(PagingLandScapePage* p)
  {
    removeFromQueues(p);
    p->uninit();
  ...
  //-----------------------------------------------------------------------
  PagingLandScapePage*
  PagingLandScapePageManager::getPage(unsigned int x, unsigned int z,
      bool alwaysReturn)
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::LoadFirstPage(PagingLandScapeCamera* cam)
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::updateLoadedPages()
  {
    // Make any pending updates to the calculated frustum
    mCurrentcam->updateView();

    const Vector3 pos(mCurrentcam->getDerivedPosition().x, 127.0f,
        mCurrentcam->getDerivedPosition().z);

    for (PagingLandScapePageList::iterator it = mLoadedPages.begin();
        it != mLoadedPages.end(); ++it)
      {
        PagingLandScapePage* p = (*it);

        unsigned int x, z;
        p->getCoordinates(x, z);
        if ((z >= mCurrentcam->mIniZ) && (z <= mCurrentcam->mFinZ)
            && (x >= mCurrentcam->mIniX) && (x <= mCurrentcam->mFinX))
          {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::loadNow(PagingLandScapeCamera* cam)
  {
    updatePaging(cam);
    const Vector3 pos(cam->getDerivedPosition().x, 127.0f,
        cam->getDerivedPosition().z);
    while (!mTerrainReady || !mPageLoadQueue.empty())
      {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::queuePageNeighbors()
  {
    // Queue the rest				
    ...
    for (unsigned int i = mCurrentcam->mPreIniX; i <= mCurrentcam->mPreFinX;
        i++)
      {
        for (unsigned int j = mCurrentcam->mPreIniZ; j <= mCurrentcam->mPreFinZ;
            j++)
          {
            ...
            if (!(p->isInLoadQueue() || p->isLoaded()))
              {
                if ((j >= mCurrentcam->mIniZ) && (j <= mCurrentcam->mFinZ)
                    && (i >= mCurrentcam->mIniX) && (i <= mCurrentcam->mFinX))
                  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::updatePaging(PagingLandScapeCamera* cam)
  {
    mCurrentcam = cam;
    ...
    if (cam->mLastCameraPos != pos
            ...
            || mOptions->cameraThreshold < fabs(cam->mLastCameraPos.z - pos.z)))
      {
        ...
        PagingLandScapePage * const oldPage = getPage(cam->mCurrentCameraPageX,
            cam->mCurrentCameraPageZ, false);
        PagingLandScapeTile * const oldTile = (
            oldPage && oldPage->isLoaded() ?
                oldPage->getTile(cam->mCurrentCameraTileX,
                    cam->mCurrentCameraTileZ) :
                0);
...
//			p->setInQueue(PagingLandScapePage::QUEUE_LOAD);
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::processUnloadQueues()
  {
    ...
    // LIST CHECKS
    for (PagingLandScapePageList::iterator itl = mLoadedPages.begin();
        itl != mLoadedPages.end();)
      {
    ...
    PagingLandScapePage* p = 0;
    for (PagingLandScapeQueue<PagingLandScapePage>::MsgQueType::iterator itq =
        mPageLoadQueue.begin(); itq != mPageLoadQueue.end();)
      {
        assert(!(*itq)->isLoaded());
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::processLoadQueues()
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::removeFromQueues(PagingLandScapePage* p)
  {
    assert(p);
  ...
  //-----------------------------------------------------------------------
  unsigned int
  PagingLandScapePageManager::getCurrentCameraPageX() const
  {
    if (mCurrentcam)
      {
  ...
  //-----------------------------------------------------------------------
  unsigned int
  PagingLandScapePageManager::getCurrentCameraPageZ() const
  {
    if (mCurrentcam)
      {
  ...
  //-----------------------------------------------------------------------
  unsigned int
  PagingLandScapePageManager::getCurrentCameraTileX() const
  {
    if (mCurrentcam)
      {
  ...
  //-----------------------------------------------------------------------
  unsigned int
  PagingLandScapePageManager::getCurrentCameraTileZ() const
  {
    if (mCurrentcam)
      {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::addLoadedPage(PagingLandScapePage* p)
  {
    mLoadedPages.push_back(p);
  }
  //-----------------------------------------------------------------------
  int
  PagingLandScapePageManager::getLoadedPageSize() const
  {
    return static_cast<int>(mLoadedPages.size());
  }
  //-----------------------------------------------------------------------
  int
  PagingLandScapePageManager::getUnloadedPagesSize() const
  {
    return static_cast<int>(mWidth * mHeight - mLoadedPages.size());
  }
  //-----------------------------------------------------------------------
  int
  PagingLandScapePageManager::getPageLoadQueueSize() const
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::getGlobalToPage(Real& x, Real& z) const
  {
    const Real inv_pSize = 1.0f / (mOptions->PageSize - 1);

    x = static_cast<int>(((x / mOptions->scale.x) + mOptions->maxUnScaledX)
        * inv_pSize);
    z = static_cast<int>(((z / mOptions->scale.z) + mOptions->maxUnScaledZ)
        * inv_pSize);
  }

  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::getNearestPageIndicesUnscaled(Real posx,
      Real posz, unsigned int& x, unsigned int& z) const
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePageManager::getNearestTileIndicesUnscaled(Real posx,
      Real posz, unsigned int pagex, unsigned int pagez, unsigned int& x,
      unsigned int& z) const
  {
    ...
    const int tilex = static_cast<int>((posx
        - ((pagex * pSize) - mOptions->maxUnScaledX)) * inv_tSize);
    //- mOptions->maxUnScaledX

    const int tilez = static_cast<int>((posz
        - ((pagez * pSize) - mOptions->maxUnScaledZ)) * inv_tSize);
  ...
  //-----------------------------------------------------------------------
  bool
  PagingLandScapePageManager::getTileIndices(Real posx, Real posz,
      unsigned int pagex, unsigned int pagez, unsigned int& x, unsigned int& z,
      bool alwaysAnswer) const
  {
    if (alwaysAnswer)
      {
        getNearestTileIndicesUnscaled(posx / mOptions->scale.x,
            posz / mOptions->scale.z, pagex, pagez, x, z);
        return true;
      }
    else
      {
        return getRealTileIndicesUnscaled(posx / mOptions->scale.x,
            posz / mOptions->scale.z, pagex, pagez, x, z);
      }
  }
  //-----------------------------------------------------------------------
  PagingLandScapeTile*
  PagingLandScapePageManager::getTileUnscaled(Real posx, Real posz,
      unsigned int pagex, unsigned int pagez, bool alwaysAnswer)
  {
  ...
  //-----------------------------------------------------------------------
  PagingLandScapeTile*
  PagingLandScapePageManager::getTile(Real posx, Real posz, unsigned int pagex,
      unsigned int pagez, bool alwaysAnswer)
  {
    return getTileUnscaled(posx / mOptions->scale.x, posz / mOptions->scale.z,
        pagex, pagez, alwaysAnswer);
  }
  //-----------------------------------------------------------------------
  PagingLandScapeTile*
  PagingLandScapePageManager::getTile(Real posx, Real posz, bool alwaysAnswer)
  {
    return getTileUnscaled(posx / mOptions->scale.x, posz / mOptions->scale.z,
        alwaysAnswer);
  }
    ...
    if (alwaysAnswer)
    ...
    else
      {
        if (getRealPageIndicesUnscaled(posx, posz, pagex, pagez))
          {
            unsigned int tilex, tilez;
            if (getRealTileIndicesUnscaled(posx, posz, pagex, pagez, tilex,
                tilez))
              {
                PagingLandScapePage * const p = getPage(pagex, pagez, false);
                if (p)
  ...
  //-------------------------------------------------------------------------
  PagingLandScapeTile*
  PagingLandScapePageManager::getTilePage(unsigned int& posx,
      unsigned int& posz, unsigned int pagex, unsigned int pagez)
  {
  ...
  //-------------------------------------------------------------------------
  void
  PagingLandScapePageManager::setWorldGeometryRenderQueue(uint8 qid)
  {
  ...
  //-------------------------------------------------------------------------
  void
  PagingLandScapePageManager::eventPageLoaded(PagingLandScapePage* page)
  {
