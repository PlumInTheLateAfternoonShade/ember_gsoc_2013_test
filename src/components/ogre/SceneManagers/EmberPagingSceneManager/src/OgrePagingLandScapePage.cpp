...
namespace Ogre
{
  PagingLandScapePage::PagingLandScapePage(PagingLandScapePageManager& pageMgr) :
      mPageMgr(pageMgr), mPageState(STATE_UNINITED), mPageInQueue(QUEUE_NONE), mTableX(
          0), mTableZ(0), mWorldPosition(Ogre::Vector3::ZERO), mVisible(false), mPageNode(
          0), mRenderable(0), mNumTiles(0)
  {
    touch();
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::init(unsigned int tableX, unsigned int tableZ)
  {
    if (mPageState != STATE_UNINITED)
      {
        S_LOG_WARNING(
            "PagingLandScapePage at (" << tableX << ", " << tableZ << ") already inited, ignoring request");
        return;
      }

    S_LOG_VERBOSE(
        "Initing PagingLandScapePage at (" << tableX << ", " << tableZ << ")");
    ...
    const Real iniX = static_cast<Real>(static_cast<int>(mTableX + mTableX
        - options->world_width)) * 0.5f * factorX + options->position.x;
    const Real iniZ = static_cast<Real>(static_cast<int>(mTableZ + mTableZ
        - options->world_height)) * 0.5f * factorZ + options->position.z;
    const Real endX = iniX + factorX;
    const Real endZ = iniZ + factorZ;
    const Real minHeight = 0.0f;
    const Real maxHeight =
        mPageMgr.getSceneManager()->getData2DManager()->getMaxHeight(mTableX,
            mTableZ);
    ...
    // set bounds
    mBounds.setExtents(iniX, minHeight, iniZ, endX, maxHeight, endZ);
    ...
    if (options->BigImage)
      {
        mRenderable = new PagingLandScapePageRenderable(&mPageMgr,
            mPageNode->getName() + "rend", mTableX, mTableZ, mBounds);
  ...
  PagingLandScapePage::load()
  {
    ...
    if (isLoaded())
      {
        S_LOG_WARNING(
            "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") already loading/loaded, ignoring load() request");
        return;
      }

    S_LOG_VERBOSE(
        "Loading PagingLandScapePage at (" << mTableX << ", " << mTableZ << ")");
    ...
    if (mPageState == STATE_PRELOADED)
      {
        ...
        // load texture
        S_LOG_VERBOSE(
            "Loading texture for PagingLandScapePage at (" << mTableX << ", " << mTableZ << ")");
        ...
        // initialize tiles
        PagingLandScapeTileManager* tileMgr =
            mPageMgr.getSceneManager()->getTileManager();
      ...
      {
        S_LOG_WARNING(
            "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") already loaded, proceeding to show it");
      }
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::preloadInBackground()
  {
    touch();
    ...
    if (mPageState == STATE_INITED)
      {
        ...
        S_LOG_VERBOSE(
            "Preloading PagingLandScapePage at (" << mTableX << ", " << mTableZ << ")");
  ...
  PagingLandScapePage::uninit()
  {
    if (mPageState == STATE_UNINITED)
      {
        S_LOG_WARNING(
            "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") already uninited, ignoring request");
        return;
      }

    S_LOG_VERBOSE(
        "Uniniting PagingLandScapePage at (" << mTableX << ", " << mTableZ << ")");
    ...
    if (isLoaded())
      {
        ...
        // unload texture
        mPageMgr.getSceneManager()->getTextureManager()->unload(mTableX,
            mTableZ);

        // post unload
        mPageMgr.getSceneManager()->getData2DManager()->unload(mTableX,
            mTableZ);
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::setNeighbors()
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::resetNeighbors()
  {
    // update neighbors (tell them that we're gone)
    if (mNeighbors[NORTH])
      {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::setNeighbor(Neighbor n, PagingLandScapePage* p)
  {
    // set page neighbor
    mNeighbors[n] = p;
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::setMapMaterial()
  {
    if (mPageMgr.getOptions()->BigImage)
      {
        mRenderable->setMaterial(
            mPageMgr.getSceneManager()->getTextureManager()->getMapMaterial());
      }
  }
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::touch()
  {
    mTimeUntouched = mPageMgr.getOptions()->PageInvisibleUnloadFrames;
  }
  //-----------------------------------------------------------------------
  bool
  PagingLandScapePage::unloadUntouched()
  {
    if (mTimeUntouched == 0)
      {
        return true;
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::updateTerrain()
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::show(bool do_show)
  {
    if (mPageState != STATE_LOADED)
      {
        S_LOG_WARNING(
            "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") not loaded, ignoring show() request");
        return;
      }

    S_LOG_VERBOSE(
        (do_show ? "Showing" : "Hiding") << " PagingLandScapePage at (" << mTableX << ", " << mTableZ << ")");
    ...
    if (do_show)
    ...
    else
      {
        ...
        if (mPageNode->getParent())
          {
            mPageMgr.getSceneManager()->getRootSceneNode()->removeChild(
                mPageNode->getName());
          }
  ...
  //-----------------------------------------------------------------------
  bool
  PagingLandScapePage::notify(const Vector3& pos,
      const PagingLandScapeCamera* cam)
  {
    if (mPageState == STATE_LOADED)
      {
        touch();
  ...
  //-----------------------------------------------------------------------
  PagingLandScapeTile*
  PagingLandScapePage::getTile(const Vector3& pos) const
  {
    const size_t x = static_cast<unsigned int>(pos.x
        / mPageMgr.getOptions()->scale.x / (mPageMgr.getOptions()->TileSize));
    const size_t z = static_cast<unsigned int>(pos.z
        / mPageMgr.getOptions()->scale.z / (mPageMgr.getOptions()->TileSize));
  ...
  //-------------------------------------------------------------------------
  PagingLandScapeTile*
  PagingLandScapePage::getTile(unsigned int i, unsigned int j) const
  {
    if (mPageState == STATE_LOADED && (i < mTiles.size())
        && (j < mTiles[i].size()))
      {
        return mTiles[i][j];
      }
  ...
  //-------------------------------------------------------------------------
  void
  PagingLandScapePage::setRenderQueue(uint8 qid)
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapePage::fireEvent(EventType type)
  {
    Ogre::PagingLandScapeListenerManager* listenerMgr =
        mPageMgr.getSceneManager()->getListenerManager();
    Ogre::PagingLandScapeData2DManager* data2dMgr =
        mPageMgr.getSceneManager()->getData2DManager();
    ...
    case EVENT_PRELOADED:
      {
        listenerMgr->firePagePreloaded(mTableX, mTableZ,
            data2dMgr->getData2D(mTableX, mTableZ)->getHeightData(), mBounds);
      }
      break;
    case EVENT_LOADED:
      {
  ...
  //-------------------------------------------------------------------------
  void
  PagingLandScapePage::eventData2DLoaded(bool operationOK)
  {
    if (mPageState == STATE_PRELOADING)
      {
        S_LOG_VERBOSE(
            "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") got Data2D loaded");
        if (!operationOK)
          {
            // set new state
            ...
            S_LOG_WARNING(
                "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") is not loadable");
          }
        else
          {
            // set new state
      ...
      {
        S_LOG_WARNING(
            "PagingLandScapePage at (" << mTableX << ", " << mTableZ << ") already preloaded (or not in preloading state), ignoring Data2DLoaded event");
      }
  }
