...
//-----------------------------------------------------------------------
  PagingLandScapeTexture::PagingLandScapeTexture(
      PagingLandScapeTextureManager *textureMgr, const String materialBaseName,
      const unsigned int numTexture, const bool isSplatMode) :
      mParent(textureMgr), mIsLoaded(false), mDataX(0), mDataZ(0)
  {
    mMaterial.setNull();
  }
...
//-----------------------------------------------------------------------
  const Ogre::String&
  PagingLandScapeTexture::getMaterialName() const
  {
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeTexture::bindCompressionSettings()
  {
                ...
                if (opt->VertexCompression)
                  {
                    ...
                    bindCompressionSettings(
                        p->getShadowReceiverVertexProgramParameters());
                  }

                // splat settings.
#ifdef PLSM2_EIHORT
                GpuConstantDefinition const * const e =
                    params->_findNamedConstantDefinition("splatSettings",
                        false);
                if (e)
                  {
                    // use index to get RealConstantEntry
                    params->_writeRawConstant(e->physicalIndex + 0,
                        opt->matHeight[1]);
                    params->_writeRawConstant(e->physicalIndex + 1,
                        opt->matHeight[2]);
                    params->_writeRawConstant(e->physicalIndex + 2,
                        float(opt->maxValue));
...
//-----------------------------------------------------------------------
  ...
  PagingLandScapeTexture::bindCompressionSettings(
      GpuProgramParametersSharedPtr params)
  {
    GpuProgramParameters::AutoConstantIterator aci =
        params->getAutoConstantIterator();
    bool found = false;
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeTexture::_loadMaterial()
  {
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeTexture::_unloadMaterial()
  {
  }

//-----------------------------------------------------------------------
  void
  PagingLandScapeTexture::load(unsigned int x, unsigned int z)
  {
    if (!mIsLoaded)
      {
        mDataX = x;
...
//-----------------------------------------------------------------------
  bool
  PagingLandScapeTexture::isLoaded() const
  {
    return mIsLoaded;
  }
...
//-----------------------------------------------------------------------
  const MaterialPtr&
  PagingLandScapeTexture::getMaterial() const
  {
    return mMaterial;
  }
