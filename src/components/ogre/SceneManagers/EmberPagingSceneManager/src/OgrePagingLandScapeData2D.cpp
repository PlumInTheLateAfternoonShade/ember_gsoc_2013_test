...
namespace Ogre
{
  ...
  PagingLandScapeData2D::PagingLandScapeData2D(
      PagingLandScapeData2DManager* pageMgr) :
      mParent(pageMgr), mHeightData(0), mIsLoaded(false), mIsModified(false), mIsRectModified(
          false), mRect(0, 0, 0, 0, 0, 1)
  {
  }
...
//-----------------------------------------------------------------------
  bool
  PagingLandScapeData2D::load(unsigned int x, unsigned int z)
  {
    if (!mIsLoaded)
      {
        mIsModified = false;
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeData2D::computePowerof2PlusOneSize()
  {
    mSize = mParent->getOptions()->PageSize;
...
//-----------------------------------------------------------------------
  bool
  PagingLandScapeData2D::_checkSize(size_t s)
  {
    // ispow2 - 1
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeData2D::resetDeformationRectangle()
  {
    mRect.left = 0;
    mRect.right = 0;
    mRect.top = 0;
...
//-----------------------------------------------------------------------
  const Image::Box&
  PagingLandScapeData2D::getDeformationRectangle() const
  {
    return mRect;
  }
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeData2D::adjustDeformationRectangle(unsigned int x,
      unsigned int z)
  {
    if (mIsRectModified)
      {
    ...
    PagingLandScapeTile *t =
        mParent->getSceneManager()->getPageManager()->getTilePage(tileposx,
            tileposz, mPageX, mPageZ);
    if (t && t->isLoaded())
      {
...
//-----------------------------------------------------------------------
  bool
  PagingLandScapeData2D::deformHeight(const Vector3& deformationPoint,
      Real& modificationHeight)
  {
    ...
    const unsigned int ux = static_cast<unsigned int>(std::max(
        std::min(x, size), 0));
    const unsigned int uz = static_cast<unsigned int>(std::max(
        std::min(z, size), 0));
...
//-----------------------------------------------------------------------
  bool
  PagingLandScapeData2D::deformHeight(unsigned int x, unsigned int z,
      Real& modificationHeight)
  {
...
//-----------------------------------------------------------------------
  bool
  PagingLandScapeData2D::setHeight(unsigned int x, unsigned int z,
      unsigned int Pos, const Real& h)
  {
                ...
                if (rn)
                  {
                    if (down)
                      {
                        PagingLandScapeRenderable * const rnUp =
                            rn->_getNeighbor(NORTH);
                      ...
                      {
                        PagingLandScapeRenderable * const rnDown =
                            rn->_getNeighbor(SOUTH);
...
//-----------------------------------------------------------------------
  Vector3
  PagingLandScapeData2D::getNormal(const Real& mX, const Real& mZ) const
  {
    ...
    const Real Divider = static_cast<Real>(pageSize)
        / mParent->getOptions()->scale.y;

    assert(mHeightData);

#define  getisIn(a, b) (mHeightData[static_cast<unsigned int> (a) + static_cast<unsigned int> (b) * mSize])

    if (mX > 0 && mZ > 0 && mX < pageSize && mZ < pageSize)
      {
