...
namespace Ember
{
  namespace OgreView
  {
    ...
    EmberPagingLandScapeTexture::EmberPagingLandScapeTexture(
        Ogre::PagingLandScapeTextureManager* pageMgr) :
    ...
    EmberPagingLandScapeTexture::newTexture()
    {
      return new EmberPagingLandScapeTexture(mParent);
    }

    bool
    EmberPagingLandScapeTexture::isMaterialSupported(bool recursive)
    {
    ...
    void
    EmberPagingLandScapeTexture::setOptions()
    {
    }

    void
    EmberPagingLandScapeTexture::_loadMaterial()
    {
      EmberPagingSceneManager* emberPagingSceneManager =
          static_cast<EmberPagingSceneManager*>(mParent->getSceneManager());
      IPageDataProvider* provider = emberPagingSceneManager->getProvider();
      if (provider)
        {
          std::unique_ptr<IPageData> pageData(
              provider->getPageData(
                  IPageDataProvider::OgreIndex(mDataX, mDataZ)));
          mMaterial = pageData->getMaterial();
        }
    }

    void
    EmberPagingLandScapeTexture::_unloadMaterial()
    {
