...
namespace Ember
{
  namespace OgreView
  {
    ...
    const Ogre::String EmberPagingSceneManagerFactory::FACTORY_TYPE_NAME =
        "EmberPagingSceneManager";
    //-----------------------------------------------------------------------
    void
    EmberPagingSceneManagerFactory::initMetaData(void) const
    {
    ...
    //-----------------------------------------------------------------------
    Ogre::SceneManager*
    EmberPagingSceneManagerFactory::createInstance(
    ...
    //-----------------------------------------------------------------------
    void
    EmberPagingSceneManagerFactory::destroyInstance(
        Ogre::SceneManager* instance)
    {
      delete instance;
    }

    EmberPagingSceneManager::EmberPagingSceneManager(const Ogre::String &name) :
        PagingLandScapeSceneManager(name)
    {
      if (!mOptions)
    ...
    //-----------------------------------------------------------------------
    void
    EmberPagingSceneManager::InitScene()
    {
      PagingLandScapeSceneManager::InitScene();

      getData2DManager()->registerDataType(
          new EmberPagingLandScapeData2D_HeightField(getData2DManager()));
      getTextureManager()->registerTextureType(
          new EmberPagingLandScapeTexture(getTextureManager()));

    }

    void
    EmberPagingSceneManager::registerProvider(IPageDataProvider* provider)
    {
      mProvider = provider;
    }

    IPageDataProvider*
    EmberPagingSceneManager::getProvider()
    {
      return mProvider;
    }

    const Ogre::String&
    EmberPagingSceneManager::getTypeName(void) const
    {
      return EmberPagingSceneManagerFactory::FACTORY_TYPE_NAME;
    }

    const Ogre::Pass*
    EmberPagingSceneManager::deriveShadowCasterPass(const Ogre::Pass* pass)
    {
      ...
      if (isShadowTechniqueTextureBased())
        {
          ...
          Ogre::MaterialPtr shadowCasterMaterial =
              pass->getParent()->getShadowCasterMaterial();
          if (!shadowCasterMaterial.isNull()
              && shadowCasterMaterial->getBestTechnique())
            {
              customMaterial = true;
              retPass =
                  pass->getParent()->getShadowCasterMaterial()->getBestTechnique()->getPass(
                      0);
              if (!retPass)
                {
                  return pass;
          ...
          if ((pass->getSourceBlendFactor() == SBF_SOURCE_ALPHA
              ...
              || pass->getAlphaRejectFunction() != CMPF_ALWAYS_PASS)
            {
              ...
              retPass->setSceneBlending(pass->getSourceBlendFactor(),
                  pass->getDestBlendFactor());
              retPass->getParent()->getParent()->setTransparencyCastsShadows(
                  true);
              ...
              for (unsigned short t = 0; t < origPassTUCount; ++t)
                {
                  ...
                  tex->setColourOperationEx(LBX_SOURCE1, LBS_MANUAL,
                      LBS_CURRENT,
                      isShadowTechniqueAdditive() ?
                          ColourValue::Black : mShadowColour);
