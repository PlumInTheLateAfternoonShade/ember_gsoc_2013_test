...
// Supplied by Praetor. Thanks a lot. ]:)
  void
  PagingLandScapeRaySceneQuery::execute(RaySceneQueryListener* listener)
  {
    //Ember start
    //Make sure that there really is some world geometry first
    if ((static_cast<PagingLandScapeSceneManager*>(mParentSceneMgr)->mWorldGeomIsSetup)
        && (getQueryTypeMask() & SceneManager::WORLD_GEOMETRY_TYPE_MASK))
    //Ember end
      {
        ...
        PagingLandScapeSceneManager* mSceneMgr =
            static_cast<PagingLandScapeSceneManager*>(mParentSceneMgr);
        if (mWorldFragmentType & WFT_SINGLE_INTERSECTION)
            ...
            if (dir == Vector3::UNIT_Y || dir == Vector3::NEGATIVE_UNIT_Y)
              {
                ...
                if (mSceneMgr->getOptions()->queryNoInterpolation)
                  height = mSceneMgr->getData2DManager()->getWorldHeight(
                      origin.x, origin.z);
                else
                  height =
                      mSceneMgr->getData2DManager()->getInterpolatedWorldHeight(
                          origin.x, origin.z);
                ...
                mWorldFrag.singleIntersection +=
                    mSceneMgr->getOptions()->position; //consider terrain offset

                listener->queryResult(&mWorldFrag,
                    (Math::Abs(mWorldFrag.singleIntersection.y - origin.y)));
                return;
              }
            else if (mSceneMgr->intersectSegmentTerrain(origin,
        ...
        else
          {
            ...
            const Vector3 raymove(
                raydir * mSceneMgr->getOptions()->queryResolutionFactor);
            const Real distmove = mSceneMgr->getOptions()->queryResolutionFactor;
            const Real maxHeight =
                mSceneMgr->getData2DManager()->getMaxHeight();
                    ...
                    if (ray.y < land.y)
                      {
                        ...
                        frag->fragmentType =
                            SceneQuery::WFT_SINGLE_INTERSECTION;
...
//----------------------------------------------------------------------------
  Vector3
  PagingLandScapeRaySceneQuery::getHeightAt(const Vector3& origin) const
  {

    PagingLandScapeSceneManager * mSceneMgr =
        static_cast<PagingLandScapeSceneManager*>(mParentSceneMgr);
    return Vector3(origin.x,
        mSceneMgr->getData2DManager()->getInterpolatedWorldHeight(origin.x,
            origin.z), origin.z);
