...
namespace Ogre
{
  ...
  PagingLandScapeTileManager::PagingLandScapeTileManager(
      ...
      mSceneManager(scnMgr), mOptions(scnMgr->getOptions())
  {
    mNumTiles = 0;
...
//-----------------------------------------------------------------------
  PagingLandScapeTileManager::~PagingLandScapeTileManager()
  {
    ...
    std::for_each(mTiles.begin(), mTiles.end(), delete_object());
    // destroy tiles
    mTiles.clear();
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeTileManager::clear()
  {
    assert(
        mTiles.size() == (unsigned int) mQueue.getSize() && mTiles.size() == mNumTiles);
  }
//-----------------------------------------------------------------------
  void
...
//-----------------------------------------------------------------------
  PagingLandScapeTile *
  PagingLandScapeTileManager::getTile()
  {
    if (mQueue.empty())
      {
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeTileManager::freeTile(PagingLandScapeTile *tile)
  {
...
//-----------------------------------------------------------------------
  unsigned int
  PagingLandScapeTileManager::numTiles(void) const
  {
    return mNumTiles;
  }
//-----------------------------------------------------------------------
  size_t
  PagingLandScapeTileManager::numFree() const
  {
    return mQueue.getSize();
  }
//-----------------------------------------------------------------------
  void
  PagingLandScapeTileManager::_addBatch(const unsigned int num)
  {
    mNumTiles += num;
...
//-----------------------------------------------------------------------
  void
  PagingLandScapeTileManager::unloadUntouched()
  {
    std::for_each(mTiles.begin(), mTiles.end(),
