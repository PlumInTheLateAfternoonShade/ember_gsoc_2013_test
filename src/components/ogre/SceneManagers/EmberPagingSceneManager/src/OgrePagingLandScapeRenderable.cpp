...
#define TEXTURE_BINDING		1
  ...
  PagingLandScapeRenderable::PagingLandScapeRenderable(
      PagingLandScapeRenderableManager *renderableMgr) :
      Renderable(), MovableObject(), mParent(renderableMgr), mCurrVertexes(0), mInfo(
          0), mMaterialLODIndex(0), mInUse(false), mIsLoaded(false), mDeltaBuffers(
          0), mLastNextLevel(-1), mLODMorphFactor(0.0f), mHeightfield(0), mMinLevelDistSqr(
          0), mRenderLevel(-1), mIsRectModified(false), mRect(0, 0, 0, 0, 0, 1), mForcedMaxLod(
          false)
          //Ember added start
          , mNeedReload(false), mIsFrameListener(false)
  //Ember added stop

  {
    ...
    HardwareVertexBufferSharedPtr vbuf =
        HardwareBufferManager::getSingleton().createVertexBuffer(
            decl->getVertexSize(MAIN_BINDING), mCurrVertexes->vertexCount,
    ...
    mCustomGpuParameters = Vector4(mParent->getOptions()->ScaledPageSizeX,
        ...
        mParent->getOptions()->ScaledPageSizeZ, mLODMorphFactor);
  ...
  //------------------------------------------------------------------------
  void
  PagingLandScapeRenderable::uninit()
  {
    if (mIsLoaded)
      unload();
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::init(PagingLandScapeTileInfo* info)
  {
    mMinLevelDistSqr = 0;
    mQueued = false;
    mParentTile = 0;
  ...
  PagingLandScapeRenderable::load()
  {
    ...
    PagingLandScapeData2D* data =
        mParent->getSceneManager()->getData2DManager()->getData2D(mInfo->mPageX,
            mInfo->mPageZ);
...
//                +  " | " + StringConverter::toString(mRect.top) + " - " + StringConverter::toString(mRect.bottom));
    ...
    const unsigned int offSetX = static_cast<unsigned int>(mInfo->mTileX
        * (tileSize - 1) + mRect.left);
    const unsigned int endx = static_cast<unsigned int>(offSetX
        + (mRect.right - mRect.left));

    const unsigned int offSetZ = static_cast<unsigned int>(mInfo->mTileZ
        * (tileSize - 1) + mRect.top);
    const unsigned int endz = static_cast<unsigned int>(offSetZ
    ...
    uchar* pMain = static_cast<uchar*>(vVertices->lock(
        VertexTileSize * mRect.top + mRect.left * VertexSize, // offset
        VertexTileSize * (mRect.bottom - mRect.top)
            - (tileSize - mRect.right) * VertexSize, //length
            ...
            if (b_colored)
              {
                ...
                Root::getSingleton().convertColourValue(RGBA_precalc, pColor);
              }
    ...
    if (!mIsRectModified)
      {
        mBounds.setExtents(offSetX * scale_x, min, offSetZ * scale_z,
            endx * scale_x, max, endz * scale_z);
      } // if (!mIsRectModified) 
    else
      {
        const Vector3& maxbound(mBounds.getMaximum());
        const Vector3& minbound(mBounds.getMinimum());
        mBounds.setExtents(minbound.x, min, minbound.z, maxbound.x, max,
            maxbound.z);
      }
    mBoundingRadius = Math::Sqrt(
        Math::Sqr(max - min) + Math::Sqr((endx - 1 - offSetX) * scale_x)
            + Math::Sqr((endz - 1 - offSetZ) * scale_z)) / 2;

    if (mParent->getOptions()->VisMap)
      mParent->getSceneManager()->getHorizon()->registerMinMaxHeightTile(mInfo,
          min, max);
...
//         getWorldBoundingBox(true);
    if (!mIsLoaded)
      {
        mParent->getSceneManager()->getListenerManager()->fireTileLoaded(
            mInfo->mPageX, mInfo->mPageZ, mInfo->mTileX, mInfo->mTileZ,
            mParentTile->getWorldBbox());
        mIsLoaded = true;
      }
    else if (mNeedReload)
      {
        mParent->getSceneManager()->getListenerManager()->fireTileDeformed(
            mInfo->mPageX, mInfo->mPageZ, mInfo->mTileX, mInfo->mTileZ,
            mParentTile->getWorldBbox());
        mNeedReload = false;
      }
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::unload()
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::_notifyCurrentCamera(Camera* cam)
  {
        ...
        if (mParent->getOptions()->primaryCamera == cam || mRenderLevel == -1)
          {
            ...
            if (mParent->getOptions()->VisMap)
              {
                PagingLandScapeCamera* plscam =
                    static_cast<PagingLandScapeCamera*>(cam);
                if (!mParent->getSceneManager()->getHorizon()->IsTileVisible(
                    plscam, mInfo))
                  {
                    mVisible = false;
                    return;
            ...
            if (mForcedMaxLod)
            ...
            else
              {
                ...
                mDistanceToCam =
                    (getWorldPosition() - cam->getDerivedPosition()).squaredLength()
                        * cam->getLodBias();

                // Material LOD
                if (!mMaterial.isNull()
                    && mMaterial->getNumLodLevels(
                        MaterialManager::getSingleton()._getActiveSchemeIndex())
                        > 1)
                  {
                    const unsigned short LODIndex = mMaterial->getLodIndex(
                        mDistanceToCam);
                ...
                mRenderLevel = -1;
                const int maxMipmap =
                    static_cast<int>(mParent->getOptions()->maxRenderLevel);
                ...
                if (mParent->getOptions()->lodMorph)
                  {
                    ...
                    if (nextLevel == 0)
                    ...
                    else
                      {
                        ...
                        // the distance range
                        const Real range = (*mMinLevelDistSqr)[nextLevel]
                            - (*mMinLevelDistSqr)[mRenderLevel];
                        if (range)
                          {
                            const Real percent = (mDistanceToCam
                                - (*mMinLevelDistSqr)[mRenderLevel]) / range;
                            // scale result so that msLODMorphStart == 0, 1 == 1, clamp to 0 below that
                            const Real rescale = 1.0f
                                / (1.0f - mParent->getOptions()->lodMorphStart);
                            mLODMorphFactor = std::max(
                                (percent - mParent->getOptions()->lodMorphStart)
                                    * rescale, static_cast<Real>(0.0f));
                            mCustomGpuParameters.w = mLODMorphFactor;
                    ...
                    if (mLastNextLevel != nextLevel)
                      {
                        ...
                        if (nextLevel > 1)
                          {
                            mCurrVertexes->vertexBufferBinding->setBinding(
                                DELTA_BINDING, mDeltaBuffers[nextLevel - 1]);
                          }
                        else
                          {
                            // bind dummy (in case bindings checked)
                            mCurrVertexes->vertexBufferBinding->setBinding(
                                DELTA_BINDING, mDeltaBuffers[0]);
                          }
                      }
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::update()
  {
    mChangedRenderLevel = true;
  }
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::_updateRenderQueue(RenderQueue* queue)
  {
  ...
  //-----------------------------------------------------------------------
  Technique*
  PagingLandScapeRenderable::getTechnique(void) const
  {
    return mMaterial->getBestTechnique(mMaterialLODIndex);
  }
  //-----------------------------------------------------------------------
  const LightList&
  PagingLandScapeRenderable::getLights(void) const
  {
    if (mLightListDirty)
      {
        getParentSceneNode()->getCreator()->_populateLightList(mCenter,
            this->getBoundingRadius(), mLightList);
  ...
  //-----------------------------------------------------------------------
  Real
  PagingLandScapeRenderable::getSquaredViewDepth(const Camera* cam) const
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::getRenderOperation(RenderOperation& op)
  {
    assert(mIsLoaded);
    assert(mInUse);
    assert(mVisible);
    ...
    if (mChangedRenderLevel)
      {
        mIndex =
            mParent->getSceneManager()->getIndexBufferManager()->getIndexData(
                mRenderLevel, mNeighbors);
        mChangedRenderLevel = false;
      }
    assert(mIndex);
    op.indexData = mIndex;
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::getWorldTransforms(Matrix4* xform) const
  {
    *xform = mParentNode->_getFullTransform();
  }
  //-----------------------------------------------------------------------
  const Quaternion&
  PagingLandScapeRenderable::getWorldOrientation(void) const
  {
    return mParentNode->_getDerivedOrientation();
  }
  //-----------------------------------------------------------------------
  const Vector3&
  PagingLandScapeRenderable::getWorldPosition(void) const
  {
    return mCenter;
  }
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::setMaterial(const MaterialPtr &mat)
  {
    mMaterial = mat;
  }
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::_calculateMinLevelDist2(const Real C)
  {
...
// should change if we're just refilling a part
    ...
    const int offSetX = static_cast<int>(mInfo->mTileX * (tileSize - 1)
        + mRect.left);
    const int endx = static_cast<int>(offSetX
        + static_cast<int>(mRect.right - mRect.left));

    const int offSetZ = static_cast<int>(mInfo->mTileZ * (tileSize - 1)
        + mRect.top);
    ...
    const Real * const ogre_restrict heightField = mHeightfield
        + offSetZ * pageSize;
            ...
            for (i = offSetX; i < endx; i++)
              {
                ...
                BaseHeight[k] = static_cast<short>((heightField[i
                    + K_heightFieldPos] * inv_scale) - 32768);
...
// should change to an interval locking (as in load) ? if we're just refilling a part to whole tile 
            pDeltaInit =
                static_cast<ushort*>(mDeltaBuffers[lvl_minus_one]->lock(
...
// even with offset that would make us in "middle of a step"
                ...
                bool isInValidVertex;
                  {
                    ...
                    const Vector3 v2(i + step,
                        heightField[i + step + K_heightFieldPos], j);
                    const Vector3 v3(i,
                        heightField[i + K_heightFieldPos + ZShift], j + step);
                    const Vector3 v4(i + step,
                        heightField[i + step + K_heightFieldPos + ZShift],
                        j + step);
                    ...
                    isInValidVertex = v1.y == uninitializedHeight
                        || v2.y == uninitializedHeight
                        || v3.y == uninitializedHeight
                        || v4.y == uninitializedHeight;
                  }
                ...
                if (!isInValidVertex)
                  {
                    ...
                    const int zubound = (
                        j == (tilesizeMinusstepZ) ? step : step - 1);
                        ...
                        const int zPageSize = z * pageSize;
                          {
                            ...
                            const int xubound = (
                                i == (tilesizeMinusstepX) ? step : step - 1);
                            for (int x = 0; x <= xubound; x++)
                              {
                                const int fulldetailx = i + x;

                                if (isFullDetailZ && fulldetailx % step == 0)
                                  {
                                ...
                                if (xpct + zpct <= 1.0f)
                                  {
                                    // Solve for x/z
                                    interp_h = (-(t1.normal.x * fulldetailx)
                                        - t1.normal.z * fulldetailz - t1.d)
                                        / t1.normal.y;
                                  }
                                else
                                  {
                                    // Second triangle
                                    interp_h = (-(t2.normal.x * fulldetailx)
                                        - t2.normal.z * fulldetailz - t2.d)
                                        / t2.normal.y;
                                  }

                                assert(
                                    (fulldetailx + K_heightFieldPos + zPageSize) < (pageSize*pageSize));

                                const Real actual_h = heightField[fulldetailx
                                    + K_heightFieldPos + zPageSize];
                                ...
                                if (lodMorph)
                                  {
                                    ...
                                    if (delta != 0.0f)
                                      {
                                        const int tileposx = fulldetailx
                                            - offSetX;
                                        const int tileposy = fulldetailz
                                            - offSetZ;
                                        ...
                                        if (tileposx != 0
                                            ...
                                            && tileposy != (tilesize - 1))
                                          {
                                            ...
                                            assert(
                                                (tileposx + (tileposy * tilesize))*blendWeights < size);

                                            pDeltas[(tileposx
                                                + (tileposy * tilesize))
                                                * blendWeights] =
                                                static_cast<short>((interp_h
                                                    * inv_scale) - 32768);
    ...
    if (doComputeMinLevelDistSqr)
      {
        if (roughnessLod)
          ...
          {
            const int maxMip =
                static_cast<int>(mParent->getOptions()->maxRenderLevel) - 1;
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::fillNextLevelDown(void)
  {
  ...
  PagingLandScapeRenderable::createDeltaBuffer(void) const
  {
  ...
  //-----------------------------------------------------------------------
  Real
  PagingLandScapeRenderable::_vertex(const int x, const int z,
      const int n) const
  {
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::_updateCustomGpuParameter(
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::adjustDeformationRectangle(unsigned int x,
      unsigned int z)
  {
    assert(
        x < mParent->getOptions ()->TileSize && z < mParent->getOptions ()->TileSize);
    if (mIsRectModified)
      {
        if (mRect.left > x)
          mRect.left = x;
  ...
  //-----------------------------------------------------------------------
  uint32
  PagingLandScapeRenderable::getTypeFlags(void) const
  {
    // return world flag
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeRenderable::_notifyAttached(Node* parent, bool isTagPoint)
  {
    ...
    if (parent)
      {
        ...
        mWorldBoundingSphere.setRadius(
            (mBounds.getMaximum() - mBounds.getMinimum()).length() / 2);
        mParentNode->needUpdate();
      }
  }
  //-----------------------------------------------------------------------
  Vector3
  PagingLandScapeRenderable::_getvertex(const int x, const int z) const
  {
    const unsigned int tilesize = mParent->getOptions()->TileSize - 1;
    ...
    vertex.y = mHeightfield[static_cast<unsigned int>(vertex.x
        + (vertex.z * mParent->getOptions()->PageSize))];
   ...
   */
  IndexData*
  PagingLandScapeRenderable::getRawIndexData(const int renderlevel)
  {
    return mParent->getSceneManager()->getIndexBufferManager()->getRawIndexes(
        renderlevel);
  }
   ...
   */
  void
  PagingLandScapeRenderable::getRawVertexData(Vector3* pVerts)
  {
   ...
   */
  const unsigned int
  PagingLandScapeRenderable::getVertexCount()
  {
  ...
  //ember addition start
  bool
  PagingLandScapeRenderable::frameStarted(const Ogre::FrameEvent& event)
  {
  ...
  void
  PagingLandScapeRenderable::visitRenderables(Renderable::Visitor* visitor,
