...
namespace Ogre
{
  ...
  Occlusion::Occlusion(unsigned int visibilityThsd) :
      mCurrentCam(0), mVisibilityTreshold(visibilityThsd), mIsQueryPoolNotInitiated(
          true), mFrameConservativeVisibility(300)
  {
  }
  ;
  //-----------------------------------------------------------------------
  bool
  Occlusion::nextFrame(PagingLandScapeOctreeCamera *cam,
      MovableObjectList *camInProgressVisibles, bool onlyshadowcaster,
      RenderQueue *q)
  {
    bool newframe = true;
  ...
  //-----------------------------------------------------------------------
  void
  Occlusion::queueDraw(PagingLandScapeOctreeNode& n)
  {
#ifdef _VISIBILITYDEBUG
    object_cnt++;
#endif //_VISIBILITYDEBUG  
  ...
  //-----------------------------------------------------------------------
  bool
  Occlusion::issueDrawQuery(PagingLandScapeOctreeNode& n)
  {
    VisibilityData * const vis = n.getNodeData(mCurrentCam);
    ...
    MovableObjectList *moToRender = n.getVisibleNotifiedNodeObjects(mCurrentCam,
        mOnlyShadowCaster);
    ...
    if (!moToRender->empty())
      {
        ...
        MovableObjectList::iterator it = moToRender->begin(), itend =
            moToRender->end();
        while (it != itend)
          {
  ...
  //-----------------------------------------------------------------------
  void
  Occlusion::issueBboxQuery(OcclusionElement& n)
  {
#ifdef _VISIBILITYDEBUG
    query_cnt++;
#endif //_VISIBILITYDEBUG    
  ...
  //-----------------------------------------------------------------------
  bool
  Occlusion::isQueryResultIsVisible(OcclusionElement& node)
  {
  ...
  //-----------------------------------------------------------------------
  void
  Occlusion::pullUpVisibility(OcclusionElement& node)
  {
  ...
  //-----------------------------------------------------------------------
  bool
  Occlusion::insideViewFrustum(OcclusionElement& n)
  {
  ...
  void
  Occlusion::CHCtraversal(PagingLandScapeOctree *octree,
      VisibleObjectsBoundsInfo * const visibleBounds)
  {
    ...
    FrontToBackNodeSorterPriorityQueue traversalStack =
        FrontToBackNodeSorterPriorityQueue(
            FrontToBackNodeSorterOperator(camPos));
                    ...
                    if (wasVisible && node.isOccluder())
                      {
                        ...
                        if (issueDrawQuery(
                            static_cast<PagingLandScapeOctreeNode &>(node)))
                          {
  ...
  void
  Occlusion::CHCtraversalConservative(PagingLandScapeOctree *octree,
      VisibleObjectsBoundsInfo * const visibleBounds)
  {

    const unsigned int frameConservativeVisibility =
        mFrameConservativeVisibility;
    const unsigned int visibilityTreshold = mVisibilityTreshold;
            ...
            if (insideViewFrustum(node))
              {
                ...
                const bool wasVisibleandQueriedBeforeLastNFrame = wasVisible
                    && lastframeId - frameConservativeVisibility
                        < vis->lastQueryFrameId;
                if (wasVisibleandQueriedBeforeLastNFrame)
                  {
                    if (node.isLeaf() && node.isOccluder())
                      {
                        MovableObjectList *moToRender =
                            static_cast<PagingLandScapeOctreeNode *>(&node)->getVisibleNotifiedNodeObjects(
                                mCurrentCam, mOnlyShadowCaster);
                        // create the render Queue.
                        if (!moToRender->empty())
                          {
                            MovableObjectList::iterator it =
                                moToRender->begin(), itend = moToRender->end();
                            while (it != itend)
                              {
                                (*it)->_updateRenderQueue(
                                    &*mCurrentRenderQueue);
                                ++it;
                              }
                            mScnMngr->directRenderSingleQueue(
                                &*mCurrentRenderQueue);
                          }
                        vis->notified = true;
                      }
                  ...
                  {
                    const bool wasVisibleandQueriedExacltyLastNFrame =
                        wasVisible
                            && lastframeId - frameConservativeVisibility
                                == vis->lastQueryFrameId;
                    if (node.isLeaf())
