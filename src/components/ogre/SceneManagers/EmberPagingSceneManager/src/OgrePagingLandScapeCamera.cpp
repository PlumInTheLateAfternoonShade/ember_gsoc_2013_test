...
{
  //-----------------------------------------------------------------------
  PagingLandScapeCamera::PagingLandScapeCamera(const String& name,
      SceneManager* sm) :
      PagingLandScapeOctreeCamera(name, sm),

      mCurrentCameraPageX(0), mCurrentCameraPageZ(0), mCurrentCameraTileX(0), mCurrentCameraTileZ(
          0), mIniX(0), mFinX(0), mIniZ(0), mFinZ(0), mPreIniX(0), mPreFinX(0), mPreIniZ(
          0), mPreFinZ(0), mLastCameraPos(
          Vector3(std::numeric_limits<Real>::max(), 0.0f,
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeCamera::resetPaging()
  {
    ...
    mPreIniZ = 0;
    mPreFinZ = 0;
    mLastCameraPos = Vector3(std::numeric_limits<Real>::max(), 0.0f,
        std::numeric_limits<Real>::max());
    updatePaging(0, 0);
...
//        mLastCameraPos.z = dist;
  ...
  //-----------------------------------------------------------------------
  void
  PagingLandScapeCamera::updatePaging(const unsigned int x,
      const unsigned int z)
  {
    ...
    if (lx + adjpages < w)
    ...
    else
      {
        mFinX = w - 1;
        mIniX =
            (static_cast<int>(mIniX + (w - (lx + adjpages))) > 0) ?
                mIniX + (w - (lx + adjpages)) : 0;
      }
    ...
    if (lz + adjpages < h)
    ...
    else
      {
        mFinZ = h - 1;
        mIniZ =
            (static_cast<int>(mIniZ + (h - (lz + adjpages))) > 0) ?
                mIniZ + (h - (lz + adjpages)) : 0;
      }
    ...
    if (x + prepages < w)
    ...
    else
      {
        mPreFinX = w - 1;
        mPreIniX =
            (static_cast<int>(mPreIniX + (w - (lx + prepages))) > 0) ?
                mPreIniX + (w - (lx + prepages)) : 0;
      }
    ...
    if (lz + prepages < h)
    ...
    else
      {
        mPreFinZ = h - 1;
        mPreIniZ =
            (static_cast<int>(mPreIniZ + (h - (lz + prepages))) > 0) ?
                mPreIniZ + (h - (lz + prepages)) : 0;
      }
  ...
  bool
  PagingLandScapeCamera::isVisible(const AxisAlignedBox &bound) const
  {
    ...
    for (unsigned int plane = 0; plane < 6; ++plane)
      {
        ...
        const Plane::Side side = mFrustumPlanes[plane].getSide(centre,
            halfSize);
