...
//---------------------------------------------------------------------
  PagingLandScapeOctreeIntersectionSceneQuery::PagingLandScapeOctreeIntersectionSceneQuery(
...
//---------------------------------------------------------------------
  void
  PagingLandScapeOctreeIntersectionSceneQuery::execute(
      IntersectionSceneQueryListener* listener)
  {
        ...
        while (it.hasMoreElements())
          {
            ...
            static_cast<PagingLandScapeOctreeSceneManager*>(mParentSceneMgr)->findNodesIn(
                e->getWorldBoundingBox(), list, 0);
            ...
            while (itscn != list.end())
              {
                SceneNode::ObjectIterator oit =
                    (*itscn)->getAttachedObjectIterator();
                while (oit.hasMoreElements())
                  {
                    ...
                    if (m != e && set.find(MovablePair(e, m)) == set.end()
                        ...
                        && m->isInScene()
                        && e->getWorldBoundingBox().intersects(
                            m->getWorldBoundingBox()))
                      {
                        ...
                        if (m->getMovableType() == "Entity")
                          {
                            ...
                            Entity::ChildObjectListIterator childIt =
                                e2->getAttachedObjectIterator();
                            while (childIt.hasMoreElements())
                              {
                                MovableObject* c = childIt.getNext();
                                if (c->getQueryFlags() & mQueryMask
                                    && e->getWorldBoundingBox().intersects(
                                        c->getWorldBoundingBox()))
                                  {
