...
namespace Ogre
{
  ...
  public:
    ...
    PagingLandScapeTile*
    getTileUnscaled(Real posx, Real posz, bool alwaysAnswer);
    ...
    PagingLandScapeTile*
    getTile(Real posx, Real posz, unsigned int pagex, unsigned int pagez,
        bool alwaysAnswer);
    ...
    PagingLandScapeTile*
    getTileUnscaled(Real posx, Real posz, unsigned int pagex,
        unsigned int pagez, bool alwaysAnswer);
    /// get Tile at relative position but unscaled by landscape scale (in the page containing the tile page coordinate system) in space, return answer even when position is outside landscape when alwaysAnswer is true
    PagingLandScapeTile*
    getTilePage(unsigned int& posx, unsigned int& posz, unsigned int pagex,
        unsigned int pagez);

    void
    getGlobalToPage(Real& x, Real& z) const;
     ...
     */
    inline bool
    getPageIndices(Real posx, Real posz, unsigned int& x, unsigned int& z,
        bool alwaysAnswer) const
    {
      if (alwaysAnswer)
        {
          getNearestPageIndicesUnscaled(posx * mOptions->invScale.x,
              posz * mOptions->invScale.z, x, z);
          return true;
        }
      else
        {
          return getRealPageIndicesUnscaled(posx * mOptions->invScale.x,
              posz * mOptions->invScale.z, x, z);
        }
    }
     ...
     */
    inline bool
    getRealPageIndicesUnscaled(Real posx, Real posz, unsigned int& x,
        unsigned int& z) const
    {
      const Real lx = ((posx + mOptions->maxUnScaledX)
          * mOptions->invPageSizeMinusOne);
      const Real lz = ((posz + mOptions->maxUnScaledZ)
          * mOptions->invPageSizeMinusOne);
     ...
     */
    void
    getNearestPageIndicesUnscaled(Real posx, Real posz, unsigned int& x,
        unsigned int& z) const;
     ...
     */
    bool
    getTileIndices(Real posx, Real posz, unsigned int pagex, unsigned int pagez,
     ...
     */
    inline bool
    getRealTileIndicesUnscaled(Real posx, Real posz, unsigned int pagex,
        unsigned int pagez, unsigned int& x, unsigned int& z) const
    {
      // adjust x and z to be local to page
      const int pSize = mOptions->PageSize - 1;
      const int tilex = static_cast<int>((posx
          - ((pagex * pSize) - mOptions->maxUnScaledX))
          * mOptions->invTileSizeMinusOne);
      const int tilez = static_cast<int>((posz
          - ((pagez * pSize) - mOptions->maxUnScaledZ))
          * mOptions->invTileSizeMinusOne);
      ...
      if (tilex > tilesPerPage || tilex < 0 || tilez > tilesPerPage
          || tilez < 0)
        {
          return false;
        }
     ...
     */
    void
    getNearestTileIndicesUnscaled(Real posx, Real posz, unsigned int pagex,
        unsigned int pagez, unsigned int& x, unsigned int& z) const;

    void
    setTerrainReady(bool isready)
    {
      mTerrainReady = isready;
    }

    void
    removeFromQueues(PagingLandScapePage* p);
    ...
    void
    setWorldGeometryRenderQueue(uint8 qid);
    RenderQueueGroupID
    ...
    /// getter
    unsigned int
    getCurrentCameraPageX() const;
    /// getter
    unsigned int
    getCurrentCameraPageZ() const;
    /// getter
    unsigned int
    getCurrentCameraTileX() const;
    /// getter
    unsigned int
    getCurrentCameraTileZ() const;
    /// getter
    int
    getPagePreloadQueueSize() const;
    /// getter
    int
    getPageLoadQueueSize() const;
    /// getter
    int
    getLoadedPageSize() const;
    /// getter
    int
    getPreLoadedPageSize() const;
    /// getter
    int
    getUnloadedPagesSize() const;
    ...
    void
    eventPageLoaded(PagingLandScapePage* page);
  ...
  protected:
    ...
    PagingLandScapePage*
    find_nearest(const Vector3& pos, unsigned int x, unsigned int z,
        PagingLandScapePageList& mQueue) const;
    ...
    void
    releasePage(PagingLandScapePage* p);
