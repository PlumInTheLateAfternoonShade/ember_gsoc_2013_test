...
namespace Ogre
{
  ...
  public:
    ...
    clear(void);
    void
    WorldDimensionChange(void);
    PagingLandScapeTexture*
    allocateTexture() const;
    void
    reset(void);

    void
    load(const unsigned int texX, const unsigned int texZ);
    void
    reload(const unsigned int dataX, const unsigned int dataZ);
    void
    unload(const unsigned int texX, const unsigned int texZ);

    bool
    isLoaded(const unsigned int texX, const unsigned int texZ);

    const MaterialPtr&
    getMaterial(const unsigned int texX, const unsigned int texZ);
    ...
    void
    setPaintChannelValues(const std::vector<Real> *theChannelModifList)
    {
      channelModifList = theChannelModifList;
    }
    ;
    void
    paint(const Vector3 &currpoint, const Real paintForce,
        const PagingLandScapeTileInfo *info);

    void
    deformHeight(const Vector3 &currpoint, const PagingLandScapeTileInfo *info);

    String
    getNextTextureFormat();
    String
    getCurrentTextureFormat();
    ...
    clearTextureFormats();
    void
    registerTextureType(PagingLandScapeTexture* source)
    {
      mTextureTypeMap.push_back(source);
    }

    PagingLandScapeTexture*
    getTexture(const unsigned int i, const unsigned int j,
        const bool alwaysReturn = true);
    PagingLandScapeTexture*
    getNewTexture(const unsigned int i, const unsigned int j);
    void
    releaseTexture(PagingLandScapeTexture*p);

    inline PagingLandScapeSceneManager *
    getSceneManager()
    {
      return mSceneManager;
    }
    inline PagingLandScapeOptions*
    getOptions()
    {
      return mOptions;
    }
