...
namespace Ogre
{
  ...
  public:
    PagingLandScapeTexture(PagingLandScapeTextureManager *pageMgr,
        const String materialName, const unsigned int numTexture = 0,
    ...
    virtual void
    bindCompressionSettings(GpuProgramParametersSharedPtr params);
    virtual void
    bindCompressionSettings();

    virtual void
    load(unsigned int mX, unsigned int mZ);
    virtual void
    unload();

    // Real Method
    bool
    getCastsShadows() const
    {
      return false;
    }
    ...
    void
    getCoordinates(unsigned int& X, unsigned int& Z) const
    {
      X = mDataX;
      Z = mDataZ;
    }
    ...
    inline bool
    isCoord(unsigned int x, unsigned int z) const
    {
