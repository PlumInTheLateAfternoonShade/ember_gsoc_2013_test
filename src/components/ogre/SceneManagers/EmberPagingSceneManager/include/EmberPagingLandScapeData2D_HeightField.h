...
namespace Ember
{
  namespace OgreView
  {
    ...
    {
    public:
      EmberPagingLandScapeData2D_HeightField(
          Ogre::PagingLandScapeData2DManager* pageMgr);
      virtual Ogre::String
      getName() const
      {
      ...
      getNormal(const Ogre::Real& localPageX,
          const Ogre::Real& localPageZ) const;
      virtual Ogre::ColourValue
      getBase(const Ogre::Real& mX, const Ogre::Real& mZ) const;
      virtual Ogre::ColourValue
      getCoverage(const Ogre::Real& mX, const Ogre::Real& mZ) const;
      virtual Ogre::Real
      getShadow(const Ogre::Real& mX, const Ogre::Real& mZ,
          bool positive) const;

      virtual Ogre::PagingLandScapeData2D*
      newPage();

      virtual Ogre::Real
      getMaxAbsoluteHeight() const;

      void
      setMaxHeight(float maxHeight);

      void
      eventTerrainPageLoaded();
    ...
    protected:
      ...
      _save();
      virtual bool
      _load(unsigned int x, unsigned int z);
      virtual void
      _load();
      virtual void
      _unload();
