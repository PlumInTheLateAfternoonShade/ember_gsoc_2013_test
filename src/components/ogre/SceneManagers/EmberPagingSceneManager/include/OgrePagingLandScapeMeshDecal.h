...
namespace Ogre
{
  ...
  protected:
    MovableObject*
    createInstanceImpl(const String& name, const NameValuePairList* params);
  public:
    ...
    getType(void) const;
    void
    destroyInstance(MovableObject* obj);
  };

  class PagingLandScapeMeshDecal : public Renderable,
  ...
  private:
    ...
    struct Vertex
    {
      Vertex(Real x, Real y, Real z, bool included) :
          vertex_(x, y, z), included_(included)
      {
      }
      Vector3 vertex_;
      bool included_;
  ...
  public:
    PagingLandScapeMeshDecal(const String& name, const String& materialName,
        const Vector2& size, const String& sceneMgrInstanceName);
    virtual
    ...
    virtual void
    _notifyAttached(Node* parent, bool isTagPoint = false);
  ...
  private:
    ...
    GetNumIndices() const;
    int
    GetRenderLevel(int x, int z) const
    {
      assert(x >= 0 && x < width_);
    ...
    template<class T>
      int
      CreateIndices(HardwareIndexBufferSharedPtr ibuf);
    template<class T>
      int
      MeshSubSection(int x, int z, T*& indices, int& indexOffset);
    template<class T>
      int
      StitchEdge(T*& indices, const SubExtent& extents, const int hiLOD,
          const int loLOD, const bool horizontal, const bool forward,
          const bool omitFirstTri, const bool omitLastTri,
          const int indexOffset);
  protected:
    // From MovableObject
    const String&
    getMovableType() const
    {
    ...
    void
    _updateRenderQueue(RenderQueue* queue);
    ...
    virtual void
    getWorldTransforms(Matrix4* xform) const;
    virtual Real
    getSquaredViewDepth(const Camera* cam) const;
    const LightList&
    getLights() const
    {
      return lightList_;
    ...
    bool
    queryResult(MovableObject* object)
    {
      return false;
    }
    bool
    queryResult(SceneQuery::WorldFragment* fragment);
  };
}
#endif
