...
namespace Ember
{
  namespace OgreView
  {
    ...
    public:
      ...
      virtual void
      observeArea(const WFMath::AxisBox<2>& area);
      virtual void
      observeArea(const Ogre::TRect<int>& area);

      /**
       * @brief Callback method for when a new tile is shown.
       */
      void
      tileShow(Ogre::PagingLandscapeEvent* event);
    ...
    private:
       ...
       */
      bool
      testIntersects(const Ogre::TRect<Ogre::Real>& rect) const;
