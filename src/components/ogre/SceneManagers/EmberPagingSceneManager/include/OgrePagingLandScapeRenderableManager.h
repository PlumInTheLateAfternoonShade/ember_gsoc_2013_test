...
namespace Ogre
{
  ...
  public:
    ...
    void
    setRenderableManager(PagingLandScapeRenderableManager *rMgr)
    {
  ...
  protected:
    ...
    allocate();
    void
    deallocate(PagingLandScapeRenderable *r);
  ...
  public:
     ...
     */
    void
    freeRenderable(PagingLandScapeRenderable* rend);

    /** Set this renderable to be loaded
     */
    void
    queueRenderableLoading(PagingLandScapeTile* tile);

    /** Set this renderable to be unloaded
     */
    void
    unqueueRenderable(PagingLandScapeTile* tile);

    /** Load a set of renderables
     */
    bool
    executeRenderableLoading(const Vector3 &Cameraposition);

    size_t
    numRenderables(void) const;
    size_t
    numFree(void) const;
    size_t
    numLoading(void) const;

    void
    InitTextureCoordinatesBuffers(void);
    void
    freeTextureCoordinatesBuffers(void);
    HardwareVertexBufferSharedPtr
    getTextureCoordinatesBuffers(const unsigned int tilex,
        const unsigned int tilez);
    void
    setTextureCoordinatesBuffers(const unsigned int tilex,
        const unsigned int tilez, const HardwareVertexBufferSharedPtr& data);

    unsigned int
    numVisibles(void) const
    {
  ...
  protected:
    ...
    void
    _addBatch(const unsigned int num);
