...
namespace OgreOpcode
{
  ...
  TerrainCollisionShape::TerrainCollisionShape(const Ogre::String& name) :
      ICollisionShape(name), mVerticesPerRow(0)
  {
  }
  ...
  bool
  TerrainCollisionShape::load(int numVertex, float *vertices,
      int verticesPerRow, int rowCount)
  {
  ...
  /// @return bool <TODO: insert return value description here>
  bool
  TerrainCollisionShape::_refitToCachedData()
  {
    assert(mVertexBuf);
  ...
  /// @return bool <TODO: insert return value description here>
  bool
  TerrainCollisionShape::_rebuildFromCachedData()
  {
    //assert(mVertexBuf && mFaceBuf);
  ...
  void
  TerrainCollisionShape::visualize(Details::OgreOpcodeDebugger* activeDebugger)
  {
