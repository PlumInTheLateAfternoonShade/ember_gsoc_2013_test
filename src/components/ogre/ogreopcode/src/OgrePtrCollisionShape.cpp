...
namespace OgreOpcode
{
  ...
  PtrCollisionShape::~PtrCollisionShape()
  {
    ...
    if (mParentNode != NULL)
      CollisionManager::getSingleton().getSceneManager()->getRootSceneNode()->removeAndDestroyChild(
          mParentNode->getName());
    mParentNode = NULL;
  }
  ...
  bool
  PtrCollisionShape::load(size_t numVertex, size_t numIndices, float *vertices,
      size_t *indices)
  {
    ...
    mParentNode =
        CollisionManager::getSingleton().getSceneManager()->getRootSceneNode()->createChildSceneNode(
            "ptrcollnode" + this->getName());
  ...
  PtrCollisionShape::rebuild()
  {
    ...
    opcMeshAccess.SetPointers((const IceMaths::IndexedTriangle*) mFaceBuf,
  ...
  /// @return bool <TODO: insert return value description here>
  bool
  PtrCollisionShape::_refitToCachedData()
  {
    assert(mVertexBuf);
  ...
  /// @return bool <TODO: insert return value description here>
  bool
  PtrCollisionShape::_rebuildFromCachedData()
  {
    assert(mVertexBuf && mFaceBuf);
