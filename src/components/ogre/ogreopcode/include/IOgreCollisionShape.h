...
namespace OgreOpcode
{
  ...
  public:
    ...
    virtual void
    visualize(Details::OgreOpcodeDebugger* activeDebugger);
    ...
    virtual void
    setStatic(bool newstatic = true)
    {
      mShapeIsStatic = newstatic;
    }
    /// visualise OPCODE AABB tree
    virtual void
    visualizeAABBs(Details::OgreOpcodeDebugger* activeDebugger);
    ...
    virtual Ogre::Matrix4
    getFullTransform(void) const;
    /// return the local transformation matrix
    virtual Ogre::Matrix4&
    getLocalTransform(void) const;
    ///
    virtual void
    setTransform(const Ogre::Matrix4 newTransform);
    /// returns the scenenode which this shape is attached to
    virtual Ogre::SceneNode*
    getParentSceneNode(void) const;
    ...
    getRefitRate();
    void
    setRefitRate(unsigned int NumFramesPerRefit);
    /// return current world space AABB min and max
    virtual void
    getMinMax(Ogre::Vector3& bMin, Ogre::Vector3& bMax) const;
    /// return current object space AABB min and max
    virtual void
    getLocalMinMax(Ogre::Vector3& bMin, Ogre::Vector3& bMax) const;
    /// perform collision with other ICollisionShape
    virtual bool
    collide(CollisionType collType, Ogre::Matrix4& ownMatrix,
        ICollisionShape* otherShape, Ogre::Matrix4& otherMatrix,
        CollisionPair& collPair);
    /// perform collision with line
    virtual bool
    rayCheck(CollisionType collType, const Ogre::Matrix4& ownMatrix,
        const Ogre::Ray& line, const Ogre::Real dist, CollisionPair& collPair,
        bool rayCulling);
    /// perform a sphere check
    virtual bool
    sphereCheck(CollisionType collType, const Ogre::Matrix4& ownMatrix,
        const Ogre::Sphere& ball, CollisionPair& collPair);
    /// perform a swept sphere check
    virtual bool
    sweptSphereCheck(CollisionType collType, const Ogre::Matrix4& ownMatrix,
        const Ogre::Vector3& position, const Ogre::Vector3& movementVector,
        const Ogre::Real& radius, CollisionPair& collPair);
    /// get tri coords from tri index
    virtual void
    getTriCoords(size_t index, Ogre::Vector3& v0, Ogre::Vector3& v1,
        Ogre::Vector3& v2);

    /////////////////////////////////////////
    // DAVE:
    inline bool
    getLowestRoot(Ogre::Real a, Ogre::Real b, Ogre::Real c, Ogre::Real maxR,
        Ogre::Real* root);
    void
    sphereEdgeCheck(Ogre::Vector3 &velocity, Ogre::Vector3 &edge,
        Ogre::Vector3 &baseToVertex, Ogre::Real &t, bool &foundCollision,
        Ogre::Vector3 &collisionPoint, Ogre::Vector3 &pnt);
    bool
    testTriangleIntersection(Ogre::Vector3 position,
        Ogre::Vector3 movementVector, Ogre::Real radius, Ogre::Vector3 v0,
        Ogre::Vector3 v1, Ogre::Vector3 v2, CollisionPair *cp);
  ...
  protected:
    ...
    virtual void
    visualizeAABBCollisionNode(const Opcode::AABBCollisionNode* node);
    /// visualize the AABBTree of the opcode model (for no leaf trees)
    virtual void
    visualizeAABBNoLeafNode(const Opcode::AABBNoLeafNode* node);
    /// prepare OPCODE parameters
    virtual void
    _prepareOpcodeCreateParams(Opcode::OPCODECREATE& opcc);
  ...
  ICollisionShape::getParentSceneNode(void) const
  {
    return mParentNode;
  }
  ...
  inline
  void
  ICollisionShape::getTriCoords(size_t index, Ogre::Vector3& v0,
      Ogre::Vector3& v1, Ogre::Vector3& v2)
  {
