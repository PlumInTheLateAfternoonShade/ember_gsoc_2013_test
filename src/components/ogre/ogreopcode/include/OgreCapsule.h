  ...
  namespace Details
  {
    ...
    public:
      ...
      Capsule(const Capsule& c) :
          start(c.start), end(c.end), radius(c.radius)
      {
      }

      /** Complete constructor
       */
      Capsule(const Ogre::Vector3& s, const Ogre::Vector3& e, Ogre::Real r) :
       ...
       */
      Capsule(Ogre::Real sx, Ogre::Real sy, Ogre::Real sz, Ogre::Real ex,
          Ogre::Real ey, Ogre::Real ez, Ogre::Real r) :
      ...
      length2() const
      {
        return (start - end).squaredLength();
      }
...
// intersection tests
       ...
       */
      bool
      contains(const Ogre::Vector3& point) const;

      /** Tests intersection between this capsule and the given Axis-Aligned
       *  Bounding Box
       */
      bool
      intersects(const Aabb& aabb) const;
       ...
       */
      bool
      intersects(const OrientedBox& obb) const;

      /** Tests intersection between this capsule and the given one
       */
      bool
      intersects(const Capsule& cap) const;
