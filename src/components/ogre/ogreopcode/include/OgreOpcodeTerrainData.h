...
namespace OgreOpcode
{
  ...
  public:
    ...
    TerrainData(const std::vector<float>& HeightmapData, size_t Width,
        size_t Depth, Ogre::Vector3 Scale, Ogre::Vector3 Offset =
            Ogre::Vector3::ZERO);
    ~TerrainData();
    ...
    {
      return (*mHeightmapData)[x + y * mWidth];
    }

    // It is up to the user to properly destroy the created Tile Shapes, just like all other created shapes!
    std::vector<OgreOpcode::PtrCollisionShape*>*
    createTileShapes(unsigned int numTilesX = 2, unsigned int numTilesZ = 2);
