...
namespace OgreOpcode
{
  ...
  public:
    CollisionInfo() :
        this_contact(Ogre::Vector3::ZERO), other_contact(Ogre::Vector3::ZERO), contact(
            Ogre::Vector3::ZERO), distance(0.0f), this_normal(
            Ogre::Vector3::ZERO), other_normal(Ogre::Vector3::ZERO)
    {
    }
    ;
  ...
  public:
    CollisionPair() :
        this_object(0), other_object(0), this_contact(Ogre::Vector3::ZERO), other_contact(
            Ogre::Vector3::ZERO), contact(Ogre::Vector3::ZERO), distance(0.0f), this_normal(
            Ogre::Vector3::ZERO), other_normal(Ogre::Vector3::ZERO), tstamp(
            0.0f), numBVBVTests(0), numBVPrimTests(0), numPrimPrimTests(0)
    {
    }
    ;
  ...
  private:
    ...
    bool
    get_merged_id(int id1, int id2, unsigned int& mrg)
    {
      if (id1 > id2)
        {
  ...
  public:
    ...
    bool
    collisionExists(int id1, int id2)
    {
    ...
    /// add a new collision 
    void
    addCollision(CollisionPair& cr, int id1, int id2)
    {
    ...
    bool
    collisionTested(int id1, int id2)
    {
      // generate the merged 32 bit id, and query key array
      // for the collision
      unsigned int key;
    ...
    void
    addCollisionTest(int id1, int id2)
    {
    ...
    int
    getCollisions(CollisionObject *co, CollisionPair **& cr_ptr)
    {
    ...
    /// @return number of collisions this object is involved in
    int
    getCollisions(CollisionObject *co)
    {
      // fill report array with all collisions which this
      // object is involved in.
      assert(co);
    ...
    int
    getAllCollisions(CollisionPair **& cr_ptr)
    {
      //coll_pairs.sort()
    ...
    int
    getClosestCollision(const Ogre::Vector3& v, CollisionPair **& crPtr)
    {
