...
namespace OgreOpcode
{
  namespace Details
  {
    ...
    typedef bool
    (*T_Overlap)(const BP_Proxy&, const BP_Proxy&);
    ...
    public:
      ...
      void
      setPos(Ogre::Real pos)
      {
        m_pos = pos;
      }

      void
      move(Ogre::Real x, BP_Scene& scene, T_Overlap);

      friend void
      encounters(const BP_Endpoint& a, const BP_Endpoint& b, BP_Scene& scene,
          T_Overlap overlap);

      friend bool
      operator<(const BP_Endpoint& a, const BP_Endpoint& b);
