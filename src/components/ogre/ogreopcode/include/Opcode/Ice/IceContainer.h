...
public:
  ...
  inline_
  Container&
  Add(const udword* entries, udword nb)
  {
    // Resize if needed
  ...
  inline_
  Container&
  Add(const float* entries, udword nb)
  {
    // Resize if needed
    if (mCurNbEntries + nb > mMaxNbEntries)
      Resize(nb);
  ...
  // Helpers
  Container&
  FindNext(udword& entry, FindMode find_mode = FIND_CLAMP);
  Container&
  FindPrev(udword& entry, FindMode find_mode = FIND_CLAMP);
  // Data access.
  inline_
  udword
  GetNbEntries() const
  ...
  inline_
  float
  GetGrowthFactor() const
  {
    return mGrowthFactor;
  }	//!< Returns the growth factor
  inline_
  void
  SetGrowthFactor(float growth)
  {
  ...
  void
  operator =(const Container& object);

#ifdef CONTAINER_STATS
  inline_
  udword
  GetNbContainers() const
  {
