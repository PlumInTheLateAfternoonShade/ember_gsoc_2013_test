...
inline_ void
SetLeastNBits(udword& x, udword n)
{
  x |= ~(~0 << n);
}
 ...
 */
inline_ float
FeedbackFilter(float val, float& memory, float sharpness)
{
  ASSERT(
...
// Generic functions
template<class Type> inline_ void TSwap(Type& a, Type& b)
  { const Type c = a; a = b; b = c;}
template<class Type> inline_ Type TClamp(const Type& x, const Type& lo, const Type& hi)
...
inline_ void
Compute2DCoords(udword& u, udword& v, udword i, udword nbu)
{
  v = i / nbu;
  u = i - (v * nbu);
}
...
inline_ void
Compute3DCoords(udword& u, udword& v, udword& w, udword i, udword nbu,
    udword nbu_nbv)
{
  w = i / (nbu_nbv);
