...
public:
  ...
  inline_
  HPoint(float _x, float _y, float _z, float _w = 0.0f) :
      Point(_x, _y, _z), w(_w)
  {
  }
  //! Constructor from array
  inline_
  HPoint(const float f[4]) :
      Point(f), w(f[3])
  {
  }
  //! Constructor from a Point
  inline_
  HPoint(const Point& p, float _w = 0.0f) :
      Point(p), w(_w)
  {
  }
  //! Destructor
  ...
  inline_
  HPoint&
  Add(float _x, float _y, float _z, float _w)
  {
    x += _x;
    y += _y;
    z += _z;
  ...
  inline_
  HPoint&
  Sub(float _x, float _y, float _z, float _w)
  {
    x -= _x;
    y -= _y;
    z -= _z;
  ...
  inline_
  friend HPoint
  operator*(float s, const HPoint& p)
  {
