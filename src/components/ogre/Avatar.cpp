...
namespace Ember
{
  namespace OgreView
  {
    ...
    Avatar::Avatar(EmberEntity& erisAvatarEntity, Scene& scene,
        ...
        SetAttachedOrientation("setattachedorientation", this,
            "Sets the orientation of an item attached to the avatar: <attachpointname> <x> <y> <z> <degrees>"), mErisAvatarEntity(
            erisAvatarEntity), mMaxSpeed(5), mAvatarAttachmentController(
            new AvatarAttachmentController(*this)), mCameraMount(
            new Camera::ThirdPersonCameraMount(cameraSettings,
                scene.getSceneManager())), mIsAdmin(false), mHasChangedLocation(
            false), mChatLoggerParent(nullptr), mIsMovingServerOnly(false), mScene(
            scene), mEntityMaker(
            new Authoring::EntityMaker(erisAvatarEntity,
                *EmberServices::getSingleton().getServerService().getConnection()))
    {
      setMinIntervalOfRotationChanges(1000); //milliseconds

      MainLoopController::getSingleton().EventAfterInputProcessing.connect(
          sigc::mem_fun(*this, &Avatar::application_AfterInputProcessing));

      registerConfigListener("general", "logchatmessages",
          sigc::mem_fun(*this, &Avatar::Config_LogChatMessages));
      registerConfigListener("general", "avatarrotationupdatefrequency",
          sigc::mem_fun(*this, &Avatar::Config_AvatarRotationUpdateFrequency));
      registerConfigListener("input", "maxspeed",
          sigc::mem_fun(*this, &Avatar::Config_MaxSpeed));
      ...
      Eris::TypeService* typeService =
          EmberServices::getSingleton().getServerService().getConnection()->getTypeService();
      if (mErisAvatarEntity.getType()->isA(
          typeService->getTypeByName("creator")))
        {
          mIsAdmin = true;
          EmberServices::getSingleton().getServerService().getAvatar()->setIsAdmin(
              true);
        }
      else
        {
      ...
      mErisAvatarEntity.LocationChanged.connect(
          sigc::mem_fun(*this, &Avatar::avatar_LocationChanged));
      mErisAvatarEntity.Moved.connect(
          sigc::mem_fun(*this, &Avatar::avatar_Moved));
      mErisAvatarEntity.ChildAdded.connect(
          sigc::mem_fun(*this, &Avatar::entity_ChildAdded));
      mErisAvatarEntity.ChildRemoved.connect(
          sigc::mem_fun(*this, &Avatar::entity_ChildRemoved));

      mClientSideAvatarOrientation = mErisAvatarEntity.getOrientation();
      mClientSideAvatarPosition = mErisAvatarEntity.getPosition();

      mErisAvatarEntity.setAttachmentControlDelegate(
          mAvatarAttachmentController);
    ...
    void
    Avatar::runCommand(const std::string &command, const std::string &args)
    {
          ...
          if (attachPointName != "")
            {
              ...
              if (x != "" && y != "" && z != "" && degrees != "")
                {
                  Ogre::Degree ogreDegrees(
                      Ogre::StringConverter::parseReal(degrees));
                  Ogre::Quaternion rotation(ogreDegrees,
                      Ogre::Vector3(Ogre::StringConverter::parseReal(x),
                          Ogre::StringConverter::parseReal(y),
                          Ogre::StringConverter::parseReal(z)));
                  Model::ModelRepresentation* modelRepresentation =
                      Model::ModelRepresentationManager::getSingleton().getRepresentationForEntity(
                          mErisAvatarEntity);
                  if (modelRepresentation)
                    {
                      Model::Model& model = modelRepresentation->getModel();
                      const Model::Model::AttachPointWrapperStore* attachPoints =
                          model.getAttachedPoints();
                      if (attachPoints)
                        {
                          for (Model::Model::AttachPointWrapperStore::const_iterator I =
                              attachPoints->begin(); I != attachPoints->end();
                              ++I)
                            {
    ...
    Camera::ThirdPersonCameraMount&
    Avatar::getCameraMount() const
    {
      return *mCameraMount;
    }

    void
    Avatar::setMinIntervalOfRotationChanges(Ogre::Real milliseconds)
    {
      mMinIntervalOfRotationChanges = milliseconds;
    }

    void
    Avatar::application_AfterInputProcessing(float timeSinceLastEvent)
    {
      attemptMove();
    }

    void
    Avatar::moveClientSide(const WFMath::Quaternion& orientation,
        const WFMath::Vector<3>& movement, float timeslice)
    {
      ...
      if (movement != WFMath::Vector<3>::ZERO())
        {
          ...
          if (isOkayToSendRotationMovementChangeToServer())
            {
              ...
              adjustedOrientation.fromRotMatrix(
                  WFMath::RotMatrix<3>().rotationZ(
                      atan2(rotator.y(), rotator.x())));
              ...
              mClientSideAvatarOrientation.rotate(
                  WFMath::Quaternion(WFMath::Vector<3>(0, 0, 1),
                      WFMath::numeric_constants < WFMath::CoordType
                          > ::pi() / 2));
            }
          ...
          adjustedOrientation.rotate(
              WFMath::Quaternion(WFMath::Vector<3>(0, 0, -1),
                  WFMath::numeric_constants < WFMath::CoordType > ::pi() / 2));
    ...
    Avatar::attemptMove()
    {
      ...
      bool isMoving = mCurrentMovement.isValid()
          && mCurrentMovement != WFMath::Vector<3>::ZERO();
      bool wasMoving = mCurrentMovementState.movement.isValid()
          && mCurrentMovementState.movement != WFMath::Vector<3>::ZERO();
      ...
      if (!wasMoving)
      ...
      else
        {
          ...
          else if (newMovementState.movement != mCurrentMovementState.movement
              || newMovementState.orientation
                  != mCurrentMovementState.orientation)
            {
      ...
      if (sendToServer)
        {
          std::stringstream ss;
          ss << "Sending move op to server, direction: "
              << newMovementState.movement << ", orientation: "
              << newMovementState.orientation << ", speed: "
              << sqrt(newMovementState.movement.sqrMag()) << ".";
          ...
          mLastTransmittedMovements.push_back(
              TimedMovementStateList::value_type(currentTime,
                  newMovementState));
          if (mLastTransmittedMovements.size() > 10)
            {
              mLastTransmittedMovements.erase(
                  mLastTransmittedMovements.begin());
            }
          EmberServices::getSingleton().getServerService().moveInDirection(
              newMovementState.movement, newMovementState.orientation);
    ...
    EmberEntity&
    Avatar::getEmberEntity()
    {
      return mErisAvatarEntity;
    }

    bool
    Avatar::isOkayToSendRotationMovementChangeToServer()
    {
      ...
      long long currentTime = Time::currentTimeMillis();
      if ((currentTime - mLastTransmittedMovements.rbegin()->first)
          > mMinIntervalOfRotationChanges)
        {
          return true;
        }
      return false;
    ...
    Ogre::Node*
    Avatar::getAvatarSceneNode() const
    {
      NodeAttachment* attachment =
          dynamic_cast<NodeAttachment*>(mErisAvatarEntity.getAttachment());
      if (attachment)
        {
    ...
    void
    Avatar::avatar_LocationChanged(Eris::Entity* entity)
    {
      mCameraMount->attachToNode(getAvatarSceneNode());
    ...
    void
    Avatar::entity_ChildAdded(Eris::Entity* childEntity)
    {
      EventAddedEntityToInventory.emit(static_cast<EmberEntity*>(childEntity));
    }

    void
    Avatar::entity_ChildRemoved(Eris::Entity* childEntity)
    {
      EventRemovedEntityFromInventory.emit(
          static_cast<EmberEntity*>(childEntity));
    }

    void
    Avatar::Config_AvatarRotationUpdateFrequency(const std::string& section,
        const std::string& key, varconf::Variable& variable)
    {
      setMinIntervalOfRotationChanges(static_cast<double>(variable));
    }

    void
    Avatar::Config_MaxSpeed(const std::string& section, const std::string& key,
    ...
    void
    Avatar::Config_LogChatMessages(const std::string& section,
        const std::string& key, varconf::Variable& variable)
    {
      if (static_cast<bool>(variable))
        {
          mChatLoggerParent = std::unique_ptr < AvatarLoggerParent
              > (new AvatarLoggerParent(*this));
        }
      else
        {
    ...
    WFMath::Point<3>
    Avatar::getClientSideAvatarPosition() const
    {
...
//	}
    ...
    WFMath::Quaternion
    Avatar::getClientSideAvatarOrientation() const
    {
      //NOTE: for now we've deactivated the client side prediction as it doesn't really work as it should
      return
          mErisAvatarEntity.getOrientation().isValid() ?
...
//	}
    ...
    WFMath::Vector<3>
    Avatar::getClientSideAvatarVelocity() const
    {
      //NOTE: for now we've deactivated the client side prediction as it doesn't really work as it should
      return
          mErisAvatarEntity.getVelocity().isValid() ?
              mErisAvatarEntity.getVelocity() : WFMath::Vector<3>::ZERO();
//	if (mIsMovingServerOnly) {
