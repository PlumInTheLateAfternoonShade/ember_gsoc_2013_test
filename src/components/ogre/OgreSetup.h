...
namespace Ember
{
  namespace OgreView
  {
    ...
    private:
      ...
      void
      parseWindowGeometry(Ogre::ConfigOptionMap& config, unsigned int& width,
          unsigned int& height, bool& fullscreen);

      void
      input_SizeChanged(unsigned int width, unsigned int height);

    };

    inline Ogre::RenderWindow*
    OgreSetup::getRenderWindow() const
    {
