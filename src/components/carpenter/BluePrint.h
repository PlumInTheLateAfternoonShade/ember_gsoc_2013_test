...
namespace Carpenter
{
  ...
  public:
    BuildingBlockBinding(const BuildingBlock* block1, const AttachPoint* point1,
        const BuildingBlock* block2, const AttachPoint* point2);
    const std::string&
    getType() const;
    const BuildingBlock*
    getBlock1() const
    ...
    const AttachPoint*
    getAttachPoint1() const
    {
      return mPoint1;
    }
    const AttachPoint*
    getAttachPoint2() const
    {
      return mPoint2;
    }
  ...
  public:
    BuildingBlock();
    const std::vector<BuildingBlockBinding*>
    getBindingsForBlock() const;
    const AttachPair*
    getAttachPair(const std::string& name);
    const std::string&
    getName() const
    {
      return mBlockDefinition.mName;
    }

    WFMath::Point<3>
    getWorldPositionForPoint(const AttachPoint* point);
    bool
    isAttached() const;

    void
    setPosition(WFMath::Point<3> position);
    void
    setOrientation(WFMath::Quaternion orientation);
    const WFMath::Point<3>&
    getPosition() const;
    const WFMath::Quaternion&
    getOrientation() const;

    const BuildingBlockSpec*
    getBuildingBlockSpec() const;

    const BlockSpec*
    getBlockSpec() const;

    const std::vector<const AttachPoint*>
    getAllPoints() const;
     ...
     */
    int
    getNumberOfChildBindings() const;
  ...
  protected:
     ...
     */
    void
    removeBoundPoint(const AttachPoint* point);
  ...
  inline bool
  BuildingBlock::isAttached() const
  {
    return mAttached;
  }
  inline void
  BuildingBlock::setPosition(WFMath::Point<3> position)
  {
    mPosition = position;
  }
  inline void
  BuildingBlock::setOrientation(WFMath::Quaternion orientation)
  {
    mOrientation = orientation;
  }
  inline const WFMath::Point<3>&
  BuildingBlock::getPosition() const
  {
    return mPosition;
  }
  inline const WFMath::Quaternion&
  BuildingBlock::getOrientation() const
  {
    return mOrientation;
  }
  inline const BuildingBlockSpec*
  BuildingBlock::getBuildingBlockSpec() const
  {
    return mBuildingBlockSpec;
  }
  inline int
  BuildingBlock::getNumberOfChildBindings() const
  {
    return mChildBindings;
  }
...
//    	void deleteBuildingBlock(const std::string & name);
    BuildingBlockBinding*
    addBinding(BuildingBlockBindingDefinition definition);
    BuildingBlockBinding*
    addBinding(const BuildingBlock* block1, const AttachPoint* point1,
        const BuildingBlock* block2, const AttachPoint* point2);
    const std::vector<BuildingBlock*>
    getAttachedBlocks() const;
    const std::list<BuildingBlockBinding>*
    getBindings() const;
     ...
     */
    void
    setStartingBlock(const std::string& name);
    const BuildingBlock*
     ...
     */
    void
    placeBindings(BuildingBlock* unboundBlock,
        std::vector<BuildingBlockBinding*> bindings);
     ...
     */
    bool
    isRemovable(const BuildingBlock* bblock) const;
     ...
     */
    bool
    remove(const BuildingBlock* bblock);
  ...
  protected:
    ...
    void
    doBindingsForBlock(BuildingBlock *block);

  };

  inline Carpenter* const
  BluePrint::getCarpenter()
  {
    return mCarpenter;
  }
  inline const std::string&
  BluePrint::getName() const
  {
    return mName;
  }
  inline const BuildingBlock*
  BluePrint::getStartingBlock() const
  {
