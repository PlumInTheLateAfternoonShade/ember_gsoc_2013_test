...
namespace Ember
{
  ...
  void
  SoundSource::setPosition(const WFMath::Point<3>& pos)
  {
    assert(pos.isValid());
  ...
  void
  SoundSource::setVelocity(const WFMath::Vector<3>& vel)
  {
    assert(vel.isValid());
  ...
  void
  SoundSource::setOrientation(const WFMath::Quaternion& orientation)
  {
    //TODO: implement this
  }
