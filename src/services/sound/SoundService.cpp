...
namespace Ember
{
          ...
          , mEnabled(false)
  {
    ...
    setDescription(
        "Service for reproduction of sound effects and background music");
  }
  ...
  SoundService::start()
  {
    ...
    if (EmberServices::getSingleton().getConfigService().hasItem("audio",
        "enabled")
        && static_cast<bool>(EmberServices::getSingleton().getConfigService().getValue(
            "audio", "enabled")) == false)
      {
    ...
    else
      {
        ...
        if (isEnabled())
          {
            S_LOG_FAILURE(
                "Can't start the sound system if it's already started.");
          }
        else
          {
  ...
  void
  SoundService::stop(int code)
  {
    for (SoundInstanceStore::iterator I = mInstances.begin();
        I != mInstances.end(); ++I)
      {
        S_LOG_WARNING(
    ...
    for (SoundSampleStore::iterator I = mBaseSamples.begin();
        I != mBaseSamples.end(); ++I)
      {
        delete I->second;
      }
    mBaseSamples.clear();
...
// 	}
  ...
  bool
  SoundService::isEnabled() const
  {
    return mEnabled;
  }

  void
  SoundService::runCommand(const std::string& command, const std::string& args)
  {
  }

  void
  SoundService::registerStream(StreamedSoundSample* copy)
  {
    /*		#ifdef THREAD_SAFE
  ...
  bool
  SoundService::unregisterStream(const StreamedSoundSample* sample)
  {
// 		#ifdef THREAD_SAFE
// 		pthread_mutex_lock(&mSamplesMutex);
// 		#endif
...
// 		#endif
  ...
  void
  SoundService::updateListenerPosition(const WFMath::Point<3>& pos,
      const WFMath::Vector<3>& direction, const WFMath::Vector<3>& up)
  {
    if (!isEnabled())
      {
        return;
  ...
  BaseSoundSample*
  SoundService::createOrRetrieveSoundSample(const std::string& soundPath)
  {
    ...
    if (mResourceProvider)
      {
        ...
        if (resWrapper.hasData())
          {
            StaticSoundSample* sample = new StaticSoundSample(resWrapper, false,
                1.0);
            mBaseSamples.insert(
                SoundSampleStore::value_type(soundPath, sample));
            return sample;
          }
      }
    return 0;
  ...
  bool
  SoundService::destroySoundSample(const std::string& soundPath)
  {
    SoundSampleStore::iterator I = mBaseSamples.find(soundPath);
    if (I != mBaseSamples.end())
      {
  ...
  bool
  SoundService::destroyInstance(SoundInstance* instance)
  {
    SoundInstanceStore::iterator I = std::find(mInstances.begin(),
        mInstances.end(), instance);
    if (I != mInstances.end())
      {
        mInstances.erase(I);
        delete instance;
  ...
  IResourceProvider*
  SoundService::getResourceProvider()
  {
    return mResourceProvider;
  }

  void
  SoundService::setResourceProvider(IResourceProvider* resourceProvider)
  {
