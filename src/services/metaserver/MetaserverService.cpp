...
namespace Ember
{
  ...
  MetaserverService::MetaserverService() :
      mMetaserver(0), MetaRefresh("meta_refresh", this,
          "Refresh the meta server listing."), MetaAbort("meta_abort", this,
          "Abort the meta server update process.")
//   , MetaList("meta_list", this, "List all servers.")
  ...
  MetaserverService::start()
  {
    ...
    if (configSrv.itemExists("metaserver", "server"))
      {
        metaserverHostname = std::string(
            configSrv.getValue("metaserver", "server"));
      }
    else
      {
    ...
    S_LOG_INFO(
        "Connecting to meta server at address " << metaserverHostname << ".");
    mMetaserver = new Eris::Meta(metaserverHostname, 20);
    mMetaserver->Failure.connect(
        sigc::mem_fun(*this, &MetaserverService::gotFailure));
    mMetaserver->ReceivedServerInfo.connect(
        sigc::mem_fun(*this, &MetaserverService::receivedServerInfo));
    mMetaserver->CompletedServerList.connect(
        sigc::mem_fun(*this, &MetaserverService::completedServerList));
  ...
  void
  MetaserverService::stop(int code)
  {
    Service::stop(code);
    setStatus(Service::OK);
  }
  ...
  void
  MetaserverService::gotFailure(const string& msg)
  {
    S_LOG_WARNING("Got Meta-server error: " << msg);
  }

  void
  MetaserverService::receivedServerInfo(const Eris::ServerInfo& sInfo)
  {
  ...
  void
  MetaserverService::completedServerList(int count)
  {
  ...
  Eris::Meta&
  MetaserverService::getMetaServer() const
  {
    return *mMetaserver;
  }

  void
  MetaserverService::runCommand(const std::string &command,
  ...
  int
  MetaserverService::compareVersions(const std::string& firstVersion,
      const std::string& secondVersion)
  {
    std::vector<std::string> firstVersionStrings = Tokeniser::split(
        firstVersion, ".");
    std::vector<std::string> secondVersionStrings = Tokeniser::split(
        secondVersion, ".");

    std::vector<std::string>::const_iterator firstI =
        firstVersionStrings.begin();
    std::vector<std::string>::const_iterator secondI =
        secondVersionStrings.begin();

    while (firstI != firstVersionStrings.end()
        && secondI != secondVersionStrings.end())
      {
