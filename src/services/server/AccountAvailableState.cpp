...
namespace Ember
{
  ...
  AccountAvailableState::AccountAvailableState(IState& parentState,
      Eris::Connection& connection) :
      StateBase<LoggedInState>::StateBase(parentState), CreateAcc("create",
          this, "Create an account on the server."), Login("login", this,
          "Login to the connected server."), mAccount(&connection)
  {
    mAccount.LoginFailure.connect(
        sigc::mem_fun(*this, &AccountAvailableState::loginFailure));
    mAccount.LoginSuccess.connect(
        sigc::mem_fun(*this, &AccountAvailableState::loginSuccess));
    mAccount.LogoutComplete.connect(
        sigc::mem_fun(*this, &AccountAvailableState::logoutComplete));
  ...
  void
  AccountAvailableState::loginFailure(const std::string& msg)
  {
  ...
  void
  AccountAvailableState::logoutComplete(bool clean)
  {
