...
namespace Ember
{
  ConnectingState::ConnectingState(IState& parentState, const std::string& host,
      short port) :
      StateBase<ConnectedState>::StateBase(parentState), mConnection(
          std::string("Ember ") + VERSION, host, port, false,
          new ServerServiceConnectionListener(getSignals())), mHasSignalledDisconnected(
          false), mDeleteChildState(0)
  {
    // Bind signals
    mConnection.Connected.connect(
        sigc::mem_fun(*this, &ConnectingState::connected));
    mConnection.StatusChanged.connect(
        sigc::mem_fun(*this, &ConnectingState::statusChanged));
    mConnection.Disconnected.connect(
        sigc::mem_fun(*this, &ConnectingState::disconnected));
    mConnection.Failure.connect(
        sigc::mem_fun(*this, &ConnectingState::gotFailure));
  ...
  void
  ConnectingState::statusChanged(Eris::BaseConnection::Status status)
  {
  ...
  Eris::Connection&
  ConnectingState::getConnection()
  {
    return mConnection;
  }

  void
  ConnectingState::gotFailure(const std::string & msg)
  {
