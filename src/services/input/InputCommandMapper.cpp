...
namespace Ember
{
  ...
  InputCommandMapper::InputCommandMapper(const std::string& state) :
      mState(state), mEnabled(true), mInput(0)
  {
    initKeyMap();
  }
  ...
  void
  InputCommandMapper::readFromConfigSection(const std::string& sectionName)
  {

    //get the mappings from the config service
    ConfigService::SectionMap section =
        EmberServices::getSingleton().getConfigService().getSection(
            sectionName);

    for (ConfigService::SectionMap::const_iterator I = section.begin();
        I != section.end(); ++I)
      {
  ...
  void
  InputCommandMapper::Input_EventKeyPressed(const SDL_keysym& key,
    ...
    if (mEnabled && isActiveForInputMode(inputMode))
      {
        ...
        if (keyI != mKeymap.end())
          {
            std::pair<KeyCommandStore::iterator, KeyCommandStore::iterator> commandsI =
                mKeyCommands.equal_range(keyI->second);
            for (KeyCommandStore::iterator I = commandsI.first;
                I != commandsI.second; ++I)
              {
  ...
  void
  InputCommandMapper::Input_EventKeyReleased(const SDL_keysym& key,
      Input::InputMode inputMode)
  {
    if (mEnabled && isActiveForInputMode(inputMode))
      {
                    ...
                    if (command[0] == '+')
                      {
                        ConsoleBackend& myBackend =
                            ConsoleBackend::getSingleton();
                        //remove the "+" and replace it with "-"
                        myBackend.runCommand(
                            "-" + std::string(command).erase(0, 1), false);
                      }
                  }
              }
          }
  ...
  void
  InputCommandMapper::bindToInput(Input& input)
  {
    input.EventKeyPressed.connect(
        sigc::mem_fun(*this, &InputCommandMapper::Input_EventKeyPressed));
    input.EventKeyReleased.connect(
        sigc::mem_fun(*this, &InputCommandMapper::Input_EventKeyReleased));
    input.registerCommandMapper(this);
    mInput = &input;
  }

  bool
  InputCommandMapper::isActiveForInputMode(Input::InputMode mode) const
  {
    ...
    if (mInputModesRestriction.size() == 0)
      ...
      {
        return std::find(mInputModesRestriction.begin(),
            mInputModesRestriction.end(), mode) != mInputModesRestriction.end();
      }
  }

  void
  InputCommandMapper::bindCommand(const std::string& key,
      const std::string& command)
  {
    S_LOG_INFO(
        "Binding key " << key << " to command " << command << " for state " << getState() << ".");
    mKeyCommands.insert(KeyCommandStore::value_type(key, command));
  }

  void
  InputCommandMapper::unbindCommand(const std::string& key)
  {
  ...
  {
    std::pair<KeyCommandStore::iterator, KeyCommandStore::iterator> commandsI =
        mKeyCommands.equal_range(key);
    for (KeyCommandStore::iterator I = commandsI.first; I != commandsI.second;
        ++I)
      {
        if (I->second == command)
          {
