...
#define FORMAT_PREFIX	"SDL_scrap_0x"
  ...
  /* Convert internal data to scrap format */PRIVATE int
  ...
  /* Convert scrap data to internal format */PRIVATE int
...
#if defined(X11_SCRAP)
  /* The system message filter function -- handle clipboard messages */PRIVATE int
  clipboard_filter(const SDL_Event *event);
#endif
    ...
    /* * */
      {
        ...
        if (XGetWindowProperty(SDL_Display, owner, selection, 0, INT_MAX / 4,
            False, format, &seln_type, &seln_format, &nbytes, &overflow,
