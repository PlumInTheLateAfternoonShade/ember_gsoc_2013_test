...
_MapType_findInMap(const Atlas::Message::MapType* map, const std::string& key)
{
 ...
 */
TOLUA_API int
tolua_isnumber_type(lua_State* L, int lo, int def, tolua_Error* err)
{
 ...
 */
TOLUA_API int
tolua_isstring_type(lua_State* L, int lo, int def, tolua_Error* err)
{
  if (def && lua_gettop(L) < abs(lo))
    return 1;
